...
JQICAL_DEST_VIEWS = File.join(JQICAL_DEST_TEMPLATES, 'app', 'views', 'ui')
...
namespace :build do
  ...
  task :jqical do
    ...
    FileUtils.mkdir_p(JQICAL_DEST_CONFIG)
    FileUtils.mkdir_p(JQICAL_DEST_VIEWS)
    
    open File.join(JQICAL_DEST_TEMPLATES, 'manifest.rb'), 'w' do |manifest|
      manifest.print JQICAL_MESSAGE1
    
      Dir.foreach File.join(JQICAL_SRC, 'app', 'views', 'ui') do |file|
        next unless /\.haml$/ =~ file
        html = File.read File.join(JQICAL_SRC, 'app', 'views', 'ui', file)
        open File.join(JQICAL_DEST_VIEWS, file), 'w' do |f|
          f.print(html)
        end
        manifest.print "file 'app/views/ui/#{file}'\n"
      end      
    
      open File.join(JQICAL_DEST_CONFIG, 'jqical.rb'), 'w' do |f|
