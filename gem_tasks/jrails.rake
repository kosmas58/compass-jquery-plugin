...
JQUERY_UI_18_SRC = File.join(GEM_ROOT, 'src', 'jrails', 'jquery.ui.1.8.4')
...
namespace :build do
  ...
  task :jrails do    
    ...
    open File.join(JRAILS_DEST_TEMPLATES, 'manifest.rb'), 'w' do |manifest|
      ...
      open File.join(JRAILS_DEST_TEMPLATES, 'jquery-ui-1.8.4.js'), 'w' do |f|
        f.print concat_files(all_jquery_ui_scripts)
      end
      manifest.print "javascript 'jquery-ui-1.8.4.js'\n"
    
      open File.join(JRAILS_DEST_TEMPLATES, 'jquery-ui-1.8.4.min.js'), 'w' do |f|
        f.print compress_js(all_jquery_ui_scripts, "google")
      end
      manifest.print "javascript 'jquery-ui-1.8.4.min.js'\n"
