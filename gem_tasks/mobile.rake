  ...
  task :mobile do    
    ...
    open File.join(MOBILE_DEST_TEMPLATES, 'manifest.rb'), 'w' do |manifest|
      ...
      Dir.foreach MOBILE_SRC_THEMES do |theme|
        next if /^\./ =~ theme
        css = File.read File.join(MOBILE_SRC_THEMES, "#{theme}", "jquery.mobile.theme.css")
        # Convert the stylesheet
        open File.join(MOBILE_DEST_THEMES, "#{theme}.scss"), 'w' do |f|
          sass = MOBILE_MESSAGE2 
          IO.popen("sass-convert -F css -T scss", 'r+') { |ff| ff.print(css); ff.close_write; sass += ff.read }
          f.print sass
