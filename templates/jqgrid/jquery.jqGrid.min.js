...
this.text;if(c.p.lastpage===undefined)c.p.lastpage=1});a(c.p.xmlReader.records,p).each(function(){c.p.records=this.textContent||this.text||0});a(c.p.xmlReader.userdata,p).each(function(){c.p.userData[this.getAttribute("name")]=this.textContent||this.text});p=a(c.p.xmlReader.root+" "+c.p.xmlReader.row,p);var Fa=p.length,qa=0;if(p&&Fa){var Ha=parseInt(c.p.rowNum,10),Ja=c.p.scroll?(parseInt(c.p.page,10)-1)*Ha+1:1;if(B)Ha*=B+1;for(B=a.isFunction(c.p.afterInsertRow);qa<Fa;){ia=p[qa];Aa=pa(ia,Ja+qa);I=
t==0?0:t+1;I=(I+qa)%2==1?Ea:"";fa[ua++]='<tr id="'+Aa+'" role="row" class ="ui-widget-content jqgrow ui-row-'+c.p.direction+""+I+'">';if(c.p.rownumbers===true){fa[ua++]=D(0,qa,c.p.page,c.p.rowNum);ca=1}if(c.p.multiselect===true){fa[ua++]=z(Aa,ca,qa);K=1}if(c.p.subGrid===true){fa[ua++]=a(c).jqGrid("addSubGridCell",K+ca,qa+t);X=1}if(c.p.xmlReader.repeatitems){ra||(ra=y(K+X+ca));var Ka=a(c.p.xmlReader.cell,ia);a.each(ra,function(Ba){var ya=Ka[this];if(!ya)return false;O=ya.textContent||ya.text;ja[c.p.colModel[Ba+
K+X+ca].name]=O;fa[ua++]=u(Aa,O,Ba+K+X+ca,qa+t,ia)})}else for(I=0;I<na.length;I++){O=a(na[I],ia).text();ja[c.p.colModel[I+K+X+ca].name]=O;fa[ua++]=u(Aa,O,I+K+X+ca,qa+t,ia)}fa[ua++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){I=c.p.treeANode>=-1?c.p.treeANode:0;F=a(fa.join(""))[0];try{a(c).jqGrid("setTreeNode",ja,F)}catch(La){}Da===0?a("tbody:first",s).append(F):a(c.rows[qa+I+t]).after(F)}else a("tbody:first",s).append(fa.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",c.rows[c.rows.length-
1],K+ca)}catch(Ma){}B&&c.p.afterInsertRow.call(c,Aa,ja,ia);fa=[];ua=0}ja={};Q++;qa++;if(Q==Ha)break}}c.p.gridview===true&&a("tbody:first",s).append(fa.join(""));c.p.totaltime=new Date-M;if(Q>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=Fa}fa=null;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=Q;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},Y=function(p,s,t,A,B){var M=new Date;c.p.reccount=0;if(p){if(c.p.treeANode===
-1&&!c.p.scroll){H(s);t=0}else t=t>0?t:0;var I=0,Q,O,F,K=[],X,ca=0,ha=0,pa=0,na,ra,ja,Da={},ia,Aa=c.rows.length,fa;F=[];ia=0;var ua=c.p.altRows===true?" "+c.p.altclass:"";c.p.page=x(p,c.p.jsonReader.page)||1;ja=x(p,c.p.jsonReader.total);c.p.lastpage=ja===undefined?1:ja;c.p.records=x(p,c.p.jsonReader.records)||0;c.p.userData=x(p,c.p.jsonReader.userdata)||{};c.p.jsonReader.repeatitems||(X=K=C("json"));ja=c.p.keyIndex===false?c.p.jsonReader.id:c.p.keyIndex;if(K.length>0&&!isNaN(ja)){if(c.p.remapColumns&&
...
false){g+=parseInt(this.width,10);if(this.fixed){n+=this.width;m+=this.width+h}else c++;q++}});if(c!=0){b.p.tblwidth=g;o=d-h*c-m;if(!isNaN(b.p.height))if(a(b.grid.bDiv)[0].clientHeight<a(b.grid.bDiv)[0].scrollHeight){l=true;o-=k}g=0;var u=b.grid.cols.length>0;a.each(b.p.colModel,function(v){if(this.hidden===false&&!this.fixed){this.width=f=Math.floor(o/(b.p.tblwidth-n)*this.width);g+=f;b.grid.headers[v].width=f;b.grid.headers[v].el.style.width=f+"px";if(b.p.footerrow)b.grid.footers[v].style.width=
...
this.editoptions.value;if(C){soptions.dataValues=[];if(typeof C==="string"){z=C.split(";");for(n=0;n<z.length;n++){D=z[n].split(":");soptions.dataValues[n]={value:D[0],text:D[1]}}}else if(typeof C==="object"){n=0;for(var y in C){soptions.dataValues[n]={value:y,text:C[y]};n++}}}}if(ignoreHiding&&v||v&&!hidden)l.push(soptions)});if(l.length>0){a("<div id='"+c+"' role='dialog' tabindex='-1'></div>").insertBefore("#gview_"+j.p.id);a("#"+c).searchFilter(l,{groupOps:e.groupOps,operators:u,onClose:g,resetText:e.Reset,
...
this).val();N[this.name]=a("option:selected",this).text();break;case "select-multiple":x[this.name]=a(this).val();x[this.name]=x[this.name]?x[this.name].join(","):"";var V=[];a("option:selected",this).each(function(L,W){V[L]=a(W).text()});N[this.name]=V.join(",");break;case "password":case "text":case "textarea":case "button":x[this.name]=a(this).val();break}if(m.p.autoencode)x[this.name]=a.jgrid.htmlEncode(x[this.name])}});return true}function c(E,G,V,L){for(var W,R,S,T=0,J,ba,Z,ka=[],aa=false,wa,
ta,la="",ma=1;ma<=L;ma++)la+="<td class='CaptionTD ui-widget-content'>&#160;</td><td class='DataTD ui-widget-content' style='white-space:pre'>&#160;</td>";if(E!="_empty")aa=a(G).jqGrid("getInd",E);a(G.p.colModel).each(function(oa){W=this.name;ba=(R=this.editrules&&this.editrules.edithidden==true?false:this.hidden===true?true:false)?"style='display:none'":"";if(W!=="cb"&&W!=="subgrid"&&this.editable===true&&W!=="rn"){if(aa===false)J="";else if(W==G.p.ExpandColumn&&G.p.treeGrid===true)J=a("td:eq("+
oa+")",G.rows[aa]).text();else try{J=a.unformat(a("td:eq("+oa+")",G.rows[aa]),{rowId:E,colModel:this},oa)}catch(Ia){J=a("td:eq("+oa+")",G.rows[aa]).html()}var xa=a.extend({},this.editoptions||{},{id:W,name:W});frmopt=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:false,rowcontent:""},this.formoptions||{});wa=parseInt(frmopt.rowpos)||T+1;ta=parseInt((parseInt(frmopt.colpos)||1)*2);if(E=="_empty"&&xa.defaultValue)J=a.isFunction(xa.defaultValue)?xa.defaultValue():xa.defaultValue;if(!this.edittype)this.edittype=
"text";if(m.p.autoencode)J=a.jgrid.htmlDecode(J);Z=createEl(this.edittype,xa,J,false,a.extend({},a.jgrid.ajaxOptions,G.p.ajaxSelectOptions||{}));if(J==""&&this.edittype=="checkbox")J=a(Z).attr("offval");if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[W]=J;a(Z).addClass("FormElement");S=a(V).find("tr[rowpos="+wa+"]");if(frmopt.rowabove){xa=a("<tr><td class='contentinfo' colspan='"+L*2+"'>"+frmopt.rowcontent+"</td></tr>");a(V).append(xa);xa[0].rp=wa}if(S.length==0){S=a("<tr "+ba+" rowpos='"+wa+"'></tr>").addClass("FormData").attr("id",
...
function k(){var E,G=[true,"",""],V={},L=m.p.prmNames,W,R;if(a.isFunction(d.beforeCheckValues)){var S=d.beforeCheckValues(x,a("#"+q),x[m.p.id+"_id"]=="_empty"?L.addoper:L.editoper);if(S&&typeof S==="object")x=S}for(var T in x){G=checkValues(x[T],T,m);if(G[0]==false)break}if(G[0]){if(a.isFunction(d.onclickSubmit))V=d.onclickSubmit(d,x)||{};if(a.isFunction(d.beforeSubmit))G=d.beforeSubmit(x,a("#"+q))}H=d.url?d.url:a(m).jqGrid("getGridParam","editurl");if(G[0])if(!H){G[0]=false;G[1]+=" "+a.jgrid.errors.nourl}if(G[0]===
false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else if(!d.processing){d.processing=true;a("#sData","#"+r+"_2").addClass("ui-state-active");R=L.oper;W=L.id;x[R]=a.trim(x[m.p.id+"_id"])=="_empty"?L.addoper:L.editoper;if(x[R]!=L.addoper)x[W]=x[m.p.id+"_id"];else if(x[W]===undefined)x[W]=x[m.p.id+"_id"];delete x[m.p.id+"_id"];x=a.extend(x,d.editData,V);a.ajax(a.extend({url:H,type:d.mtype,data:a.isFunction(d.serializeEditData)?d.serializeEditData(x):x,complete:function(J,ba){if(ba!=
"success"){G[0]=false;G[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(J):ba+" Status: '"+J.statusText+"'. Error code: "+J.status}else if(a.isFunction(d.afterSubmit))G=d.afterSubmit(J,x);if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else{a.each(m.p.colModel,function(){if(N[this.name]&&this.formatter&&this.formatter=="select")try{delete N[this.name]}catch(ka){}});x=a.extend(x,N);m.p.autoencode&&a.each(x,function(ka,aa){x[ka]=a.jgrid.htmlDecode(aa)});if(x[R]==
...
"#"+r+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+q)[0].focus()}catch(Z){}},error:function(J,ba,Z){a("#FormError>td","#"+r).html(ba+" : "+Z);a("#FormError","#"+r).show();d.processing=false;a("#"+q).data("disabled",false);a("#sData","#"+r+"_2").removeClass("ui-state-active")}},a.jgrid.ajaxOptions,d.ajaxEditOptions))}}function o(E,G){var V=false,L;for(L in E)if(E[L]!=G[L]){V=true;break}return V}var m=this;if(m.grid&&e){var n=m.p.id,q="FrmGrid_"+n,r="TblGrid_"+n,u={themodal:"editmod"+
n,modalhead:"edithd"+n,modalcontent:"editcnt"+n,scrollelm:q},v=a.isFunction(d.beforeShowForm)?d.beforeShowForm:false,w=a.isFunction(d.afterShowForm)?d.afterShowForm:false,z=a.isFunction(d.beforeInitData)?d.beforeInitData:false,D=a.isFunction(d.onInitializeForm)?d.onInitializeForm:false,C=1,y=0,H,x,N,Y,da;if(e=="new"){e="_empty";b.caption=b.addCaption}else b.caption=b.editCaption;b.recreateForm===true&&a("#"+u.themodal).html()!=null&&a("#"+u.themodal).remove();var P=true;if(b.checkOnUpdate&&b.jqModal&&
!b.modal)P=false;if(a("#"+u.themodal).html()!=null){a(".ui-jqdialog-title","#"+u.modalhead).html(b.caption);a("#FormError","#"+r).hide();if(d.topinfo){a(".topinfo","#"+r+"_2").html(d.topinfo);a(".tinfo","#"+r+"_2").show()}else a(".tinfo","#"+r+"_2").hide();if(d.bottominfo){a(".bottominfo","#"+r+"_2").html(d.bottominfo);a(".binfo","#"+r+"_2").show()}else a(".binfo","#"+r+"_2").hide();z&&z(a("#"+q));l(e,m,q);e=="_empty"||!d.viewPagerButtons?a("#pData, #nData","#"+r+"_2").hide():a("#pData, #nData","#"+
...
1);b.colModel.splice(0,1)}if(b.treeGrid)for(var f in b.treeReader){b.colNames.splice(b.colNames.length-1);b.colModel.splice(b.colModel.length-1)}switch(d.exptype){case "xmlstring":e="<"+d.root+">"+xmlJsonClass.json2xml(b,d.ident)+"</"+d.root+">";break;case "jsonstring":e="{"+xmlJsonClass.toJson(b,d.root,d.ident)+"}";if(b.postData.filters!=undefined){e=e.replace(/filters":"/,'filters":');e=e.replace(/}]}"/,"}]}")}break}}});return e},excelExport:function(d){d=a.extend({exptype:"remote",url:null,oper:"oper",
tag:"excel",exportOptions:{}},d||{});return this.each(function(){$t=this;if(this.grid)if(d.exptype=="remote"){var e=a.extend({},this.p.postData);e[d.oper]=d.tag;e=jQuery.param(e);window.location=d.url+"?"+e}})}})})(jQuery);
