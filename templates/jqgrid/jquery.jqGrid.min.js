...
function(F,K){if(O)F%2==1?a(K).addClass(O):a(K).removeClass(O);a("tbody",c.grid.bDiv).append(K);K.sortKey=null})}c.grid.bDiv.scrollTop=0},sa=function(p,s){var t={m:1,d:1,y:1970,h:0,i:0,s:0},A,B,M;s=s.split(/[\\\/:_;.\t\T\s-]/);p=p.split(/[\\\/:_;.\t\T\s-]/);var I=a.jgrid.formatter.date.monthNames;A=0;for(B=p.length;A<B;A++){if(p[A]=="M"){M=a.inArray(s[A],I);if(M!==-1&&M<12)s[A]=M+1}if(p[A]=="F"){M=a.inArray(s[A],I);if(M!==-1&&M>11)s[A]=M+1-12}t[p[A].toLowerCase()]=parseInt(s[A],10)}t.m=parseInt(t.m,
10)-1;p=t.y;if(p>=70&&p<=99)t.y=1900+t.y;else if(p>=0&&p<=69)t.y=2E3+t.y;return new Date(t.y,t.m,t.d,t.h,t.i,t.s,0)};k=function(p,s){var t="",A="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",B="",M,I,Q,O,F=function(K){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];if(a.isFunction(c.p.onPaging))if(c.p.onPaging.call(c,K)=="stop")return false;return true};
p=p.substr(1);M="pg_"+p;I=p+"_left";Q=p+"_center";O=p+"_right";a("#"+p).append("<div id='"+M+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='"+I+"' align='left'></td><td id='"+Q+"' align='center' style='white-space:pre;'></td><td id='"+O+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>0){B="<td dir='"+j+"'>";B+="<select class='ui-pg-selbox' role='listbox'>";
for(I=0;I<c.p.rowList.length;I++)B+="<option role='option' value='"+c.p.rowList[I]+"'"+(c.p.rowNum==c.p.rowList[I]?" selected":"")+">"+c.p.rowList[I]+"</option>";B+="</select></td>"}if(j=="rtl")A+=B;if(c.p.pginput===true)t="<td dir='"+j+"'>"+a.jgrid.format(c.p.pgtext||"","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1'></span>")+"</td>";if(c.p.pgbuttons===true){I=["first"+s,"prev"+s,"next"+s,"last"+s];j=="rtl"&&I.reverse();A+="<td id='"+I[0]+
