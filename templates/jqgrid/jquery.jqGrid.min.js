...
t;var A=f.rowNum,B=g.scrollTop=g.bDiv.scrollTop,M=Math.round(s.position().top)-B,I=M+s.height();t=t*A;var Q,O,F;if(M<=0&&I<p&&(f.lastpage==null||parseInt((I+B+t-1)/t,10)<f.lastpage)){O=parseInt((p-I+t-1)/t,10);if(I>=0||O<2||f.scroll===true){Q=Math.round((I+B)/t)+1;M=-1}else M=1}if(M>0){Q=parseInt(B/t,10)+1;O=parseInt((B+p)/t,10)+2-Q;F=true}if(O)if(!(f.lastpage&&Q>f.lastpage))if(g.hDiv.loading)g.timer=setTimeout(g.populateVisible,200);else{f.page=Q;if(F){g.selectionPreserver(s[0]);g.emptyRows(g.bDiv)}g.populate(O)}}}},
scrollGrid:function(){if(f.scroll){var p=g.bDiv.scrollTop;if(p!=g.scrollTop){g.scrollTop=p;g.timer&&clearTimeout(g.timer);g.timer=setTimeout(g.populateVisible,200)}}g.hDiv.scrollLeft=g.bDiv.scrollLeft;if(f.footerrow)g.sDiv.scrollLeft=g.bDiv.scrollLeft},selectionPreserver:function(p){var s=p.p,t=s.selrow,A=s.selarrrow?a.makeArray(s.selarrrow):null,B=p.grid.bDiv.scrollLeft,M=s.gridComplete;s.gridComplete=function(){s.selrow=null;s.selarrrow=[];if(s.multiselect&&A&&A.length>0)for(var I=0;I<A.length;I++)A[I]!=
t&&a(p).jqGrid("setSelection",A[I],false);t&&a(p).jqGrid("setSelection",t,false);p.grid.bDiv.scrollLeft=B;if(s.gridComplete=M)M()}}};this.p=f;var h,j,c;if(this.p.colNames.length===0)for(h=0;h<this.p.colModel.length;h++)this.p.colNames[h]=this.p.colModel[h].label||this.p.colModel[h].name;if(this.p.colNames.length!==this.p.colModel.length)alert(a.jgrid.errors.model);else{var l=a("<div class='ui-jqgrid-view'></div>"),k,o=a.browser.msie?true:false,m=a.browser.safari?true:false;c=this;c.p.direction=a.trim(c.p.direction.toLowerCase());
...
this.id+"'></div>").append(k).insertBefore(l);a("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(l);a(this).attr({cellSpacing:"0",cellPadding:"0",border:"0",role:"grid","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id});var q=function(p,s){p=parseInt(p,10);return isNaN(p)?s?s:0:p},r=function(p,s){var t=c.p.colModel[p].align,A='style="',B=c.p.colModel[p].classes;if(t)A+="text-align:"+t+";";if(c.p.colModel[p].hidden===
true)A+="display:none;";if(s===0)A+="width: "+g.headers[p].width+"px;";return A+'"'+(B!==undefined?' class="'+B+'"':"")},u=function(p,s,t,A,B){p=v(p,s,t,B,"add");return'<td role="gridcell" '+r(t,A)+' title="'+a.jgrid.stripHtml(p)+'">'+p+"</td>"},v=function(p,s,t,A,B){t=c.p.colModel[t];if(typeof t.formatter!=="undefined"){p={rowId:p,colModel:t,gid:c.p.id};s=a.isFunction(t.formatter)?t.formatter.call(c,s,p,A,B):a.fmatter?a.fn.fmatter(t.formatter,s,p,A,B):w(s)}else s=w(s);return s},w=function(p){return p===
undefined||p===null||p===""?"&#160;":c.p.autoencode?a.jgrid.htmlEncode(p):p+""},z=function(p,s,t){p='<input type="checkbox" id="jqg_'+p+'" class="cbox" name="jqg_'+p+'"/>';return"<td role='gridcell' "+r(s,t)+">"+p+"</td>"},D=function(p,s,t,A){t=(parseInt(t,10)-1)*parseInt(A,10)+1+s;return'<td role="gridcell" class="ui-state-default jqgrid-rownum" '+r(p,s)+">"+t+"</td>"},C=function(p){var s,t=[],A=0,B;for(B=0;B<c.p.colModel.length;B++){s=c.p.colModel[B];if(s.name!=="cb"&&s.name!=="subgrid"&&s.name!==
"rn"){t[A]=p=="xml"?s.xmlmap||s.name:s.jsonmap||s.name;A++}}return t},y=function(p){var s=c.p.remapColumns;if(!s||!s.length)s=a.map(c.p.colModel,function(t,A){return A});if(p)s=a.map(s,function(t){return t<p?null:t-p});return s},H=function(p,s){var t=a("tbody:first",p);if(!c.p.gridview||c.p.jqgdnd)a("*",t).children().unbind();if(o)a.jgrid.empty.apply(t[0]);else t[0].innerHTML="";if(s&&c.p.scroll){a(">div:first",p).css({height:"auto"}).children("div:first").css({height:0,display:"none"});p.scrollTop=
0}},x=function(p,s){var t,A,B;if(typeof s==="function")return s(p);t=p[s];if(t===undefined){if(typeof s==="string")B=s.split(".");try{if(B.length)for(t=p;t&&B.length;){A=B.shift();t=t[A]}}catch(M){}}return t},N=function(p,s,t,A,B){var M=new Date;c.p.reccount=0;if(a.isXMLDoc(p)){if(c.p.treeANode===-1&&!c.p.scroll){H(s);t=0}else t=t>0?t:0;var I,Q=0,O,F,K=0,X=0,ca=0,ha,pa,na=[],ra,ja={},Da=c.rows.length,ia,Aa,fa=[],ua=0,Ea=c.p.altRows===true?" "+c.p.altclass:"";c.p.xmlReader.repeatitems||(na=C("xml"));
ha=c.p.keyIndex===false?c.p.xmlReader.id:c.p.keyIndex;if(na.length>0&&!isNaN(ha)){if(c.p.remapColumns&&c.p.remapColumns.length)ha=a.inArray(ha,c.p.remapColumns);ha=na[ha]}pa=(ha+"").indexOf("[")===-1?na.length?function(Ba,ya){return a(ha,Ba).text()||ya}:function(Ba,ya){return a(c.p.xmlReader.cell,Ba).eq(ha).text()||ya}:function(Ba,ya){return Ba.getAttribute(ha.replace(/[\[\]]/g,""))||ya};a(c.p.xmlReader.page,p).each(function(){c.p.page=this.textContent||this.text||1});a(c.p.xmlReader.total,p).each(function(){c.p.lastpage=
this.textContent||this.text;if(c.p.lastpage===undefined)c.p.lastpage=1});a(c.p.xmlReader.records,p).each(function(){c.p.records=this.textContent||this.text||0});a(c.p.xmlReader.userdata,p).each(function(){c.p.userData[this.getAttribute("name")]=this.textContent||this.text});p=a(c.p.xmlReader.root+" "+c.p.xmlReader.row,p);var Fa=p.length,qa=0;if(p&&Fa){var Ha=parseInt(c.p.rowNum,10),Ja=c.p.scroll?(parseInt(c.p.page,10)-1)*Ha+1:1;if(B)Ha*=B+1;for(B=a.isFunction(c.p.afterInsertRow);qa<Fa;){ia=p[qa];
Aa=pa(ia,Ja+qa);I=t==0?0:t+1;I=(I+qa)%2==1?Ea:"";fa[ua++]='<tr id="'+Aa+'" role="row" class ="ui-widget-content jqgrow ui-row-'+c.p.direction+""+I+'">';if(c.p.rownumbers===true){fa[ua++]=D(0,qa,c.p.page,c.p.rowNum);ca=1}if(c.p.multiselect===true){fa[ua++]=z(Aa,ca,qa);K=1}if(c.p.subGrid===true){fa[ua++]=a(c).jqGrid("addSubGridCell",K+ca,qa+t);X=1}if(c.p.xmlReader.repeatitems){ra||(ra=y(K+X+ca));var Ka=a(c.p.xmlReader.cell,ia);a.each(ra,function(Ba){var ya=Ka[this];if(!ya)return false;O=ya.textContent||
ya.text;ja[c.p.colModel[Ba+K+X+ca].name]=O;fa[ua++]=u(Aa,O,Ba+K+X+ca,qa+t,ia)})}else for(I=0;I<na.length;I++){O=a(na[I],ia).text();ja[c.p.colModel[I+K+X+ca].name]=O;fa[ua++]=u(Aa,O,I+K+X+ca,qa+t,ia)}fa[ua++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){I=c.p.treeANode>=-1?c.p.treeANode:0;F=a(fa.join(""))[0];try{a(c).jqGrid("setTreeNode",ja,F)}catch(La){}Da===0?a("tbody:first",s).append(F):a(c.rows[qa+I+t]).after(F)}else a("tbody:first",s).append(fa.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",
c.rows[c.rows.length-1],K+ca)}catch(Ma){}B&&c.p.afterInsertRow.call(c,Aa,ja,ia);fa=[];ua=0}ja={};Q++;qa++;if(Q==Ha)break}}c.p.gridview===true&&a("tbody:first",s).append(fa.join(""));c.p.totaltime=new Date-M;if(Q>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=Fa}fa=null;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=Q;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},Y=function(p,s,t,A,B){var M=new Date;c.p.reccount=
0;if(p){if(c.p.treeANode===-1&&!c.p.scroll){H(s);t=0}else t=t>0?t:0;var I=0,Q,O,F,K=[],X,ca=0,ha=0,pa=0,na,ra,ja,Da={},ia,Aa=c.rows.length,fa;F=[];ia=0;var ua=c.p.altRows===true?" "+c.p.altclass:"";c.p.page=x(p,c.p.jsonReader.page)||1;ja=x(p,c.p.jsonReader.total);c.p.lastpage=ja===undefined?1:ja;c.p.records=x(p,c.p.jsonReader.records)||0;c.p.userData=x(p,c.p.jsonReader.userdata)||{};c.p.jsonReader.repeatitems||(X=K=C("json"));ja=c.p.keyIndex===false?c.p.jsonReader.id:c.p.keyIndex;if(K.length>0&&!isNaN(ja)){if(c.p.remapColumns&&
c.p.remapColumns.length)ja=a.inArray(ja,c.p.remapColumns);ja=K[ja]}if(ra=x(p,c.p.jsonReader.root)){na=ra.length;p=0;var Ea=parseInt(c.p.rowNum,10),Fa=c.p.scroll?(parseInt(c.p.page,10)-1)*Ea+1:1;if(B)Ea*=B+1;for(var qa=a.isFunction(c.p.afterInsertRow);p<na;){B=ra[p];fa=B[ja];if(fa===undefined){fa=Fa+p;if(K.length===0)if(c.p.jsonReader.cell)fa=B[c.p.jsonReader.cell][ja]||fa}Q=t==0?0:t+1;Q=(Q+p)%2==1?ua:"";F[ia++]='<tr id="'+fa+'" role="row" class= "ui-widget-content jqgrow ui-row-'+c.p.direction+""+
Q+'">';if(c.p.rownumbers===true){F[ia++]=D(0,p,c.p.page,c.p.rowNum);pa=1}if(c.p.multiselect){F[ia++]=z(fa,pa,p);ca=1}if(c.p.subGrid){F[ia++]=a(c).jqGrid("addSubGridCell",ca+pa,p+t);ha=1}if(c.p.jsonReader.repeatitems){if(c.p.jsonReader.cell)B=x(B,c.p.jsonReader.cell);X||(X=y(ca+ha+pa))}for(O=0;O<X.length;O++){Q=x(B,X[O]);F[ia++]=u(fa,Q,O+ca+ha+pa,p+t,B);Da[c.p.colModel[O+ca+ha+pa].name]=Q}F[ia++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){ia=c.p.treeANode>=-1?c.p.treeANode:0;F=a(F.join(""))[0];
try{a(c).jqGrid("setTreeNode",Da,F)}catch(Ha){}Aa===0?a("tbody:first",s).append(F):a(c.rows[p+ia+t]).after(F)}else a("tbody:first",s).append(F.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",c.rows[c.rows.length-1],ca+pa)}catch(Ja){}qa&&c.p.afterInsertRow.call(c,fa,Da,B);F=[];ia=0}Da={};I++;p++;if(I==Ea)break}c.p.gridview===true&&a("tbody:first",s).append(F.join(""));c.p.totaltime=new Date-M;if(I>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=na}}if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=
0;c.p.reccount=I;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},da=function(p,s){var t,A,B,M,I,Q,O,F="";B=(parseInt(c.p.page,10)-1)*parseInt(c.p.rowNum,10);I=B+c.p.reccount;if(c.p.scroll){t=a("tbody:first > tr",c.grid.bDiv);B=I-t.length;if(A=t.outerHeight()){t=B*A;A=parseInt(c.p.records,10)*A;a(">div:first",c.grid.bDiv).css({height:A}).children("div:first").css({height:t,display:t?"":"none"})}}F=c.p.pager?c.p.pager:"";F+=c.p.toppager?F?
","+c.p.toppager:c.p.toppager:"";if(F){O=a.jgrid.formatter.integer||{};if(c.p.loadonce){t=A=1;c.p.lastpage=c.page=1;a(".selbox",F).attr("disabled",true)}else{t=q(c.p.page,1);A=q(c.p.lastpage,1);a(".selbox",F).attr("disabled",false)}if(c.p.pginput===true){a(".ui-pg-input",F).val(c.p.page);a("#sp_1",F).html(a.fmatter?a.fmatter.util.NumberFormat(c.p.lastpage,O):c.p.lastpage)}if(c.p.viewrecords)if(c.p.reccount===0)a(".ui-paging-info",F).html(c.p.emptyrecords);else{M=B+1;Q=c.p.records;if(a.fmatter){M=
a.fmatter.util.NumberFormat(M,O);I=a.fmatter.util.NumberFormat(I,O);Q=a.fmatter.util.NumberFormat(Q,O)}a(".ui-paging-info",F).html(a.jgrid.format(c.p.recordtext,M,I,Q))}if(c.p.pgbuttons===true){if(t<=0)t=A=1;if(t==1){a("#first, #prev",c.p.pager).addClass("ui-state-disabled").removeClass("ui-state-hover");c.p.toppager&&a("#first_t, #prev_t",c.p.toppager).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#first, #prev",c.p.pager).removeClass("ui-state-disabled");c.p.toppager&&a("#first_t, #prev_t",
c.p.toppager).removeClass("ui-state-disabled")}if(t==A){a("#next, #last",c.p.pager).addClass("ui-state-disabled").removeClass("ui-state-hover");c.p.toppager&&a("#next_t, #last_t",c.p.toppager).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#next, #last",c.p.pager).removeClass("ui-state-disabled");c.p.toppager&&a("#next_t, #last_t",c.p.toppager).removeClass("ui-state-disabled")}}}p===true&&c.p.rownumbers===true&&a("td.jqgrid-rownum",c.rows).each(function(K){a(this).html(B+1+K)});
s&&c.p.jqgdnd&&a(c).jqGrid("gridDnD","updateDnD");a.isFunction(c.p.gridComplete)&&c.p.gridComplete.call(c)},P=function(p){if(!c.grid.hDiv.loading){var s=c.p.scroll&&p==false,t={},A,B=c.p.prmNames;if(B.search!==null)t[B.search]=c.p.search;if(B.nd!=null)t[B.nd]=(new Date).getTime();if(B.rows!==null)t[B.rows]=c.p.rowNum;if(B.page!==null)t[B.page]=c.p.page;if(B.sort!==null)t[B.sort]=c.p.sortname;if(B.order!==null)t[B.order]=c.p.sortorder;var M=c.p.loadComplete,I=a.isFunction(M);I||(M=null);var Q=0;p=
p||1;if(p>1)if(B.npage!=null){t[B.npage]=p;Q=p-1;p=1}else M=function(F){I&&c.p.loadComplete.call(c,F);c.grid.hDiv.loading=false;c.p.page++;P(p-1)};else B.npage!=null&&delete c.p.postData[B.npage];a.extend(c.p.postData,t);var O=!c.p.scroll?0:c.rows.length-1;if(a.isFunction(c.p.datatype))c.p.datatype.call(c,c.p.postData,"load_"+c.p.id);else{a.isFunction(c.p.beforeRequest)&&c.p.beforeRequest.call(c);A=c.p.datatype.toLowerCase();switch(A){case "json":case "jsonp":case "xml":case "script":a.ajax(a.extend({url:c.p.url,
type:c.p.mtype,dataType:A,data:a.isFunction(c.p.serializeGridData)?c.p.serializeGridData.call(c,c.p.postData):c.p.postData,complete:function(F,K){if(K=="success"||F.statusText=="OK"&&F.status=="200"){A==="xml"?N(F.responseXML,c.grid.bDiv,O,p>1,Q):Y(a.jgrid.parse(F.responseText),c.grid.bDiv,O,p>1,Q);M&&M.call(c,F);s&&c.grid.populateVisible()}U()},error:function(F,K,X){a.isFunction(c.p.loadError)&&c.p.loadError.call(c,F,K,X);U()},beforeSend:function(F){ea();a.isFunction(c.p.loadBeforeSend)&&c.p.loadBeforeSend.call(c,
...
s=[],t=0,A,B,M,I=c.p.sortorder=="asc"?1:-1,Q=false;a.each(c.p.colModel,function(F){if(this.index==c.p.sortname||this.name==c.p.sortname){if(c.p.lastsort==F)Q=true;t=F;A=this.sorttype;return false}});M=A=="float"||A=="number"||A=="currency"?function(F){F=parseFloat(F.replace(p,""));return isNaN(F)?0:F}:A=="int"||A=="integer"?function(F){return q(F.replace(p,""),0)}:A=="date"||A=="datetime"?function(F){return sa(c.p.colModel[t].datefmt||"Y-m-d",F).getTime()}:function(F){return a.trim(F.toUpperCase())};
a.each(c.rows,function(F,K){try{B=a.unformat(a(K).children("td").eq(t),{rowId:K.id,colModel:c.p.colModel[t]},t,true)}catch(X){B=a(K).children("td").eq(t).text()}K.sortKey=M(B);s[F]=this});if(c.p.treeGrid)a(c).jqGrid("SortTree",I);else{Q?s.reverse():s.sort(function(F,K){if(F.sortKey<K.sortKey)return-I;if(F.sortKey>K.sortKey)return I;return 0});if(s[0]){a("td",s[0]).each(function(F){a(this).css("width",g.headers[F].width+"px")});c.grid.cols=s[0].cells}var O="";if(c.p.altRows)O=c.p.altclass;a.each(s,
function(F,K){if(O)F%2==1?a(K).addClass(O):a(K).removeClass(O);a("tbody",c.grid.bDiv).append(K);K.sortKey=null})}c.grid.bDiv.scrollTop=0},sa=function(p,s){var t={m:1,d:1,y:1970,h:0,i:0,s:0},A,B,M;if(s=a.trim(s)){s=s.split(/[\\\/:_;.\t\T\s-]/);p=p.split(/[\\\/:_;.\t\T\s-]/);var I=a.jgrid.formatter.date.monthNames;A=0;for(B=p.length;A<B;A++){if(p[A]=="M"){M=a.inArray(s[A],I);if(M!==-1&&M<12)s[A]=M+1}if(p[A]=="F"){M=a.inArray(s[A],I);if(M!==-1&&M>11)s[A]=M+1-12}t[p[A].toLowerCase()]=parseInt(s[A],10)}t.m=
parseInt(t.m,10)-1;p=t.y;if(p>=70&&p<=99)t.y=1900+t.y;else if(p>=0&&p<=69)t.y=2E3+t.y}return new Date(t.y,t.m,t.d,t.h,t.i,t.s,0)};k=function(p,s){var t="",A="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",B="",M,I,Q,O,F=function(K){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];if(a.isFunction(c.p.onPaging))if(c.p.onPaging.call(c,K)=="stop")return false;
return true};p=p.substr(1);M="pg_"+p;I=p+"_left";Q=p+"_center";O=p+"_right";a("#"+p).append("<div id='"+M+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='"+I+"' align='left'></td><td id='"+Q+"' align='center' style='white-space:pre;'></td><td id='"+O+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>0){B="<td dir='"+j+"'>";B+=
...
A+="<td id='"+I[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-end'></span></td>"}else if(t!="")A+=t;if(j=="ltr")A+=B;A+="</tr></tbody></table>";c.p.viewrecords===true&&a("td#"+p+"_"+c.p.recordpos,"#"+M).append("<div dir='"+j+"' style='text-align:"+c.p.recordpos+"' class='ui-paging-info'></div>");a("td#"+p+"_"+c.p.pagerpos,"#"+M).append(A);B=a(".ui-jqgrid").css("font-size")||"11px";a("body").append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+
B+";visibility:hidden;' ></div>");A=a(A).clone().appendTo("#testpg").width();a("#testpg").remove();if(A>0){if(t!="")A+=50;a("td#"+p+"_"+c.p.pagerpos,"#"+M).width(A)}c.p._nvtd=[];c.p._nvtd[0]=A?Math.floor((c.p.width-A)/2):Math.floor(c.p.width/3);c.p._nvtd[1]=0;A=null;a(".ui-pg-selbox","#"+M).bind("change",function(){c.p.page=Math.round(c.p.rowNum*(c.p.page-1)/this.value-0.5)+1;c.p.rowNum=this.value;if(s)a(".ui-pg-selbox",c.p.pager).val(this.value);else c.p.toppager&&a(".ui-pg-selbox",c.p.toppager).val(this.value);
if(!F("records"))return false;P();return false});if(c.p.pgbuttons===true){a(".ui-pg-button","#"+M).hover(function(){if(a(this).hasClass("ui-state-disabled"))this.style.cursor="default";else{a(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!a(this).hasClass("ui-state-disabled")){a(this).removeClass("ui-state-hover");this.style.cursor="default"}});a("#first"+s+", #prev"+s+", #next"+s+", #last"+s,"#"+p).click(function(){var K=q(c.p.page,1),X=q(c.p.lastpage,1),ca=false,ha=
true,pa=true,na=true,ra=true;if(X===0||X===1)ra=na=pa=ha=false;else if(X>1&&K>=1)if(K===1)pa=ha=false;else{if(!(K>1&&K<X))if(K===X)ra=na=false}else if(X>1&&K===0){ra=na=false;K=X-1}if(this.id==="first"+s&&ha){c.p.page=1;ca=true}if(this.id==="prev"+s&&pa){c.p.page=K-1;ca=true}if(this.id==="next"+s&&na){c.p.page=K+1;ca=true}if(this.id==="last"+s&&ra){c.p.page=X;ca=true}if(ca){if(!F(this.id))return false;P()}return false})}c.p.pginput===true&&a("input.ui-pg-input","#"+M).keypress(function(K){if((K.charCode?
K.charCode:K.keyCode?K.keyCode:0)==13){c.p.page=a(this).val()>0?a(this).val():c.p.page;if(!F("user"))return false;P();return false}return this})};var za=function(p,s,t,A){if(c.p.colModel[s].sortable)if(!(c.p.savedRow.length>0)){if(!t){if(c.p.lastsort==s)if(c.p.sortorder=="asc")c.p.sortorder="desc";else{if(c.p.sortorder=="desc")c.p.sortorder="asc"}else c.p.sortorder="asc";c.p.page=1}if(A)if(c.p.lastsort==s&&c.p.sortorder==A)return;else c.p.sortorder=A;t=a("thead:first",c.grid.hDiv).get(0);a("tr th:eq("+
...
repeatitems:true,cell:"cell"}},c.p.jsonReader);if(c.p.scroll){c.p.pgbuttons=false;c.p.pginput=false;c.p.rowList=[]}var G="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",V,L,W,R,S,T,J,ba;L=ba="";if(c.p.shrinkToFit===true&&c.p.forceFit===true)for(h=c.p.colModel.length-1;h>=0;h--)if(!c.p.colModel[h].hidden){c.p.colModel[h].resizable=false;break}if(c.p.viewsortcols[1]=="horizontal"){ba=" ui-i-asc";L=" ui-i-desc"}V=o?"class='ui-th-div-ie'":"";ba="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+
ba+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+j+"'></span>";ba+="<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+L+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+j+"'></span></span>";for(h=0;h<this.p.colNames.length;h++){G+="<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+j+"'>";L=c.p.colModel[h].index||c.p.colModel[h].name;G+="<div id='jqgh_"+c.p.colModel[h].name+"' "+V+">"+c.p.colNames[h];if(L==c.p.sortname)c.p.lastsort=h;G+=ba+"</div></th>"}G+=
...
c.p.selarrrow[p]=c.p.selrow=this.id}});aa=true;ka=[]}else{a("[id^=jqg_]",c.rows).attr("checked",false);a(c.rows).each(function(p){if(!a(this).hasClass("subgrid")){a(this).removeClass("ui-state-highlight").attr("aria-selected","false");ka[p]=this.id}});c.p.selarrrow=[];c.p.selrow=null;aa=false}if(Z)c.p.onSelectAll.call(c,aa?c.p.selarrrow:ka,aa)})}a.each(c.p.colModel,function(){if(!this.width)this.width=150;this.width=parseInt(this.width,10)});if(c.p.autowidth===true){G=a(n).innerWidth();c.p.width=
G>0?G:"nw"}(function(){var p=0,s=c.p.cellLayout,t=0,A,B=c.p.scrollOffset,M,I=false,Q,O=0,F=0,K=0,X;if(m)s=0;a.each(c.p.colModel,function(){if(typeof this.hidden==="undefined")this.hidden=false;if(this.hidden===false){p+=q(this.width,0);if(this.fixed){O+=this.width;F+=this.width+s}else t++;K++}});if(isNaN(c.p.width))c.p.width=g.width=p;else g.width=c.p.width;c.p.tblwidth=p;if(c.p.shrinkToFit===false&&c.p.forceFit===true)c.p.forceFit=false;if(c.p.shrinkToFit===true&&t>0){Q=g.width-s*t-F;if(!isNaN(c.p.height)){Q-=
B;I=true}p=0;a.each(c.p.colModel,function(ca){if(this.hidden===false&&!this.fixed){this.width=M=Math.floor(Q/(c.p.tblwidth-O)*this.width);p+=M;A=ca}});X=0;if(I){if(g.width-F-(p+s*t)!==B)X=g.width-F-(p+s*t)-B}else if(!I&&Math.abs(g.width-F-(p+s*t))!==1)X=g.width-F-(p+s*t);c.p.colModel[A].width+=X;c.p.tblwidth=p+X+O+K*s}})();a(n).css("width",g.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+c.p.id+"'>&#160;</div>");a(l).css("width",g.width+"px");G=a("thead:first",c).get(0);var wa="<table role='grid' style='width:"+
c.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+j+"'>";l=a("tr:first",G);c.p.disableClick=false;a("th",l).each(function(p){a("div",this);W=c.p.colModel[p].width;if(typeof c.p.colModel[p].resizable==="undefined")c.p.colModel[p].resizable=true;if(c.p.colModel[p].resizable){R=document.createElement("span");a(R).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+j);!a.browser.opera&&a(R).css("cursor",
"col-resize");a(this).addClass(c.p.resizeclass)}else R="";a(this).css("width",W+"px").prepend(R);c.p.colModel[p].hidden&&a(this).css("display","none");g.headers[p]={width:W,el:this};S=c.p.colModel[p].sortable;if(typeof S!=="boolean")S=c.p.colModel[p].sortable=true;var s=c.p.colModel[p].name;s=="cb"||s=="subgrid"||s=="rn"||c.p.viewsortcols[2]&&a("div",this).addClass("ui-jqgrid-sortable");if(S)if(c.p.viewsortcols[0]){a("div span.s-ico",this).show();p==c.p.lastsort&&a("div span.ui-icon-"+c.p.sortorder,
this).removeClass("ui-state-disabled")}else if(p==c.p.lastsort){a("div span.s-ico",this).show();a("div span.ui-icon-"+c.p.sortorder,this).removeClass("ui-state-disabled")}wa+="<td role='gridcell' "+r(p,0)+">&#160;</td>"}).mousedown(function(p){if(a(p.target).closest("th>span.ui-jqgrid-resize").length==1){var s=a.jgrid.getCellIndex(this);if(c.p.forceFit===true)c.p.nv=Ca(s);g.dragStart(s,p,va(s));return false}}).click(function(p){if(c.p.disableClick)return c.p.disableClick=false;var s="th>div.ui-jqgrid-sortable",
t,A;c.p.viewsortcols[2]||(s="th>div>span>span.ui-grid-ico-sort");p=a(p.target).closest(s);if(p.length==1){s=a.jgrid.getCellIndex(this);if(!c.p.viewsortcols[2]){t=true;A=p.attr("sort")}za(a("div",this)[0].id,s,t,A);return false}});if(c.p.sortable&&a.fn.sortable)try{a(c).jqGrid("sortableColumns",l)}catch(ta){}wa+="</tr></tbody></table>";this.appendChild(document.createElement("tbody"));a(this).addClass("ui-jqgrid-btable");l=a("<table class='ui-jqgrid-htable' style='width:"+c.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+
this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(G);var la=c.p.caption&&c.p.hiddengrid===true?true:false;G=a("<div class='ui-jqgrid-hbox"+(j=="rtl"?"-rtl":"")+"'></div>");g.hDiv=document.createElement("div");a(g.hDiv).css({width:g.width+"px"}).addClass("ui-state-default ui-jqgrid-hdiv").append(G);a(G).append(l);la&&a(g.hDiv).hide();if(c.p.pager){if(typeof c.p.pager=="string"){if(c.p.pager.substr(0,1)!="#")c.p.pager="#"+c.p.pager}else c.p.pager="#"+a(c.p.pager).attr("id");a(c.p.pager).css({width:g.width+
"px"}).appendTo(n).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom");la&&a(c.p.pager).hide();k(c.p.pager,"")}c.p.cellEdit===false&&c.p.hoverrows===true&&a(c).bind("mouseover",function(p){J=a(p.target).closest("tr.jqgrow");a(J).attr("class")!=="subgrid"&&a(J).addClass("ui-state-hover");return false}).bind("mouseout",function(p){J=a(p.target).closest("tr.jqgrow");a(J).removeClass("ui-state-hover");return false});var ma,oa;a(c).before(g.hDiv).click(function(p){T=p.target;var s=a(T).hasClass("cbox");
J=a(T,c.rows).closest("tr.jqgrow");if(a(J).length===0)return this;var t=true;if(a.isFunction(c.p.beforeSelectRow))t=c.p.beforeSelectRow.call(c,J[0].id,p);if(T.tagName=="A"||(T.tagName=="INPUT"||T.tagName=="TEXTAREA"||T.tagName=="OPTION"||T.tagName=="SELECT")&&!s)return true;if(t===true){if(c.p.cellEdit===true)if(c.p.multiselect&&s)a(c).jqGrid("setSelection",J[0].id,true);else{ma=J[0].rowIndex;oa=a.jgrid.getCellIndex(T);try{a(c).jqGrid("editCell",ma,oa,true)}catch(A){}}else if(c.p.multikey)if(p[c.p.multikey])a(c).jqGrid("setSelection",
J[0].id,true);else{if(c.p.multiselect&&s){s=a("[id^=jqg_]",J).attr("checked");a("[id^=jqg_]",J).attr("checked",!s)}}else{if(c.p.multiselect&&c.p.multiboxonly)if(!s){a(c.p.selarrrow).each(function(B,M){B=c.rows.namedItem(M);a(B).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(M),B).attr("checked",false)});c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}a(c).jqGrid("setSelection",J[0].id,true)}if(a.isFunction(c.p.onCellSelect)){ma=J[0].id;oa=a.jgrid.getCellIndex(T);
c.p.onCellSelect.call(c,ma,oa,a(T).html(),p)}}p.stopPropagation()}).bind("reloadGrid",function(p,s){if(c.p.treeGrid===true)c.p.datatype=c.p.treedatatype;s&&s.current&&c.grid.selectionPreserver(c);if(c.p.datatype=="local")a(c).jqGrid("resetSelection");else if(!c.p.treeGrid){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];c.p.scroll&&H(c.grid.bDiv)}if(s&&s.page){p=s.page;if(p>c.p.lastpage)p=c.p.lastpage;if(p<1)p=1;
c.p.page=p;c.grid.bDiv.scrollTop=c.grid.prevRowHeight?(p-1)*c.grid.prevRowHeight*c.p.rowNum:0}if(c.grid.prevRowHeight&&c.p.scroll){delete c.p.lastpage;c.grid.populateVisible()}else c.grid.populate();return false});a.isFunction(this.p.ondblClickRow)&&a(this).dblclick(function(p){T=p.target;J=a(T,c.rows).closest("tr.jqgrow");if(a(J).length===0)return false;ma=J[0].rowIndex;oa=a.jgrid.getCellIndex(T);c.p.ondblClickRow.call(c,a(J).attr("id"),ma,oa,p);return false});a.isFunction(this.p.onRightClickRow)&&
a(this).bind("contextmenu",function(p){T=p.target;J=a(T,c.rows).closest("tr.jqgrow");if(a(J).length===0)return false;c.p.multiselect||a(c).jqGrid("setSelection",J[0].id,true);ma=J[0].rowIndex;oa=a.jgrid.getCellIndex(T);c.p.onRightClickRow.call(c,a(J).attr("id"),ma,oa,p);return false});g.bDiv=document.createElement("div");a(g.bDiv).append(a('<div style="position:relative;'+(o&&a.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:c.p.height+
(isNaN(c.p.height)?"":"px"),width:g.width+"px"}).scroll(g.scrollGrid);a("table:first",g.bDiv).css({width:c.p.tblwidth+"px"});if(o){a("tbody",this).size()==2&&a("tbody:first",this).remove();c.p.multikey&&a(g.bDiv).bind("selectstart",function(){return false})}else c.p.multikey&&a(g.bDiv).bind("mousedown",function(){return false});la&&a(g.bDiv).hide();g.cDiv=document.createElement("div");var Ia=c.p.hidegrid===true?a("<a role='link' href='javascript:void(0)'/>").addClass("ui-jqgrid-titlebar-close HeaderButton").hover(function(){Ia.addClass("ui-state-hover")},
...
c.p.toppager+"'></div>")[0];c.p.toppager="#"+c.p.toppager;a(g.topDiv).insertBefore(g.hDiv).addClass("ui-state-default ui-jqgrid-toppager").width(g.width);k(c.p.toppager,"_t")}if(c.p.footerrow){g.sDiv=a("<div class='ui-jqgrid-sdiv'></div>")[0];G=a("<div class='ui-jqgrid-hbox"+(j=="rtl"?"-rtl":"")+"'></div>");a(g.sDiv).append(G).insertAfter(g.hDiv).width(g.width);a(G).append(wa);g.footers=a(".ui-jqgrid-ftable",g.sDiv)[0].rows[0].cells;if(c.p.rownumbers)g.footers[0].className="ui-state-default jqgrid-rownum";
la&&a(g.sDiv).hide()}if(c.p.caption){var xa=c.p.datatype;if(c.p.hidegrid===true){a(".ui-jqgrid-titlebar-close",g.cDiv).click(function(p){var s=a.isFunction(c.p.onHeaderClick);if(c.p.gridstate=="visible"){a(".ui-jqgrid-bdiv, .ui-jqgrid-hdiv","#gview_"+c.p.id).slideUp("fast");c.p.pager&&a(c.p.pager).slideUp("fast");c.p.toppager&&a(c.p.toppager).slideUp("fast");if(c.p.toolbar[0]===true){c.p.toolbar[1]=="both"&&a(g.ubDiv).slideUp("fast");a(g.uDiv).slideUp("fast")}c.p.footerrow&&a(".ui-jqgrid-sdiv","#gbox_"+
c.p.id).slideUp("fast");a("span",this).removeClass("ui-icon-circle-triangle-n").addClass("ui-icon-circle-triangle-s");c.p.gridstate="hidden";if(s)la||c.p.onHeaderClick.call(c,c.p.gridstate,p)}else if(c.p.gridstate=="hidden"){a(".ui-jqgrid-hdiv, .ui-jqgrid-bdiv","#gview_"+c.p.id).slideDown("fast");c.p.pager&&a(c.p.pager).slideDown("fast");c.p.toppager&&a(c.p.toppager).slideDown("fast");if(c.p.toolbar[0]===true){c.p.toolbar[1]=="both"&&a(g.ubDiv).slideDown("fast");a(g.uDiv).slideDown("fast")}c.p.footerrow&&
a(".ui-jqgrid-sdiv","#gbox_"+c.p.id).slideDown("fast");a("span",this).removeClass("ui-icon-circle-triangle-s").addClass("ui-icon-circle-triangle-n");if(la){c.p.datatype=xa;P();la=false}c.p.gridstate="visible";s&&c.p.onHeaderClick.call(c,c.p.gridstate,p)}return false});if(la){c.p.datatype="local";a(".ui-jqgrid-titlebar-close",g.cDiv).trigger("click")}}}else a(g.cDiv).hide();a(g.hDiv).after(g.bDiv).mousemove(function(p){if(g.resizing){g.dragMove(p);return false}});a(".ui-jqgrid-labels",g.hDiv).bind("selectstart",
function(){return false});a(document).mouseup(function(){if(g.resizing){g.dragEnd();return false}return true});this.updateColumns=function(){var p=this.rows[0],s=this;if(p){a("td",p).each(function(t){a(this).css("width",s.grid.headers[t].width+"px")});this.grid.cols=p.cells}return this};c.formatCol=r;c.sortData=za;c.updatepager=da;c.formatter=function(p,s,t,A,B){return v(p,s,t,A,B)};a.extend(g,{populate:P,emptyRows:H});this.grid=g;c.addXmlData=function(p){N(p,c.grid.bDiv)};c.addJSONData=function(p){Y(p,
c.grid.bDiv)};P();c.p.hiddengrid=false;a(window).unload(function(){a(this).empty();this.p=this.grid=null})}}})};a.jgrid.extend({getGridParam:function(d){var e=this[0];if(e.grid)return d?typeof e.p[d]!="undefined"?e.p[d]:null:e.p},setGridParam:function(d){return this.each(function(){this.grid&&typeof d==="object"&&a.extend(true,this.p,d)})},getDataIDs:function(){var d=[],e=0,b;this.each(function(){if((b=this.rows.length)&&b>0)for(;e<b;){d[e]=this.rows[e].id;e++}});return d},setSelection:function(d,
e){return this.each(function(){function b(c,l){var k=a(f.grid.bDiv)[0].clientHeight,o=a(f.grid.bDiv)[0].scrollTop,m=f.rows[c].offsetTop+f.rows[c].clientHeight;c=f.rows[c].offsetTop;if(l=="d")if(m>=k)a(f.grid.bDiv)[0].scrollTop=o+m-c;if(l=="u")if(c<o)a(f.grid.bDiv)[0].scrollTop=o-m+c}var f=this,g,h,j;if(d!==undefined){e=e===false?false:true;h=f.rows.namedItem(d+"");if(h!==null){if(f.p.selrow&&f.p.scrollrows===true){g=f.rows.namedItem(f.p.selrow).rowIndex;j=f.rows.namedItem(d).rowIndex;if(j>=0)j>g?
b(j,"d"):b(j,"u")}if(f.p.multiselect){f.p.selrow=h.id;j=a.inArray(f.p.selrow,f.p.selarrrow);if(j===-1){h.className!=="ui-subgrid"&&a(h).addClass("ui-state-highlight").attr("aria-selected","true");g=true;a("#jqg_"+a.jgrid.jqID(f.p.selrow),f.rows[h.rowIndex]).attr("checked",g);f.p.selarrrow.push(f.p.selrow);f.p.onSelectRow&&e&&f.p.onSelectRow.call(f,f.p.selrow,g)}else{h.className!=="ui-subgrid"&&a(h).removeClass("ui-state-highlight").attr("aria-selected","false");g=false;a("#jqg_"+a.jgrid.jqID(f.p.selrow),
f.rows[h.rowIndex]).attr("checked",g);f.p.selarrrow.splice(j,1);f.p.onSelectRow&&e&&f.p.onSelectRow.call(f,f.p.selrow,g);h=f.p.selarrrow[0];f.p.selrow=h===undefined?null:h}}else if(h.className!=="ui-subgrid"){f.p.selrow&&a("tr#"+a.jgrid.jqID(f.p.selrow),f.grid.bDiv).removeClass("ui-state-highlight").attr("aria-selected","false");f.p.selrow=h.id;a(h).addClass("ui-state-highlight").attr("aria-selected","true");f.p.onSelectRow&&e&&f.p.onSelectRow.call(f,f.p.selrow,true)}}}})},resetSelection:function(){return this.each(function(){var d=
...
this.each(function(){var j=this,c,l;if(typeof d=="undefined"){f=true;b=[];g=j.rows.length}else{l=j.rows.namedItem(d);if(!l)return e;g=1}for(;h<g;){if(f)l=j.rows[h];a("td",l).each(function(k){c=j.p.colModel[k].name;if(c!=="cb"&&c!=="subgrid")if(j.p.treeGrid===true&&c==j.p.ExpandColumn)e[c]=a.jgrid.htmlDecode(a("span:first",this).html());else try{e[c]=a.unformat(this,{rowId:l.id,colModel:j.p.colModel[k]},k)}catch(o){e[c]=a.jgrid.htmlDecode(a(this).html())}});h++;if(f){b.push(e);e={}}}});return b?b:
...
e,b){var f,g=false;this.each(function(){var h=this,j,c,l=typeof b;if(!h.grid)return false;c=h.rows.namedItem(d);if(!c)return false;e&&a(this.p.colModel).each(function(k){f=this.name;if(e[f]!=undefined){j=h.formatter(d,e[f],k,e,"edit");h.p.treeGrid===true&&f==h.p.ExpandColumn?a("td:eq("+k+") > span:first",c).html(j).attr("title",a.jgrid.stripHtml(j)):a("td:eq("+k+")",c).html(j).attr("title",a.jgrid.stripHtml(j));g=true}});if(l==="string")a(c).addClass(b);else l==="object"&&a(c).css(b)});return g},
addRowData:function(d,e,b,f){b||(b="last");var g=false,h,j,c,l,k,o,m,n,q="",r,u,v;if(e){if(a.isArray(e)){r=true;b="last";u=d}else{e=[e];r=false}this.each(function(){var w=this,z=e.length;k=w.p.rownumbers===true?1:0;c=w.p.multiselect===true?1:0;l=w.p.subGrid===true?1:0;if(!r)if(typeof d!="undefined")d+="";else{d=w.p.records+1+"";if(w.p.keyIndex!==false){cmn=w.p.colModel[w.p.keyIndex+c+l+k].name;if(typeof e[0][cmn]!="undefined")d=e[0][cmn]}}v=w.p.altclass;for(var D=0,C=a.isFunction(w.p.afterInsertRow)?
...
false){g+=parseInt(this.width,10);if(this.fixed){n+=this.width;m+=this.width+h}else c++;q++}});if(c!=0){b.p.tblwidth=g;o=d-h*c-m;if(!isNaN(b.p.height))if(a(b.grid.bDiv)[0].clientHeight<a(b.grid.bDiv)[0].scrollHeight){l=true;o-=k}g=0;var u=b.grid.cols.length>0;a.each(b.p.colModel,function(v){if(this.hidden===false&&!this.fixed){f=Math.floor(o/(b.p.tblwidth-n)*this.width);if(!(f<0)){this.width=f;g+=f;b.grid.headers[v].width=f;b.grid.headers[v].el.style.width=f+"px";if(b.p.footerrow)b.grid.footers[v].style.width=
...
d){h=l;return false}});else h=parseInt(d,10);if(h>=0){var j=a("tr.ui-jqgrid-labels th:eq("+h+")",g.grid.hDiv);if(e){var c=a(".s-ico",j);a("[id^=jqgh_]",j).empty().html(e).append(c);g.p.colNames[h]=e}if(b)typeof b==="string"?a(j).addClass(b):a(j).css(b);typeof f==="object"&&a(j).attr(f)}}})},setCell:function(d,e,b,f,g){return this.each(function(){var h=this,j=-1,c;if(h.grid){if(isNaN(e))a(h.p.colModel).each(function(k){if(this.name==e){j=k;return false}});else j=parseInt(e,10);if(j>=0)if(c=h.rows.namedItem(d)){var l=
a("td:eq("+j+")",c);if(b!==""){c=h.formatter(d,b,j,c,"edit");h.p.treeGrid&&a(".tree-wrap",a(l)).length>0?a("span",a(l)).html(c).attr("title",a.jgrid.stripHtml(c)):a(l).html(c).attr("title",a.jgrid.stripHtml(c))}if(typeof f==="string")f?a(l).addClass(f):a(l).removeClass();else f&&a(l).css(f);typeof g==="object"&&a(l).attr(g)}}})},getCell:function(d,e){var b=false;this.each(function(){var f=this,g=-1;if(f.grid){if(isNaN(e))a(f.p.colModel).each(function(c){if(this.name===e){g=c;return false}});else g=
...
b.p.autosearch===true&&a("select",this).change(function(){j();return false})});else if(v.sopt.value){var w=v.sopt.value,z=document.createElement("select");a(z).attr({name:v.index||v.name,id:"sg_"+v.name}).attr(v.sopt);if(typeof w==="string"){u=w.split(";");for(var D,C=0;C<u.length;C++){w=u[C].split(":");D=document.createElement("option");D.value=w[0];D.innerHTML=w[1];if(w[1]==v.defval)D.selected="selected";z.appendChild(D)}}else if(typeof w==="object")for(C in w){u++;D=document.createElement("option");
...
c=0,l,k,o={};a.each(b.p.colModel,function(){k=this.index||this.name;var v=this.searchoptions&&this.searchoptions.sopt?this.searchoptions.sopt[0]:"bw";switch(this.stype){case "select":if(l=a("select[name="+k+"]",b.grid.hDiv).val()){j[k]=l;o[k]=v;c++}else try{delete b.p.postData[k]}catch(w){}break;case "text":if(l=a("input[name="+k+"]",b.grid.hDiv).val()){j[k]=l;o[k]=v;c++}else try{delete b.p.postData[k]}catch(z){}break}});var m=c>0?true:false;if(d.stringResult){var n='{"groupOp":"'+d.groupOp+'","rules":[',
q=0;a.each(j,function(v,w){if(q>0)n+=",";n+='{"field":"'+v+'",';n+='"op":"'+o[v]+'",';n+='"data":"'+w+'"}';q++});n+="]}";a.extend(b.p.postData,{filters:n})}else a.extend(b.p.postData,j);var r;if(b.p.searchurl){r=b.p.url;a(b).jqGrid("setGridParam",{url:b.p.searchurl})}var u=false;if(a.isFunction(d.beforeSearch))u=d.beforeSearch.call(b);u||a(b).jqGrid("setGridParam",{search:m}).trigger("reloadGrid",[{page:1}]);r&&a(b).jqGrid("setGridParam",{url:r});a.isFunction(d.afterSearch)&&d.afterSearch()},g=a("<tr class='ui-search-toolbar' role='rowheader'></tr>"),
h;a.each(b.p.colModel,function(){var j=this,c,l,k,o;l=a("<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+b.p.direction+"'></th>");c=a("<div style='width:100%;position:relative;height:100%;padding-right:0.3em;'></div>");this.hidden===true&&a(l).css("display","none");this.search=this.search===false?false:true;if(typeof this.stype=="undefined")this.stype="text";k=a.extend({},this.searchoptions||{});if(this.search)switch(this.stype){case "select":if(o=this.surl||k.dataUrl)a.ajax(a.extend({url:o,
dataType:"html",complete:function(u){if(k.buildSelect!=null)(u=k.buildSelect(u))&&a(c).append(u);else a(c).append(u.responseText);k.defaultValue&&a("select",c).val(k.defaultValue);a("select",c).attr({name:j.index||j.name,id:"gs_"+j.name});k.attr&&a("select",c).attr(k.attr);a("select",c).css({width:"100%"});k.dataInit!=null&&k.dataInit(a("select",c)[0]);k.dataEvents!=null&&e(a("select",c)[0],k.dataEvents);d.autosearch===true&&a("select",c).change(function(){f();return false});u=null}},a.jgrid.ajaxOptions,
b.p.ajaxSelectOptions||{}));else{var m;if(j.searchoptions&&j.searchoptions.value)m=j.searchoptions.value;else if(j.editoptions&&j.editoptions.value)m=j.editoptions.value;if(m){o=document.createElement("select");o.style.width="100%";a(o).attr({name:j.index||j.name,id:"gs_"+j.name});if(typeof m==="string"){m=m.split(";");for(var n,q,r=0;r<m.length;r++){n=m[r].split(":");q=document.createElement("option");q.value=n[0];q.innerHTML=n[1];o.appendChild(q)}}else if(typeof m==="object")for(n in m){q=document.createElement("option");
q.value=n;q.innerHTML=m[n];o.appendChild(q)}k.defaultValue&&a(o).val(k.defaultValue);k.attr&&a(o).attr(k.attr);k.dataInit!=null&&k.dataInit(o);k.dataEvents!=null&&e(o,k.dataEvents);a(c).append(o);d.autosearch===true&&a(o).change(function(){f();return false})}}break;case "text":o=k.defaultValue?k.defaultValue:"";a(c).append("<input type='text' style='width:95%;padding:0px;' name='"+(j.index||j.name)+"' id='gs_"+j.name+"' value='"+o+"'/>");k.attr&&a("input",c).attr(k.attr);k.dataInit!=null&&k.dataInit(a("input",
c)[0]);k.dataEvents!=null&&e(a("input",c)[0],k.dataEvents);if(d.autosearch===true)d.searchOnEnter?a("input",c).keypress(function(u){if((u.charCode?u.charCode:u.keyCode?u.keyCode:0)==13){f();return false}return this}):a("input",c).keydown(function(u){switch(u.which){case 9:case 16:case 37:case 38:case 39:case 40:case 27:break;default:h&&clearTimeout(h);h=setTimeout(function(){f()},500)}});break}a(l).append(c);a(g).append(l)});a("table thead",b.grid.hDiv).append(g);this.triggerToolbar=f;this.clearToolbar=
function(j){var c={},l,k=0,o;j=typeof j!="boolean"?true:j;a.each(b.p.colModel,function(){l=this.searchoptions&&this.searchoptions.defaultValue?this.searchoptions.defaultValue:"";o=this.index||this.name;switch(this.stype){case "select":var v;a("select[name="+o+"] option",b.grid.hDiv).each(function(D){if(D==0)this.selected=true;if(a(this).text()==l){this.selected=true;v=a(this).val();return false}});if(v){c[o]=v;k++}else try{delete b.p.postData[o]}catch(w){}break;case "text":a("input[name="+o+"]",b.grid.hDiv).val(l);
if(l){c[o]=l;k++}else try{delete b.p.postData[o]}catch(z){}break}});var m=k>0?true:false;if(d.stringResult){var n='{"groupOp":"'+d.groupOp+'","rules":[',q=0;a.each(c,function(v,w){if(q>0)n+=",";n+='{"field":"'+v+'",';n+='"op":"eq",';n+='"data":"'+w+'"}';q++});n+="]}";a.extend(b.p.postData,{filters:n})}else a.extend(b.p.postData,c);var r;if(b.p.searchurl){r=b.p.url;a(b).jqGrid("setGridParam",{url:b.p.searchurl})}var u=false;if(a.isFunction(d.beforeClear))u=d.beforeClear.call(b);u||j&&a(b).jqGrid("setGridParam",
{search:m}).trigger("reloadGrid",[{page:1}]);r&&a(b).jqGrid("setGridParam",{url:r});a.isFunction(d.afterClear)&&d.afterClear()};this.toggleToolbar=function(){var j=a("tr.ui-search-toolbar",b.grid.hDiv);j.css("display")=="none"?j.show():j.hide()}})}})})(jQuery);
...
a.jgrid.htmlEncode(x[this.name])}});return true}function c(E,G,V,L){for(var W,R,S,T=0,J,ba,Z,ka=[],aa=false,wa,ta,la="",ma=1;ma<=L;ma++)la+="<td class='CaptionTD ui-widget-content'>&#160;</td><td class='DataTD ui-widget-content' style='white-space:pre'>&#160;</td>";if(E!="_empty")aa=a(G).jqGrid("getInd",E);a(G.p.colModel).each(function(oa){W=this.name;ba=(R=this.editrules&&this.editrules.edithidden==true?false:this.hidden===true?true:false)?"style='display:none'":"";if(W!=="cb"&&W!=="subgrid"&&this.editable===
true&&W!=="rn"){if(aa===false)J="";else if(W==G.p.ExpandColumn&&G.p.treeGrid===true)J=a("td:eq("+oa+")",G.rows[aa]).text();else try{J=a.unformat(a("td:eq("+oa+")",G.rows[aa]),{rowId:E,colModel:this},oa)}catch(Ia){J=a("td:eq("+oa+")",G.rows[aa]).html()}var xa=a.extend({},this.editoptions||{},{id:W,name:W});frmopt=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:false,rowcontent:""},this.formoptions||{});wa=parseInt(frmopt.rowpos)||T+1;ta=parseInt((parseInt(frmopt.colpos)||1)*2);if(E=="_empty"&&xa.defaultValue)J=
a.isFunction(xa.defaultValue)?xa.defaultValue():xa.defaultValue;if(!this.edittype)this.edittype="text";if(m.p.autoencode)J=a.jgrid.htmlDecode(J);Z=createEl(this.edittype,xa,J,false,a.extend({},a.jgrid.ajaxOptions,G.p.ajaxSelectOptions||{}));if(J==""&&this.edittype=="checkbox")J=a(Z).attr("offval");if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[W]=J;a(Z).addClass("FormElement");S=a(V).find("tr[rowpos="+wa+"]");if(frmopt.rowabove){xa=a("<tr><td class='contentinfo' colspan='"+L*2+"'>"+frmopt.rowcontent+
"</td></tr>");a(V).append(xa);xa[0].rp=wa}if(S.length==0){S=a("<tr "+ba+" rowpos='"+wa+"'></tr>").addClass("FormData").attr("id","tr_"+W);a(S).append(la);a(V).append(S);S[0].rp=wa}a("td:eq("+(ta-2)+")",S[0]).html(typeof frmopt.label==="undefined"?G.p.colNames[oa]:frmopt.label);a("td:eq("+(ta-1)+")",S[0]).append(frmopt.elmprefix).append(Z).append(frmopt.elmsuffix);ka[T]=oa;T++}});if(T>0){ma=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(L*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+
G.p.id+"_id' value='"+E+"'/></td></tr>");ma[0].rp=T+999;a(V).append(ma);if(d.checkOnSubmit||d.checkOnUpdate)d._savedData.id=E}return ka}function l(E,G,V){var L,W=0,R,S,T,J,ba;if(d.checkOnSubmit||d.checkOnUpdate){d._savedData={};d._savedData.id=E}var Z=G.p.colModel;if(E=="_empty"){a(Z).each(function(){L=this.name;T=a.extend({},this.editoptions||{});S=a("#"+a.jgrid.jqID(L),"#"+V);if(S[0]!=null){J="";if(T.defaultValue){J=a.isFunction(T.defaultValue)?T.defaultValue():T.defaultValue;if(S[0].type=="checkbox"){ba=
J.toLowerCase();if(ba.search(/(false|0|no|off|undefined)/i)<0&&ba!==""){S[0].checked=true;S[0].defaultChecked=true;S[0].value=J}else S.attr({checked:"",defaultChecked:""})}else S.val(J)}else if(S[0].type=="checkbox"){S[0].checked=false;S[0].defaultChecked=false;J=a(S).attr("offval")}else if(S[0].type.substr(0,6)=="select")S[0].selectedIndex=0;else S.val(J);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=J}});a("#id_g","#"+V).val(E)}else{var ka=a(G).jqGrid("getInd",E,true);if(ka){a("td",
ka).each(function(aa){L=Z[aa].name;if(L!=="cb"&&L!=="subgrid"&&L!=="rn"&&Z[aa].editable===true){if(L==G.p.ExpandColumn&&G.p.treeGrid===true)R=a(this).text();else try{R=a.unformat(this,{rowId:E,colModel:Z[aa]},aa)}catch(wa){R=a(this).html()}if(m.p.autoencode)R=a.jgrid.htmlDecode(R);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=R;L=a.jgrid.jqID(L);switch(Z[aa].edittype){case "password":case "text":case "button":case "image":a("#"+L,"#"+V).val(R);break;case "textarea":if(R=="&nbsp;"||R==
"&#160;"||R.length==1&&R.charCodeAt(0)==160)R="";a("#"+L,"#"+V).val(R);break;case "select":var ta=R.split(",");ta=a.map(ta,function(ma){return a.trim(ma)});a("#"+L+" option","#"+V).each(function(){this.selected=!Z[aa].editoptions.multiple&&(ta[0]==a(this).text()||ta[0]==a(this).val())?true:Z[aa].editoptions.multiple?a.inArray(a(this).text(),ta)>-1||a.inArray(a(this).val(),ta)>-1?true:false:false});break;case "checkbox":R+="";R=R.toLowerCase();if(R.search(/(false|0|no|off|undefined)/i)<0&&R!==""){a("#"+
L,"#"+V).attr("checked",true);a("#"+L,"#"+V).attr("defaultChecked",true)}else{a("#"+L,"#"+V).attr("checked",false);a("#"+L,"#"+V).attr("defaultChecked","")}break;case "custom":try{if(Z[aa].editoptions&&a.isFunction(Z[aa].editoptions.custom_value))Z[aa].editoptions.custom_value(a("#"+L,"#"+V),"set",R);else throw"e1";}catch(la){la=="e1"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,la.message,
jQuery.jgrid.edit.bClose)}break}W++}});W>0&&a("#id_g","#"+r).val(E)}}}function k(){var E,G=[true,"",""],V={},L=m.p.prmNames,W,R;if(a.isFunction(d.beforeCheckValues)){var S=d.beforeCheckValues(x,a("#"+q),x[m.p.id+"_id"]=="_empty"?L.addoper:L.editoper);if(S&&typeof S==="object")x=S}for(var T in x){G=checkValues(x[T],T,m);if(G[0]==false)break}if(G[0]){if(a.isFunction(d.onclickSubmit))V=d.onclickSubmit(d,x)||{};if(a.isFunction(d.beforeSubmit))G=d.beforeSubmit(x,a("#"+q))}H=d.url?d.url:a(m).jqGrid("getGridParam",
"editurl");if(G[0])if(!H){G[0]=false;G[1]+=" "+a.jgrid.errors.nourl}if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else if(!d.processing){d.processing=true;a("#sData","#"+r+"_2").addClass("ui-state-active");R=L.oper;W=L.id;x[R]=a.trim(x[m.p.id+"_id"])=="_empty"?L.addoper:L.editoper;if(x[R]!=L.addoper)x[W]=x[m.p.id+"_id"];else if(x[W]===undefined)x[W]=x[m.p.id+"_id"];delete x[m.p.id+"_id"];x=a.extend(x,d.editData,V);a.ajax(a.extend({url:H,type:d.mtype,data:a.isFunction(d.serializeEditData)?
d.serializeEditData(x):x,complete:function(J,ba){if(ba!="success"){G[0]=false;G[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(J):ba+" Status: '"+J.statusText+"'. Error code: "+J.status}else if(a.isFunction(d.afterSubmit))G=d.afterSubmit(J,x);if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else{a.each(m.p.colModel,function(){if(N[this.name]&&this.formatter&&this.formatter=="select")try{delete N[this.name]}catch(ka){}});x=a.extend(x,N);m.p.autoencode&&a.each(x,
function(ka,aa){x[ka]=a.jgrid.htmlDecode(aa)});if(x[R]==L.addoper){G[2]||(G[2]=parseInt(m.p.records)+1);x[W]=G[2];if(d.closeAfterAdd){if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else{a(m).jqGrid("addRowData",G[2],x,b.addedrow);a(m).jqGrid("setSelection",G[2])}hideModal("#"+u.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:d.onClose})}else if(d.clearAfterAdd){d.reloadAfterSubmit?a(m).trigger("reloadGrid"):a(m).jqGrid("addRowData",G[2],x,b.addedrow);l("_empty",m,q)}else d.reloadAfterSubmit?a(m).trigger("reloadGrid"):
a(m).jqGrid("addRowData",G[2],x,b.addedrow)}else{if(d.reloadAfterSubmit){a(m).trigger("reloadGrid");d.closeAfterEdit||setTimeout(function(){a(m).jqGrid("setSelection",x[W])},1E3)}else m.p.treeGrid===true?a(m).jqGrid("setTreeRow",x[W],x):a(m).jqGrid("setRowData",x[W],x);d.closeAfterEdit&&hideModal("#"+u.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:d.onClose})}if(a.isFunction(d.afterComplete)){E=J;setTimeout(function(){d.afterComplete(E,x,a("#"+q));E=null},500)}}d.processing=false;if(d.checkOnSubmit||
d.checkOnUpdate){a("#"+q).data("disabled",false);if(d._savedData.id!="_empty")d._savedData=x}a("#sData","#"+r+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+q)[0].focus()}catch(Z){}},error:function(J,ba,Z){a("#FormError>td","#"+r).html(ba+" : "+Z);a("#FormError","#"+r).show();d.processing=false;a("#"+q).data("disabled",false);a("#sData","#"+r+"_2").removeClass("ui-state-active")}},a.jgrid.ajaxOptions,d.ajaxEditOptions))}}function o(E,G){var V=false,L;for(L in E)if(E[L]!=G[L]){V=true;
break}return V}var m=this;if(m.grid&&e){var n=m.p.id,q="FrmGrid_"+n,r="TblGrid_"+n,u={themodal:"editmod"+n,modalhead:"edithd"+n,modalcontent:"editcnt"+n,scrollelm:q},v=a.isFunction(d.beforeShowForm)?d.beforeShowForm:false,w=a.isFunction(d.afterShowForm)?d.afterShowForm:false,z=a.isFunction(d.beforeInitData)?d.beforeInitData:false,D=a.isFunction(d.onInitializeForm)?d.onInitializeForm:false,C=1,y=0,H,x,N,Y,da;if(e=="new"){e="_empty";b.caption=b.addCaption}else b.caption=b.editCaption;b.recreateForm===
true&&a("#"+u.themodal).html()!=null&&a("#"+u.themodal).remove();var P=true;if(b.checkOnUpdate&&b.jqModal&&!b.modal)P=false;if(a("#"+u.themodal).html()!=null){a(".ui-jqdialog-title","#"+u.modalhead).html(b.caption);a("#FormError","#"+r).hide();if(d.topinfo){a(".topinfo","#"+r+"_2").html(d.topinfo);a(".tinfo","#"+r+"_2").show()}else a(".tinfo","#"+r+"_2").hide();if(d.bottominfo){a(".bottominfo","#"+r+"_2").html(d.bottominfo);a(".binfo","#"+r+"_2").show()}else a(".binfo","#"+r+"_2").hide();z&&z(a("#"+
q));l(e,m,q);e=="_empty"||!d.viewPagerButtons?a("#pData, #nData","#"+r+"_2").hide():a("#pData, #nData","#"+r+"_2").show();if(d.processing===true){d.processing=false;a("#sData","#"+r+"_2").removeClass("ui-state-active")}if(a("#"+q).data("disabled")===true){a(".confirm","#"+u.themodal).hide();a("#"+q).data("disabled",false)}v&&v(a("#"+q));a("#"+u.themodal).data("onClose",d.onClose);viewModal("#"+u.themodal,{gbox:"#gbox_"+n,jqm:b.jqModal,jqM:false,closeoverlay:P,modal:b.modal});P||a(".jqmOverlay").click(function(){if(!h())return false;
...
a(ea).append(ga);U=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+C*2+"'></td></tr>");U[0].rp=0;a(ga).append(U);U=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+C*2+"'>"+d.topinfo+"</td></tr>");U[0].rp=0;a(ga).append(U);z&&z(a("#"+q));U=(z=m.p.direction=="rtl"?true:false)?"nData":"pData";var sa=z?"pData":"nData";c(e,m,ga,C);U="<a href='javascript:void(0)' id='"+U+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></div>";
sa="<a href='javascript:void(0)' id='"+sa+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></div>";var za="<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default ui-corner-all'>"+b.bSubmit+"</a>",Ca="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+b.bCancel+"</a>";U="<table border='0' class='EditTable' id='"+r+"_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content'>"+
(z?sa+U:U+sa)+"</td><td class='EditButton ui-widget-content'>"+za+Ca+"</td></tr>";U+="<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+d.bottominfo+"</td></tr>";U+="</tbody></table>";if(y>0){var va=[];a.each(a(ga)[0].rows,function(E,G){va[E]=G});va.sort(function(E,G){if(E.rp>G.rp)return 1;if(E.rp<G.rp)return-1;return 0});a.each(va,function(E,G){a("tbody",ga).append(G)})}b.gbox="#gbox_"+n;var Ga=false;if(b.closeOnEscape===true){b.closeOnEscape=false;Ga=true}ea=a("<span></span>").append(ea).append(U);
createModal(u,ea,b,"#gview_"+m.p.id,a("#gview_"+m.p.id)[0]);if(z){a("#pData, #nData","#"+r+"_2").css("float","right");a(".EditButton","#"+r+"_2").css("text-align","left")}d.topinfo&&a(".tinfo","#"+r+"_2").show();d.bottominfo&&a(".binfo","#"+r+"_2").show();U=ea=null;a("#"+u.themodal).keydown(function(E){var G=E.target;if(a("#"+q).data("disabled")===true)return false;if(d.savekey[0]===true&&E.which==d.savekey[1])if(G.tagName!="TEXTAREA"){a("#sData","#"+r+"_2").trigger("click");return false}if(E.which===
27){if(!h())return false;Ga&&hideModal(this,{gb:b.gbox,jqm:b.jqModal,onClose:d.onClose});return false}if(d.navkeys[0]===true){if(a("#id_g","#"+r).val()=="_empty")return true;if(E.which==d.navkeys[1]){a("#pData","#"+r+"_2").trigger("click");return false}if(E.which==d.navkeys[2]){a("#nData","#"+r+"_2").trigger("click");return false}}});if(b.checkOnUpdate){a("a.ui-jqdialog-titlebar-close span","#"+u.themodal).removeClass("jqmClose");a("a.ui-jqdialog-titlebar-close","#"+u.themodal).unbind("click").click(function(){if(!h())return false;
hideModal("#"+u.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:d.onClose});return false})}b.saveicon=a.extend([true,"left","ui-icon-disk"],b.saveicon);b.closeicon=a.extend([true,"left","ui-icon-close"],b.closeicon);if(b.saveicon[0]==true)a("#sData","#"+r+"_2").addClass(b.saveicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.saveicon[2]+"'></span>");if(b.closeicon[0]==true)a("#cData","#"+r+"_2").addClass(b.closeicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+
b.closeicon[2]+"'></span>");if(d.checkOnSubmit||d.checkOnUpdate){za="<a href='javascript:void(0)' id='sNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+b.bYes+"</a>";sa="<a href='javascript:void(0)' id='nNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+b.bNo+"</a>";Ca="<a href='javascript:void(0)' id='cNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+b.bExit+"</a>";ea=b.zIndex||999;ea++;a("<div class='ui-widget-overlay jqgrid-overlay confirm' style='z-index:"+
ea+";display:none;'>&#160;"+(a.browser.msie&&a.browser.version==6?'<iframe style="display:block;position:absolute;z-index:-1;filter:Alpha(Opacity=\'0\');" src="javascript:false;"></iframe>':"")+"</div><div class='confirm ui-widget-content ui-jqconfirm' style='z-index:"+(ea+1)+"'>"+b.saveData+"<br/><br/>"+za+sa+Ca+"</div>").insertAfter("#"+q);a("#sNew","#"+u.themodal).click(function(){k();a("#"+q).data("disabled",false);a(".confirm","#"+u.themodal).hide();return false});a("#nNew","#"+u.themodal).click(function(){a(".confirm",
...
"#"+n.modalhead).focus()},0)}function g(x,N){x==0?a("#pData","#"+m+"_2").addClass("ui-state-disabled"):a("#pData","#"+m+"_2").removeClass("ui-state-disabled");x==N?a("#nData","#"+m+"_2").addClass("ui-state-disabled"):a("#nData","#"+m+"_2").removeClass("ui-state-disabled")}function h(){var x=a(l).jqGrid("getDataIDs"),N=a("#id_g","#"+m).val();return[a.inArray(N,x),x]}function j(x,N,Y,da){for(var P,ea,U,ga=0,sa,za,Ca=[],va=false,Ga="<td class='CaptionTD form-view-label ui-widget-content' width='"+b.labelswidth+
...
-1)V=Math.max(V,parseInt(this.width,10));else L=Math.max(L,parseInt(this.width,10))});W=V!==0?V:L!==0?L:0;va=a(N).jqGrid("getInd",x);a(N.p.colModel).each(function(J){P=this.name;R=false;za=(ea=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";S=typeof this.viewable!="boolean"?true:this.viewable;if(P!=="cb"&&P!=="subgrid"&&P!=="rn"&&S){sa=va===false?"":P==N.p.ExpandColumn&&N.p.treeGrid===true?a("td:eq("+J+")",N.rows[va]).text():a("td:eq("+
J+")",N.rows[va]).html();R=this.align==="right"&&W!==0?true:false;a.extend({},this.editoptions||{},{id:P,name:P});var ba=a.extend({},{rowabove:false,rowcontent:""},this.formoptions||{}),Z=parseInt(ba.rowpos)||ga+1,ka=parseInt((parseInt(ba.colpos)||1)*2);if(ba.rowabove){var aa=a("<tr><td class='contentinfo' colspan='"+da*2+"'>"+ba.rowcontent+"</td></tr>");a(Y).append(aa);aa[0].rp=Z}U=a(Y).find("tr[rowpos="+Z+"]");if(U.length==0){U=a("<tr "+za+" rowpos='"+Z+"'></tr>").addClass("FormData").attr("id",
"trv_"+P);a(U).append(E);a(Y).append(U);U[0].rp=Z}a("td:eq("+(ka-2)+")",U[0]).html("<b>"+(typeof ba.label==="undefined"?N.p.colNames[J]:ba.label)+"</b>");a("td:eq("+(ka-1)+")",U[0]).append("<span>"+sa+"</span>").attr("id","v_"+P);R&&a("td:eq("+(ka-1)+") span",U[0]).css({"text-align":"right",width:W+"px"});Ca[ga]=J;ga++}});if(ga>0){x=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(da*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+
x+"'/></td></tr>");x[0].rp=ga+99;a(Y).append(x)}return Ca}function c(x,N){var Y,da,P=0,ea,U;if(U=a(N).jqGrid("getInd",x,true)){a("td",U).each(function(ga){Y=N.p.colModel[ga].name;da=N.p.colModel[ga].editrules&&N.p.colModel[ga].editrules.edithidden===true?false:N.p.colModel[ga].hidden===true?true:false;if(Y!=="cb"&&Y!=="subgrid"&&Y!=="rn"){ea=Y==N.p.ExpandColumn&&N.p.treeGrid===true?a(this).text():a(this).html();a.extend({},N.p.colModel[ga].editoptions||{});Y=a.jgrid.jqID("v_"+Y);a("#"+Y+" span","#"+
m).html(ea);da&&a("#"+Y,"#"+m).parents("tr:first").hide();P++}});P>0&&a("#id_g","#"+m).val(x)}}var l=this;if(l.grid&&e){if(!b.imgpath)b.imgpath=l.p.imgpath;var k=l.p.id,o="ViewGrid_"+k,m="ViewTbl_"+k,n={themodal:"viewmod"+k,modalhead:"viewhd"+k,modalcontent:"viewcnt"+k,scrollelm:o},q=1,r=0;if(a("#"+n.themodal).html()!=null){a(".ui-jqdialog-title","#"+n.modalhead).html(b.caption);a("#FormError","#"+m).hide();c(e,l);a.isFunction(b.beforeShowForm)&&b.beforeShowForm(a("#"+o));viewModal("#"+n.themodal,
...
"'></span>"+b.refreshtext+"</div>");a("tr",q).append(n);a(n,q).attr({title:b.refreshtitle||"",id:"refresh_"+u}).click(function(){a.isFunction(b.beforeRefresh)&&b.beforeRefresh();k.p.search=false;try{a("#fbox_"+k.p.id).searchFilter().reset();a.isFunction(k.clearToolbar)&&k.clearToolbar(false)}catch(v){}switch(b.refreshstate){case "firstpage":a(k).trigger("reloadGrid",[{page:1}]);break;case "current":a(k).trigger("reloadGrid",[{current:true}]);break}a.isFunction(b.afterRefresh)&&b.afterRefresh();return false}).hover(function(){a(this).addClass("ui-state-hover")},
...
{update:f.p.sortable};if(g.start){var h=g.start;g.start=function(j,c){b();h.call(this,j,c)}}else g.start=b;if(f.p.sortable.exclude)g.items+=":not("+f.p.sortable.exclude+")";e.sortable(g).data("sortable").floating=true})},columnChooser:function(e){function b(n,q,r){if(q>=0){var u=n.slice(),v=u.splice(q,Math.max(n.length-q,q));if(q>n.length)q=n.length;u[q]=r;return u.concat(v)}}function f(n,q){if(n)if(typeof n=="string")a.fn[n]&&a.fn[n].apply(q,a.makeArray(arguments).slice(2));else a.isFunction(n)&&
n.apply(q,a.makeArray(arguments).slice(2))}var g=this;if(!a("#colchooser_"+g[0].p.id).length){var h=a('<div id="colchooser_'+g[0].p.id+'" style="position:relative;overflow:hidden"><div><select multiple="multiple"></select></div></div>'),j=a("select",h);e=a.extend({width:420,height:240,classname:null,done:function(n){n&&g.jqGrid("remapColumns",n,true)},msel:"multiselect",dlog:"dialog",dlog_opts:function(n){var q={};q[n.bSubmit]=function(){n.apply_perm();n.cleanup(false)};q[n.bCancel]=function(){n.cleanup(true)};
return{buttons:q,close:function(){n.cleanup(true)},modal:false,resizable:false,width:n.width+20}},apply_perm:function(){a("option",j).each(function(){this.selected?g.jqGrid("showCol",c[this.value].name):g.jqGrid("hideCol",c[this.value].name)});var n=[];a("option[selected]",j).each(function(){n.push(parseInt(this.value))});a.each(n,function(){delete k[c[parseInt(this)].name]});a.each(k,function(){var q=parseInt(this);n=b(n,q,q)});e.done&&e.done.call(g,n)},cleanup:function(n){f(e.dlog,h,"destroy");
f(e.msel,j,"destroy");h.remove();n&&e.done&&e.done.call(g)},msel_opts:{}},a.jgrid.col,e||{});e.caption&&h.attr("title",e.caption);if(e.classname){h.addClass(classname);j.addClass(classname)}if(e.width){a(">div",h).css({width:e.width,margin:"0 auto"});j.css("width",e.width)}if(e.height){a(">div",h).css("height",e.height);j.css("height",e.height-10)}var c=g.jqGrid("getGridParam","colModel"),l=g.jqGrid("getGridParam","colNames"),k={},o=[];j.empty();a.each(c,function(n){k[this.name]=n;if(this.hidedlg)this.hidden||
o.push(n);else j.append("<option value='"+n+"' "+(this.hidden?"":"selected='selected'")+">"+l[n]+"</option>")});var m=a.isFunction(e.dlog_opts)?e.dlog_opts.call(g,e):e.dlog_opts;f(e.dlog,h,m);m=a.isFunction(e.msel_opts)?e.msel_opts.call(g,e):e.msel_opts;f(e.msel,j,m)}},sortableRows:function(e){return this.each(function(){var b=this;if(b.grid)if(!b.p.treeGrid)if(a.fn.sortable){e=a.extend({cursor:"move",axis:"y",items:".jqgrow"},e||{});if(e.start&&a.isFunction(e.start)){e._start_=e.start;delete e.start}else e._start_=
...
1)});e._update_&&e._update_.apply(this,[f,g])};a("tbody:first",b).sortable(e)}})},gridDnD:function(e){return this.each(function(){function b(){var h=a.data(f,"dnd");a("tr.jqgrow:not(.ui-draggable)",f).draggable(a.isFunction(h.drag)?h.drag.call(a(f),h):h.drag)}var f=this;if(f.grid)if(!f.p.treeGrid)if(a.fn.draggable&&a.fn.droppable){a("#jqgrid_dnd").html()==null&&a("body").append("<table id='jqgrid_dnd' class='ui-jqgrid-dnd'></table>");if(typeof e=="string"&&e=="updateDnD"&&f.p.jqgdnd==true)b();else{e=
a.extend({drag:function(h){return a.extend({start:function(j,c){if(f.p.subGrid){var l=a(c.helper).attr("id");try{a(f).jqGrid("collapseSubGridRow",l)}catch(k){}}for(l=0;l<a.data(f,"dnd").connectWith.length;l++)a(a.data(f,"dnd").connectWith[l]).jqGrid("getGridParam","reccount")=="0"&&a(a.data(f,"dnd").connectWith[l]).jqGrid("addRowData","jqg_empty_row",{});c.helper.addClass("ui-state-highlight");a("td",c.helper).each(function(o){this.style.width=f.grid.headers[o].width+"px"});h.onstart&&a.isFunction(h.onstart)&&
h.onstart.call(a(f),j,c)},stop:function(j,c){if(c.helper.dropped){var l=a(c.helper).attr("id");a(f).jqGrid("delRowData",l)}for(l=0;l<a.data(f,"dnd").connectWith.length;l++)a(a.data(f,"dnd").connectWith[l]).jqGrid("delRowData","jqg_empty_row");h.onstop&&a.isFunction(h.onstop)&&h.onstop.call(a(f),j,c)}},h.drag_opts||{})},drop:function(h){return a.extend({accept:function(j){var c=a(j).closest("table.ui-jqgrid-btable");if(a.data(c[0],"dnd")!=undefined){j=a.data(c[0],"dnd").connectWith;return a.inArray("#"+
this.id,j)!=-1?true:false}return j},drop:function(j,c){var l=a(c.draggable).attr("id");l=a("#"+f.id).jqGrid("getRowData",l);if(!h.dropbyname){var k=0,o={},m,n=a("#"+this.id).jqGrid("getGridParam","colModel");try{for(key in l){if(n[k]){m=n[k].name;o[m]=l[key]}k++}l=o}catch(q){}}c.helper.dropped=true;if(h.beforedrop&&a.isFunction(h.beforedrop)){m=h.beforedrop.call(this,j,c,l,a("#"+f.id),a(this));if(typeof m!="undefined"&&m!==null&&typeof m=="object")l=m}if(c.helper.dropped){var r;if(h.autoid)if(a.isFunction(h.autoid))r=
h.autoid.call(this,l);else{r=Math.ceil(Math.random()*1E3);r=h.autoidprefix+r}a("#"+this.id).jqGrid("addRowData",r,l,h.droppos)}h.ondrop&&a.isFunction(h.ondrop)&&h.ondrop.call(this,j,c,l)}},h.drop_opts||{})},onstart:null,onstop:null,beforedrop:null,ondrop:null,drop_opts:{activeClass:"ui-state-active",hoverClass:"ui-state-hover"},drag_opts:{revert:"invalid",helper:"clone",cursor:"move",appendTo:"#jqgrid_dnd",zIndex:5E3},dropbyname:false,droppos:"first",autoid:true,autoidprefix:"dnd_"},e||{});if(e.connectWith){e.connectWith=
e.connectWith.split(",");e.connectWith=a.map(e.connectWith,function(h){return a.trim(h)});a.data(f,"dnd",e);f.p.reccount!="0"&&!f.p.jqgdnd&&b();f.p.jqgdnd=true;for(var g=0;g<e.connectWith.length;g++)a(e.connectWith[g]).droppable(a.isFunction(e.drop)?e.drop.call(a(f),e):e.drop)}}}})},gridResize:function(e){return this.each(function(){var b=this;if(b.grid&&a.fn.resizable){e=a.extend({},e||{});if(e.alsoResize){e._alsoResize_=e.alsoResize;delete e.alsoResize}else e._alsoResize_=false;if(e.stop&&a.isFunction(e.stop)){e._stop_=
e.stop;delete e.stop}else e._stop_=false;e.stop=function(f,g){a(b).jqGrid("setGridParam",{height:a("#gview_"+b.p.id+" .ui-jqgrid-bdiv").height()});a(b).jqGrid("setGridWidth",g.size.width,e.shrinkToFit);e._stop_&&e._stop_.call(b,f,g)};e.alsoResize=e._alsoResize_?eval("("+("{'#gview_"+b.p.id+" .ui-jqgrid-bdiv':true,'"+e._alsoResize_+"':true}")+")"):a(".ui-jqgrid-bdiv","#gview_"+b.p.id);delete e._alsoResize_;a("#gbox_"+b.p.id).resizable(e)}})}})})(jQuery);
