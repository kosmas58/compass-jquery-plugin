        ...
        _fn : {
                ...
                obj.each(function () {
                    ...
                    t.find("li").andSelf().each(function () {
                        ...
                        if(rc) {
                            ...
                            if(c === "jstree-checked") { $t.children(":radio").attr("checked","checked"); }
                        }
                    });
                });
                if(!ts) {
            ...
            change_state : function (obj, state) {
                ...
                if(this._get_settings().radio.two_state) {
                    if(state) { 
                        obj.removeClass("jstree-checked").addClass("jstree-unchecked"); 
                        if(rc) { obj.children(":radio").removeAttr("checked"); }
                    }
                    else { 
                        obj.removeClass("jstree-unchecked").addClass("jstree-checked"); 
                        if(rc) { obj.children(":radio").attr("checked","checked"); }
                    }
                }
                else {
                    if(state) { 
                        ...
                        if(!coll.filter(".jstree-checked, .jstree-undetermined").length) { return false; }
                        coll.removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked"); 
                        if(rc) { coll.children(":radio").removeAttr("checked"); }
                    }
                    else { 
                        coll = obj.find("li").andSelf();
                        if(!coll.filter(".jstree-unchecked, .jstree-undetermined").length) { return false; }
                        coll.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked"); 
                        if(rc) { coll.children(":radio").attr("checked","checked"); }
                        if(this.data.ui) { this.data.ui.last_selected = obj; }
                    ...
                    obj.parentsUntil(".jstree", "li").each(function () {
                        ...
                        if(state) {
                            if($this.children("ul").children("li.jstree-checked, li.jstree-undetermined").length) {
                                $this.parentsUntil(".jstree", "li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
                                if(rc) { $this.parentsUntil(".jstree", "li").andSelf().children(":radio").removeAttr("checked"); }
                                return false;
                            }
                            else {
                                $this.removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked");
                                if(rc) { $this.children(":radio").removeAttr("checked"); }
                            }
                        }
                        else {
                            if($this.children("ul").children("li.jstree-unchecked, li.jstree-undetermined").length) {
                                ...
                                $this.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");
                                if(rc) { $this.children(":radio").attr("checked","checked"); }
                            }
                        }
                    });
                }
                if(this.data.ui && this.data.radio.noui) { this.data.ui.selected = this.get_checked(); }
                this.__callback(obj);
                return true;
            },
            check_node : function (obj) {
            ...
            uncheck_all : function () {
                var _this = this,
                    coll = this._get_settings().radio.two_state ? this.get_container_ul().find("li") : this.get_container().children("li");
                coll.each(function () {
                    _this.change_state(this, true);
                });
                this.__callback();
            ...
            _repair_state : function (obj) {
                ...
                if(c === 0) { if(obj.hasClass("jstree-undetermined")) { this.change_state(obj, false); } }
                    ...
                    obj.parentsUntil(".jstree","li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
                    if(rc) { obj.parentsUntil(".jstree", "li").andSelf().children(":radio").removeAttr("checked"); }
                }
            },
            reselect : function () {
                if(this.data.ui && this.data.radio.noui) { 
