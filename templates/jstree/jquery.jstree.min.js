...
false},this))},defaults:{select_node:false,show_at_node:true,items:{create:{separator_before:false,separator_after:true,icon:"/images/jquery/ui/contextMenu/new.png",label:"Create",action:function(a){this.create(a)}},rename:{separator_before:false,separator_after:false,icon:"/images/jquery/ui/contextMenu/rename.png",label:"Rename",action:function(a){this.rename(a)}},remove:{separator_before:false,icon:"/images/jquery/ui/contextMenu/delete.png",separator_after:false,label:"Delete",action:function(a){this.is_selected(a)?
this.remove():this.remove(a)}},ccp:{separator_before:true,icon:"/images/jquery/ui/contextMenu/edit.png",separator_after:false,label:"Edit",action:false,submenu:{cut:{separator_before:false,separator_after:false,icon:"/images/jquery/ui/contextMenu/cut.png",label:"Cut",action:function(a){this.cut(a)}},copy:{separator_before:false,icon:"/images/jquery/ui/contextMenu/copy.png",separator_after:false,label:"Copy",action:function(a){this.copy(a)}},paste:{separator_before:false,icon:"/images/jquery/ui/contextMenu/paste.png",
separator_after:false,label:"Paste",action:function(a){this.paste(a)}}}}}},_fn:{show_contextmenu:function(a,d,f){a=this._get_node(a);var e=this.get_settings().contextmenu,b=a.children("a:visible:eq(0)"),g=false;g=false;if(e.select_node&&this.data.ui&&!this.is_selected(a)){this.deselect_all();this.select_node(a,true)}if(e.show_at_node||typeof d==="undefined"||typeof f==="undefined"){g=b.offset();d=g.left;f=g.top+this.data.core.li_height}g=a.data("jstree")&&a.data("jstree").contextmenu?a.data("jstree").contextmenu:
e.items;if(c.isFunction(g))g=g.call(this,a);this.data.contextmenu=true;c.vakata.context.show(g,b,d,f,this,a,this._get_settings().core.rtl);this.data.themes&&c.vakata.context.cnt.attr("class","jstree-"+this.data.themes.theme+"-context")}}})})(jQuery);(function(c){c.jstree.plugin("types",{__init:function(){var a=this._get_settings().types;this.data.types.attach_to=[];this.get_container().bind("init.jstree",c.proxy(function(){var d=a.type_attr,f="",e=this;c.each(a.types,function(b,g){c.each(g,function(h){/^(max_depth|max_children|icon|valid_children)$/.test(h)||
e.data.types.attach_to.push(h)});if(!g.icon)return true;if(g.icon.image||g.icon.position){f+=b=="default"?".jstree-"+e.get_index()+" a > .jstree-icon { ":".jstree-"+e.get_index()+" li["+d+'="'+b+'"] > a > .jstree-icon { ';if(g.icon.image)f+=" background-image:url("+g.icon.image+"); ";f+=g.icon.position?" background-position:"+g.icon.position+"; ":" background-position:0 0; ";f+="} "}});f!==""&&c.vakata.css.add_sheet({str:f,title:"jstree-types"})},this)).bind("before.jstree",c.proxy(function(d,f){var e,
b;if((e=(e=this._get_settings().types.use_data?this._get_node(f.args[0]):false)&&e!==-1&&e.length?e.data("jstree"):false)&&e.types&&e.types[f.func]===false){d.stopImmediatePropagation();return false}if(c.inArray(f.func,this.data.types.attach_to)!==-1)if(!(!f.args[0]||!f.args[0].tagName&&!f.args[0].jquery)){e=this._get_settings().types.types;b=this._get_type(f.args[0]);if((e[b]&&typeof e[b][f.func]!=="undefined"||e["default"]&&typeof e["default"][f.func]!=="undefined")&&this._check(f.func,f.args[0])===
false){d.stopImmediatePropagation();return false}}},this));r&&this.get_container().bind("load_node.jstree set_type.jstree",c.proxy(function(d,f){var e=f&&f.rslt&&f.rslt.obj&&f.rslt.obj!==-1?this._get_node(f.rslt.obj).parent():this.get_container_ul(),b=false,g=this._get_settings().types;c.each(g.types,function(h,j){if(j.icon&&(j.icon.image||j.icon.position)){b=h==="default"?e.find("li > a > .jstree-icon"):e.find("li["+g.type_attr+"='"+h+"'] > a > .jstree-icon");j.icon.image&&b.css("backgroundImage",
"url("+j.icon.image+")");b.css("backgroundPosition",j.icon.position||"0 0")}})},this))},defaults:{max_children:-1,max_depth:-1,valid_children:"all",use_data:false,type_attr:"rel",types:{"default":{max_children:-1,max_depth:-1,valid_children:"all"}}},_fn:{_types_notify:function(a,d){d.type&&this._get_settings().types.use_data&&this.set_type(d.type,a)},_get_type:function(a){a=this._get_node(a);return!a||!a.length?false:a.attr(this._get_settings().types.type_attr)||"default"},set_type:function(a,d){d=
this._get_node(d);var f=!d.length||!a?false:d.attr(this._get_settings().types.type_attr,a);f&&this.__callback({obj:d,type:a});return f},_check:function(a,d,f){d=this._get_node(d);var e=false,b=this._get_type(d),g=0,h=this,j=this._get_settings().types,l=false;if(d===-1)if(j[a])e=j[a];else return;else{if(b===false)return;if((l=j.use_data?d.data("jstree"):false)&&l.types&&typeof l.types[a]!=="undefined")e=l.types[a];else if(j.types[b]&&typeof j.types[b][a]!=="undefined")e=j.types[b][a];else if(j.types["default"]&&
typeof j.types["default"][a]!=="undefined")e=j.types["default"][a]}if(c.isFunction(e))e=e.call(this,d);a==="max_depth"&&d!==-1&&f!==false&&j.max_depth!==-2&&e!==0&&d.children("a:eq(0)").parentsUntil(".jstree","li").each(function(i){if(j.max_depth!==-1&&j.max_depth-(i+1)<=0){e=0;return false}g=i===0?e:h._check(a,this,false);if(g!==-1&&g-(i+1)<=0){e=0;return false}if(g>=0&&(g-(i+1)<e||e<0))e=g-(i+1);if(j.max_depth>=0&&(j.max_depth-(i+1)<e||e<0))e=j.max_depth-(i+1)});return e},check_move:function(){if(!this.__call_old())return false;
var a=this._get_move(),d=a.rt._get_settings().types,f=a.rt._check("max_children",a.cr),e=a.rt._check("max_depth",a.cr),b=a.rt._check("valid_children",a.cr),g=0,h=1;if(b==="none")return false;if(c.isArray(b)&&a.ot&&a.ot._get_type){a.o.each(function(){if(c.inArray(a.ot._get_type(this),b)===-1)return h=false});if(h===false)return false}if(d.max_children!==-2&&f!==-1){g=a.cr===-1?this.get_container().find("> ul > li").not(a.o).length:a.cr.find("> ul > li").not(a.o).length;if(g+a.o.length>f)return false}if(d.max_depth!==
-2&&e!==-1){h=0;if(e===0)return false;if(typeof a.o.d==="undefined"){for(d=a.o;d.length>0;){d=d.find("> ul > li");h++}a.o.d=h}if(e-a.o.d<0)return false}return true},create_node:function(a,d,f,e,b,g){if(!g&&(b||this._is_loaded(a))){var h=typeof d=="string"&&d.match(/^before|after$/i)&&a!==-1?this._get_parent(a):this._get_node(a),j=this._get_settings().types,l=this._check("max_children",h),i=this._check("max_depth",h),k=this._check("valid_children",h);if(typeof f==="string")f={data:f};f||(f={});if(k===
"none")return false;if(c.isArray(k))if(!f.attr||!f.attr[j.type_attr]){if(!f.attr)f.attr={};f.attr[j.type_attr]=k[0]}else if(c.inArray(f.attr[j.type_attr],k)===-1)return false;if(j.max_children!==-2&&l!==-1){h=h===-1?this.get_container().children("> ul > li").length:h.children("> ul > li").length;if(h+1>l)return false}if(j.max_depth!==-2&&i!==-1&&i-1<0)return false}return this.__call_old(true,a,d,f,e,b,g)}}})})(jQuery);(function(c){c.jstree.plugin("html_data",{__init:function(){this.data.html_data.original_container_html=
this.get_container().find(" > ul > li").clone(true);this.data.html_data.original_container_html.find("li").andSelf().contents().filter(function(){return this.nodeType==3}).remove()},defaults:{data:false,ajax:false,correct_state:true},_fn:{load_node:function(a,d,f){var e=this;this.load_node_html(a,function(){e.__callback({obj:e._get_node(a)});d.call(this)},f)},_is_loaded:function(a){a=this._get_node(a);return a==-1||!a||!this._get_settings().html_data.ajax&&!c.isFunction(this._get_settings().html_data.data)||
a.is(".jstree-open, .jstree-leaf")||a.children("ul").children("li").size()>0},load_node_html:function(a,d,f){var e,b=this.get_settings().html_data,g=function(){};e=function(){};if((a=this._get_node(a))&&a!==-1)if(a.data("jstree-is-loading"))return;else a.data("jstree-is-loading",true);switch(true){case c.isFunction(b.data):b.data.call(this,a,c.proxy(function(h){if(h&&h!==""&&h.toString&&h.toString().replace(/^[\s\n]+$/,"")!==""){h=c(h);h.is("ul")||(h=c("<ul />").append(h));if(a==-1||!a)this.get_container().children("ul").empty().append(h.children()).find("li, a").filter(function(){return!this.firstChild||
!this.firstChild.tagName||this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");else{a.children("a.jstree-loading").removeClass("jstree-loading");a.append(h).children("ul").find("li, a").filter(function(){return!this.firstChild||!this.firstChild.tagName||this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");
a.removeData("jstree-is-loading")}this.clean_node(a);d&&d.call(this)}else if(a&&a!==-1){a.children("a.jstree-loading").removeClass("jstree-loading");a.removeData("jstree-is-loading");if(b.correct_state){this.correct_state(a);d&&d.call(this)}}else if(b.correct_state){this.get_container().children("ul").empty();d&&d.call(this)}},this));break;case !b.data&&!b.ajax:if(!a||a==-1){this.get_container().children("ul").empty().append(this.data.html_data.original_container_html).find("li, a").filter(function(){return!this.firstChild||
!this.firstChild.tagName||this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");this.clean_node()}d&&d.call(this);break;case !!b.data&&!b.ajax||!!b.data&&!!b.ajax&&(!a||a===-1):if(!a||a==-1){e=c(b.data);e.is("ul")||(e=c("<ul />").append(e));this.get_container().children("ul").empty().append(e.children()).find("li, a").filter(function(){return!this.firstChild||!this.firstChild.tagName||
this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");this.clean_node()}d&&d.call(this);break;case !b.data&&!!b.ajax||!!b.data&&!!b.ajax&&a&&a!==-1:a=this._get_node(a);g=function(h,j,l){var i=this.get_settings().html_data.ajax.error;i&&i.call(this,h,j,l);if(a!=-1&&a.length){a.children("a.jstree-loading").removeClass("jstree-loading");a.removeData("jstree-is-loading");j==="success"&&
b.correct_state&&this.correct_state(a)}else j==="success"&&b.correct_state&&this.get_container().children("ul").empty();f&&f.call(this)};e=function(h,j,l){var i=this.get_settings().html_data.ajax.success;if(i)h=i.call(this,h,j,l)||h;if(h===""||h&&h.toString&&h.toString().replace(/^[\s\n]+$/,"")==="")return g.call(this,l,j,"");if(h){h=c(h);h.is("ul")||(h=c("<ul />").append(h));if(a==-1||!a)this.get_container().children("ul").empty().append(h.children()).find("li, a").filter(function(){return!this.firstChild||
!this.firstChild.tagName||this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");else{a.children("a.jstree-loading").removeClass("jstree-loading");a.append(h).children("ul").find("li, a").filter(function(){return!this.firstChild||!this.firstChild.tagName||this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");
a.removeData("jstree-is-loading")}this.clean_node(a);d&&d.call(this)}else if(a&&a!==-1){a.children("a.jstree-loading").removeClass("jstree-loading");a.removeData("jstree-is-loading");if(b.correct_state){this.correct_state(a);d&&d.call(this)}}else if(b.correct_state){this.get_container().children("ul").empty();d&&d.call(this)}};b.ajax.context=this;b.ajax.error=g;b.ajax.success=e;if(!b.ajax.dataType)b.ajax.dataType="html";if(c.isFunction(b.ajax.url))b.ajax.url=b.ajax.url.call(this,a);if(c.isFunction(b.ajax.data))b.ajax.data=
b.ajax.data.call(this,a);c.ajax(b.ajax)}}}});c.jstree.defaults.plugins.push("html_data")})(jQuery);(function(c){c.jstree.plugin("themeroller",{__init:function(){var a=this._get_settings().themeroller;this.get_container().addClass("ui-widget-content").addClass("jstree-themeroller").delegate("a","mouseenter.jstree",function(d){c(d.currentTarget).hasClass("jstree-loading")||c(this).addClass(a.item_h)}).delegate("a","mouseleave.jstree",function(){c(this).removeClass(a.item_h)}).bind("init.jstree",c.proxy(function(d,
f){f.inst.get_container().find("> ul > li > .jstree-loading > ins").addClass("ui-icon-refresh");this._themeroller(f.inst.get_container().find("> ul > li"))},this)).bind("open_node.jstree create_node.jstree",c.proxy(function(d,f){this._themeroller(f.rslt.obj)},this)).bind("loaded.jstree refresh.jstree",c.proxy(function(){this._themeroller()},this)).bind("close_node.jstree",c.proxy(function(d,f){this._themeroller(f.rslt.obj)},this)).bind("delete_node.jstree",c.proxy(function(d,f){this._themeroller(f.rslt.parent)},
this)).bind("correct_state.jstree",c.proxy(function(d,f){f.rslt.obj.children("ins.jstree-icon").removeClass(a.opened+" "+a.closed+" ui-icon").end().find("> a > ins.ui-icon").filter(function(){return this.className.toString().replace(a.item_clsd,"").replace(a.item_open,"").replace(a.item_leaf,"").indexOf("ui-icon-")===-1}).removeClass(a.item_open+" "+a.item_clsd).addClass(a.item_leaf||"jstree-no-icon")},this)).bind("select_node.jstree",c.proxy(function(d,f){f.rslt.obj.children("a").addClass(a.item_a)},
this)).bind("deselect_node.jstree deselect_all.jstree",c.proxy(function(){this.get_container().find("a."+a.item_a).removeClass(a.item_a).end().find("a.jstree-clicked").addClass(a.item_a)},this)).bind("dehover_node.jstree",c.proxy(function(d,f){f.rslt.obj.children("a").removeClass(a.item_h)},this)).bind("hover_node.jstree",c.proxy(function(d,f){this.get_container().find("a."+a.item_h).not(f.rslt.obj).removeClass(a.item_h);f.rslt.obj.children("a").addClass(a.item_h)},this)).bind("move_node.jstree",
c.proxy(function(d,f){this._themeroller(f.rslt.o);this._themeroller(f.rslt.op)},this))},__destroy:function(){var a=this._get_settings().themeroller,d=["ui-icon"];c.each(a,function(f,e){e=e.split(" ");if(e.length)d=d.concat(e)});this.get_container().removeClass("ui-widget-content").find("."+d.join(", .")).removeClass(d.join(" "))},_fn:{_themeroller:function(a){var d=this._get_settings().themeroller;a=!a||a==-1?this.get_container_ul():this._get_node(a).parent();a.find("li.jstree-closed").children("ins.jstree-icon").removeClass(d.opened).addClass("ui-icon "+
d.closed).end().children("a").addClass(d.item).children("ins.jstree-icon").addClass("ui-icon").filter(function(){return this.className.toString().replace(d.item_clsd,"").replace(d.item_open,"").replace(d.item_leaf,"").indexOf("ui-icon-")===-1}).removeClass(d.item_leaf+" "+d.item_open).addClass(d.item_clsd||"jstree-no-icon").end().end().end().end().find("li.jstree-open").children("ins.jstree-icon").removeClass(d.closed).addClass("ui-icon "+d.opened).end().children("a").addClass(d.item).children("ins.jstree-icon").addClass("ui-icon").filter(function(){return this.className.toString().replace(d.item_clsd,
"").replace(d.item_open,"").replace(d.item_leaf,"").indexOf("ui-icon-")===-1}).removeClass(d.item_leaf+" "+d.item_clsd).addClass(d.item_open||"jstree-no-icon").end().end().end().end().find("li.jstree-leaf").children("ins.jstree-icon").removeClass(d.closed+" ui-icon "+d.opened).end().children("a").addClass(d.item).children("ins.jstree-icon").addClass("ui-icon").filter(function(){return this.className.toString().replace(d.item_clsd,"").replace(d.item_open,"").replace(d.item_leaf,"").indexOf("ui-icon-")===
-1}).removeClass(d.item_clsd+" "+d.item_open).addClass(d.item_leaf||"jstree-no-icon")}},defaults:{opened:"ui-icon-triangle-1-se",closed:"ui-icon-triangle-1-e",item:"ui-state-default",item_h:"ui-state-hover",item_a:"ui-state-active",item_open:"ui-icon-folder-open",item_clsd:"ui-icon-folder-collapsed",item_leaf:"ui-icon-document"}});c(function(){c.vakata.css.add_sheet({str:".jstree-themeroller .ui-icon { overflow:visible; } .jstree-themeroller a { padding:0 2px; } .jstree-themeroller .jstree-no-icon { display:none; }",
title:"jstree"})})})(jQuery);(function(c){c.jstree.plugin("unique",{__init:function(){this.get_container().bind("before.jstree",c.proxy(function(a,d){var f=[],e=true,b;if(d.func=="move_node")if(d.args[4]===true)if(d.args[0].o&&d.args[0].o.length){d.args[0].o.children("a").each(function(){f.push(c(this).text().replace(/^\s+/g,""))});e=this._check_unique(f,d.args[0].np.find("> ul > li").not(d.args[0].o),"move_node")}if(d.func=="create_node")if(d.args[4]||this._is_loaded(d.args[0])){e=this._get_node(d.args[0]);
if(d.args[1]&&(d.args[1]==="before"||d.args[1]==="after")){e=this._get_parent(d.args[0]);if(!e||e===-1)e=this.get_container()}if(typeof d.args[2]==="string")f.push(d.args[2]);else!d.args[2]||!d.args[2].data?f.push(this._get_string("new_node")):f.push(d.args[2].data);e=this._check_unique(f,e.find("> ul > li"),"create_node")}if(d.func=="rename_node"){f.push(d.args[1]);b=this._get_node(d.args[0]);e=this._get_parent(b);if(!e||e===-1)e=this.get_container();e=this._check_unique(f,e.find("> ul > li").not(b),
...
"").split(",").length){this._get_settings().unique.error_callback.call(null,a,d,f);return false}return true},check_move:function(){if(!this.__call_old())return false;var a=this._get_move(),d=[];if(a.o&&a.o.length){a.o.children("a").each(function(){d.push(c(this).text().replace(/^\s+/g,""))});return this._check_unique(d,a.np.find("> ul > li").not(a.o),"check_move")}return true}}})})(jQuery);(function(c){c.jstree.plugin("radio",{__init:function(){this.data.radio.noui=this._get_settings().radio.override_ui;
if(this.data.ui&&this.data.radio.noui){this.select_node=this.deselect_node=this.deselect_all=c.noop;this.get_selected=this.get_checked}this.get_container().bind("open_node.jstree create_node.jstree clean_node.jstree refresh.jstree",c.proxy(function(a,d){this._prepare_radios(d.rslt.obj)},this)).bind("loaded.jstree",c.proxy(function(){this._prepare_radios()},this)).delegate(this.data.ui&&this.data.radio.noui?"a":"ins.jstree-radio","click.jstree",c.proxy(function(a){a.preventDefault();this._get_node(a.target).hasClass("jstree-checked")?
this.uncheck_node(a.target):this.check_node(a.target);if(this.data.ui&&this.data.radio.noui){this.save_selected();this.data.cookies&&this.save_cookie("select_node")}else{a.stopImmediatePropagation();return false}},this))},defaults:{override_ui:false,two_state:false,real_radios:false,checked_parent_open:true,real_radios_names:function(a){return["check_"+(a[0].id||Math.ceil(Math.random()*1E4)),1]}},__destroy:function(){this.get_container().find("input.jstree-real-radio").removeClass("jstree-real-radio").end().find("ins.jstree-radio").remove()},
_fn:{_radio_notify:function(a,d){d.checked&&this.check_node(a,false)},_prepare_radios:function(a){a=!a||a==-1?this.get_container().find("> ul > li"):this._get_node(a);if(a!==false){var d,f=this,e,b=this._get_settings().radio.two_state,g=this._get_settings().radio.real_radios,h=this._get_settings().radio.real_radios_names;a.each(function(){e=c(this);d=e.is("li")&&(e.hasClass("jstree-checked")||g&&e.children(":checked").length)?"jstree-checked":"jstree-unchecked";e.find("li").andSelf().each(function(){var j=
c(this),l;j.children("a"+(f.data.languages?"":":eq(0)")).not(":has(.jstree-radio)").prepend("<ins class='jstree-radio'>&#160;</ins>").parent().not(".jstree-checked, .jstree-unchecked").addClass(b?"jstree-unchecked":d);if(g){if(j.children(":radio").length)j.children(":radio").addClass("jstree-real-radio");else{l=h.call(f,j);j.prepend("<input type='radio' class='jstree-real-radio' id='"+l[0]+"' name='"+l[0]+"' value='"+l[1]+"' />")}d==="jstree-checked"&&j.children(":radio").attr("checked","checked")}d===
"jstree-checked"&&!b&&j.find("li").addClass("jstree-checked")})});if(!b){a.length===1&&a.is("li")&&this._repair_state(a);a.is("li")?a.each(function(){f._repair_state(this)}):a.find("> ul > li").each(function(){f._repair_state(this)});a.find(".jstree-checked").parent().parent().each(function(){f._repair_state(this)})}}},change_state:function(a,d){a=this._get_node(a);var f=false,e=this._get_settings().radio.real_radios;if(!a||a===-1)return false;d=d===false||d===true?d:a.hasClass("jstree-checked");
if(this._get_settings().radio.two_state)if(d){a.removeClass("jstree-checked").addClass("jstree-unchecked");e&&a.children(":radio").removeAttr("checked")}else{a.removeClass("jstree-unchecked").addClass("jstree-checked");e&&a.children(":radio").attr("checked","checked")}else{if(d){f=a.find("li").andSelf();if(!f.filter(".jstree-checked, .jstree-undetermined").length)return false;f.removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked");e&&f.children(":radio").removeAttr("checked")}else{f=
a.find("li").andSelf();if(!f.filter(".jstree-unchecked, .jstree-undetermined").length)return false;f.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");e&&f.children(":radio").attr("checked","checked");if(this.data.ui)this.data.ui.last_selected=a;this.data.radio.last_selected=a}a.parentsUntil(".jstree","li").each(function(){var b=c(this);if(d)if(b.children("ul").children("li.jstree-checked, li.jstree-undetermined").length){b.parentsUntil(".jstree","li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
...
return false}else{b.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");e&&b.children(":radio").attr("checked","checked")}})}if(this.data.ui&&this.data.radio.noui)this.data.ui.selected=this.get_checked();this.__callback(a);return true},check_node:function(a){if(this.change_state(a,false)){a=this._get_node(a);if(this._get_settings().radio.checked_parent_open){var d=this;a.parents(".jstree-closed").each(function(){d.open_node(this,false,true)})}this.__callback({obj:a})}},
uncheck_node:function(a){this.change_state(a,true)&&this.__callback({obj:this._get_node(a)})},check_all:function(){var a=this;(this._get_settings().radio.two_state?this.get_container_ul().find("li"):this.get_container_ul().children("li")).each(function(){a.change_state(this,false)});this.__callback()},uncheck_all:function(){var a=this;(this._get_settings().radio.two_state?this.get_container_ul().find("li"):this.get_container_ul().children("li")).each(function(){a.change_state(this,true)});this.__callback()},
...
show_radios:function(){this.get_container().children("ul").removeClass("jstree-no-radios")},hide_radios:function(){this.get_container().children("ul").addClass("jstree-no-radios")},_repair_state:function(a){a=this._get_node(a);if(a.length){var d=this._get_settings().radio.real_radios,f=a.find("> ul > .jstree-checked").length,e=a.find("> ul > .jstree-undetermined").length,b=a.find("> ul > li").length;if(b===0)a.hasClass("jstree-undetermined")&&this.change_state(a,false);else if(f===0&&e===0)this.change_state(a,
true);else if(f===b)this.change_state(a,false);else{a.parentsUntil(".jstree","li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");d&&a.parentsUntil(".jstree","li").andSelf().children(":radio").removeAttr("checked")}}},reselect:function(){if(this.data.ui&&this.data.radio.noui){var a=this,d=this.data.ui.to_select;d=c.map(c.makeArray(d),function(f){return"#"+f.toString().replace(/^#/,"").replace(/\\\//g,"/").replace(/\//g,"\\/").replace(/\\\./g,".").replace(/\./g,
"\\.")});this.deselect_all();c.each(d,function(f,e){a.check_node(e)});this.__callback()}else this.__call_old()},save_loaded:function(){var a=this;this.data.core.to_load=[];this.get_container_ul().find("li.jstree-closed.jstree-undetermined").each(function(){this.id&&a.data.core.to_load.push("#"+this.id)})}}});c(function(){c.vakata.css.add_sheet({str:".jstree .jstree-real-radio { display:none; } ",title:"jstree"})})})(jQuery);(function(c){c.jstree.plugin("wholerow",{__init:function(){if(!this.data.ui)throw"jsTree wholerow: jsTree UI plugin not included.";
this.data.wholerow.html=false;this.data.wholerow.to=false;this.get_container().bind("init.jstree",c.proxy(function(){this._get_settings().core.animation=0},this)).bind("open_node.jstree create_node.jstree clean_node.jstree loaded.jstree",c.proxy(function(a,d){this._prepare_wholerow_span(d&&d.rslt&&d.rslt.obj?d.rslt.obj:-1)},this)).bind("search.jstree clear_search.jstree reopen.jstree after_open.jstree after_close.jstree create_node.jstree delete_node.jstree clean_node.jstree",c.proxy(function(a,d){this.data.to&&
clearTimeout(this.data.to);this.data.to=setTimeout(function(f,e){return function(){f._prepare_wholerow_ul(e)}}(this,d&&d.rslt&&d.rslt.obj?d.rslt.obj:-1),0)},this)).bind("deselect_all.jstree",c.proxy(function(){this.get_container().find(" > .jstree-wholerow .jstree-clicked").removeClass("jstree-clicked "+(this.data.themeroller?this._get_settings().themeroller.item_a:""))},this)).bind("select_node.jstree deselect_node.jstree ",c.proxy(function(a,d){d.rslt.obj.each(function(){d.inst.get_container().find(" > .jstree-wholerow li:visible:eq("+
