...
this.hide_icons()},this))},defaults:{theme:"default",url:false,dots:true,icons:true},_fn:{set_theme:function(d,f){if(!d)return false;f||(f=b.jstree._themes+d+"/style.css");if(b.inArray(f,a)==-1){b.vakata.css.add_sheet({url:f});a.push(f)}if(this.data.themes.theme!=d){this.get_container().removeClass("jstree-"+this.data.themes.theme);this.data.themes.theme=d}this.get_container().addClass("jstree-"+d);this.data.themes.dots?this.show_dots():this.hide_dots();this.data.themes.icons?this.show_icons():this.hide_icons();
...
(function(b){var a=[];b.jstree.plugin("hotkeys",{__init:function(){if(typeof b.hotkeys==="undefined")throw"jsTree hotkeys: jQuery hotkeys plugin not included.";if(!this.data.ui)throw"jsTree hotkeys: jsTree UI plugin not included.";b.each(this._get_settings().hotkeys,function(d,f){if(f!==false&&b.inArray(d,a)==-1){b(document).bind("keydown",d,function(e){var c;var h=b.jstree._focused(),g;if(h&&h.data&&h.data.hotkeys&&h.data.hotkeys.enabled)if(g=h._get_settings().hotkeys[d])c=g.call(h,e);return c});
...
_fn:{load_node:function(a,d,f){var e=this;this.load_node_json(a,function(){e.__callback({obj:e._get_node(a)});d.call(this)},f)},_is_loaded:function(a){var d=this._get_settings().json_data;a=this._get_node(a);return a==-1||!a||!d.ajax&&!d.progressive_render&&!b.isFunction(d.data)||a.is(".jstree-open, .jstree-leaf")||a.children("ul").children("li").length>0},refresh:function(a){a=this._get_node(a);var d=this._get_settings().json_data;if(a&&a!==-1&&d.progressive_unload&&(b.isFunction(d.data)||d.ajax))b.removeData(a,
"jstree-children");return this.__call_old()},load_node_json:function(a,d,f){var e=this.get_settings().json_data,c,h=function(){};c=function(){};if((a=this._get_node(a))&&a!==-1&&(e.progressive_render||e.progressive_unload)&&!a.is(".jstree-open, .jstree-leaf")&&a.children("ul").children("li").length===0&&a.data("jstree-children")){if(c=this._parse_json(a.data("jstree-children"),a)){a.append(c);e.progressive_unload||b.removeData(a,"jstree-children")}this.clean_node(a);d&&d.call(this)}else{if(a&&a!==
-1)if(a.data("jstree-is-loading"))return;else a.data("jstree-is-loading",true);switch(true){case !e.data&&!e.ajax:throw"Neither data nor ajax settings supplied.";case b.isFunction(e.data):e.data.call(this,a,b.proxy(function(g){if(g=this._parse_json(g,a)){if(a===-1||!a)this.get_container().children("ul").empty().append(g.children());else{a.append(g).children("a.jstree-loading").removeClass("jstree-loading");b.removeData(a,"jstree-is-loading")}this.clean_node(a);d&&d.call(this)}else{if(a===-1||!a)e.correct_state&&
this.get_container().children("ul").empty();else{a.children("a.jstree-loading").removeClass("jstree-loading");b.removeData(a,"jstree-is-loading");e.correct_state&&this.correct_state(a)}f&&f.call(this)}},this));break;case !!e.data&&!e.ajax||!!e.data&&!!e.ajax&&(!a||a===-1):if(!a||a==-1)if(c=this._parse_json(e.data,a)){this.get_container().children("ul").empty().append(c.children());this.clean_node()}else e.correct_state&&this.get_container().children("ul").empty();d&&d.call(this);break;case !e.data&&
!!e.ajax||!!e.data&&!!e.ajax&&a&&a!==-1:h=function(g,j,l){var i=this.get_settings().json_data.ajax.error;i&&i.call(this,g,j,l);if(a!=-1&&a.length){a.children("a.jstree-loading").removeClass("jstree-loading");b.removeData(a,"jstree-is-loading");j==="success"&&e.correct_state&&this.correct_state(a)}else j==="success"&&e.correct_state&&this.get_container().children("ul").empty();f&&f.call(this)};c=function(g,j,l){var i=this.get_settings().json_data.ajax.success;if(i)g=i.call(this,g,j,l)||g;if(g===""||
g&&g.toString&&g.toString().replace(/^[\s\n]+$/,"")===""||!b.isArray(g)&&!b.isPlainObject(g))return h.call(this,l,j,"");if(g=this._parse_json(g,a)){if(a===-1||!a)this.get_container().children("ul").empty().append(g.children());else{a.append(g).children("a.jstree-loading").removeClass("jstree-loading");b.removeData(a,"jstree-is-loading")}this.clean_node(a);d&&d.call(this)}else if(a===-1||!a){if(e.correct_state){this.get_container().children("ul").empty();d&&d.call(this)}}else{a.children("a.jstree-loading").removeClass("jstree-loading");
b.removeData(a,"jstree-is-loading");if(e.correct_state){this.correct_state(a);d&&d.call(this)}}};e.ajax.context=this;e.ajax.error=h;e.ajax.success=c;if(!e.ajax.dataType)e.ajax.dataType="json";if(b.isFunction(e.ajax.url))e.ajax.url=e.ajax.url.call(this,a);if(b.isFunction(e.ajax.data))e.ajax.data=e.ajax.data.call(this,a);b.ajax(e.ajax)}}},_parse_json:function(a,d,f){var e=false,c=this._get_settings(),h=c.json_data,g=c.core.html_titles,j;if(!a)return e;h.progressive_unload&&d&&d!==-1&&d.data("jstree-children",
e);if(b.isArray(a)){e=b();if(!a.length)return false;c=0;for(h=a.length;c<h;c++){j=this._parse_json(a[c],d,true);if(j.length)e=e.add(j)}}else{if(typeof a=="string")a={data:a};if(!a.data&&a.data!=="")return e;e=b("<li />");a.attr&&e.attr(a.attr);a.metadata&&e.data(a.metadata);a.state&&e.addClass("jstree-"+a.state);if(!b.isArray(a.data)){j=a.data;a.data=[];a.data.push(j)}b.each(a.data,function(l,i){j=b("<a />");if(b.isFunction(i))i=i.call(this,a);if(typeof i=="string")j.attr("href","#")[g?"html":"text"](i);
else{if(!i.attr)i.attr={};if(!i.attr.href)i.attr.href="#";j.attr(i.attr)[g?"html":"text"](i.title);i.language&&j.addClass(i.language)}j.prepend("<ins class='jstree-icon'>&#160;</ins>");if(!i.icon&&a.icon)i.icon=a.icon;if(i.icon)i.icon.indexOf("/")===-1?j.children("ins").addClass(i.icon):j.children("ins").css("background","url('"+i.icon+"') center center no-repeat");e.append(j)});e.prepend("<ins class='jstree-icon'>&#160;</ins>");if(a.children)if(h.progressive_render&&a.state!=="open")e.addClass("jstree-closed").data("jstree-children",
a.children);else{h.progressive_unload&&e.data("jstree-children",a.children);if(b.isArray(a.children)&&a.children.length){j=this._parse_json(a.children,d,true);if(j.length){d=b("<ul />");d.append(j);e.append(d)}}}}if(!f){f=b("<ul />");f.append(e);e=f}return e},get_json:function(a,d,f,e){var c=[],h=this._get_settings(),g=this,j,l,i,k,m,o;a=this._get_node(a);if(!a||a===-1)a=this.get_container().find("> ul > li");d=b.isArray(d)?d:["id","class"];!e&&this.data.types&&d.push(h.types.type_attr);f=b.isArray(f)?
f:[];a.each(function(){i=b(this);j={data:[]};if(d.length)j.attr={};b.each(d,function(n,p){if((l=i.attr(p))&&l.length&&l.replace(/jstree[^ ]*/ig,"").length)j.attr[p]=(" "+l).replace(/ jstree[^ ]*/ig,"").replace(/\s+$/ig," ").replace(/^ /,"").replace(/ $/,"")});if(i.hasClass("jstree-open"))j.state="open";if(i.hasClass("jstree-closed"))j.state="closed";if(i.data())j.metadata=i.data();k=i.children("a");k.each(function(){m=b(this);if(f.length||b.inArray("languages",h.plugins)!==-1||m.children("ins").get(0).style.backgroundImage.length||
m.children("ins").get(0).className&&m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").length){o=false;b.inArray("languages",h.plugins)!==-1&&b.isArray(h.languages)&&h.languages.length&&b.each(h.languages,function(n,p){if(m.hasClass(p)){o=p;return false}});l={attr:{},title:g.get_text(m,o)};b.each(f,function(n,p){l.attr[p]=(" "+(m.attr(p)||"")).replace(/ jstree[^ ]*/ig,"").replace(/\s+$/ig," ").replace(/^ /,"").replace(/ $/,"")});b.inArray("languages",h.plugins)!==-1&&b.isArray(h.languages)&&
h.languages.length&&b.each(h.languages,function(n,p){if(m.hasClass(p)){l.language=p;return true}});if(m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").replace(/^\s+$/ig,"").length)l.icon=m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").replace(/\s+$/ig," ").replace(/^ /,"").replace(/ $/,"");if(m.children("ins").get(0).style.backgroundImage.length)l.icon=m.children("ins").get(0).style.backgroundImage.replace("url(","").replace(")","")}else l=g.get_text(m);if(k.length>
1)j.data.push(l);else j.data=l});i=i.find("> ul > li");if(i.length)j.children=g.get_json(i,d,f,true);c.push(j)});return c}}})})(jQuery);(function(b){b.jstree.plugin("languages",{__init:function(){this._load_css()},defaults:[],_fn:{set_lang:function(a){var d=this._get_settings().languages,f=false,e=".jstree-"+this.get_index()+" a";if(!b.isArray(d)||d.length===0)return false;if(b.inArray(a,d)==-1)if(d[a])a=d[a];else return false;if(a==this.data.languages.current_language)return true;f=b.vakata.css.get_css(e+
"."+this.data.languages.current_language,false,this.data.languages.language_css);if(f!==false)f.style.display="none";f=b.vakata.css.get_css(e+"."+a,false,this.data.languages.language_css);if(f!==false)f.style.display="";this.data.languages.current_language=a;this.__callback(a);return true},get_lang:function(){return this.data.languages.current_language},_get_string:function(a,d){var f=this._get_settings().languages,e=this._get_settings().core.strings;if(b.isArray(f)&&f.length)d=d&&b.inArray(d,f)!=
...
3})[0];return a.nodeValue}},set_text:function(a,d,f){a=this._get_node(a)||this.data.ui.last_selected;if(!a.size())return false;var e=this._get_settings().languages,c=this._get_settings().core.html_titles;if(b.isArray(e)&&e.length){f=f&&b.inArray(f,e)!=-1?f:this.data.languages.current_language;a=a.children("a."+f)}else a=a.children("a:eq(0)");if(c){e=a.children("INS").clone();a.html(d).prepend(e);this.__callback({obj:a,name:d,lang:f});return true}else{a=a.contents().filter(function(){return this.nodeType==
...
a,d,f,function(c){var h=this._get_settings().languages,g=c.children("a"),j;if(b.isArray(h)&&h.length){for(j=0;j<h.length;j++)g.is("."+h[j])||c.append(g.eq(0).clone().removeClass(h.join(" ")).addClass(h[j]));g.not("."+h.join(", .")).remove()}e&&e.call(this,c)})}}})})(jQuery);(function(b){b.jstree.plugin("cookies",{__init:function(){if(typeof b.cookie==="undefined")throw"jsTree cookie: jQuery cookie plugin not included.";var a=this._get_settings().cookies,d;if(a.save_loaded)if((d=b.cookie(a.save_loaded))&&
d.length)this.data.core.to_load=d.split(",");if(a.save_opened)if((d=b.cookie(a.save_opened))&&d.length)this.data.core.to_open=d.split(",");if(a.save_selected)if((d=b.cookie(a.save_selected))&&d.length&&this.data.ui)this.data.ui.to_select=d.split(",");this.get_container().one((this.data.ui?"reselect":"reopen")+".jstree",b.proxy(function(){this.get_container().bind("open_node.jstree close_node.jstree select_node.jstree deselect_node.jstree",b.proxy(function(f){this._get_settings().cookies.auto_save&&
this.save_cookie((f.handleObj.namespace+f.handleObj.type).replace("jstree",""))},this))},this))},defaults:{save_loaded:"jstree_load",save_opened:"jstree_open",save_selected:"jstree_select",auto_save:true,cookie_options:{}},_fn:{save_cookie:function(a){if(!this.data.core.refreshing){var d=this._get_settings().cookies;if(a)switch(a){case "open_node":case "close_node":if(d.save_opened){this.save_opened();b.cookie(d.save_opened,this.data.core.to_open.join(","),d.cookie_options)}if(d.save_loaded){this.save_loaded();
b.cookie(d.save_loaded,this.data.core.to_load.join(","),d.cookie_options)}break;case "select_node":case "deselect_node":if(d.save_selected&&this.data.ui){this.save_selected();b.cookie(d.save_selected,this.data.ui.to_select.join(","),d.cookie_options)}}else{if(d.save_loaded){this.save_loaded();b.cookie(d.save_loaded,this.data.core.to_load.join(","),d.cookie_options)}if(d.save_opened){this.save_opened();b.cookie(d.save_opened,this.data.core.to_open.join(","),d.cookie_options)}if(d.save_selected&&this.data.ui){this.save_selected();
...
-1?this.get_container():d.rslt.np).children("ul"))},this))},defaults:function(a,d){return this.get_text(a)>this.get_text(d)?1:-1},_fn:{sort:function(a){var d=this._get_settings().sort,f=this;a.append(b.makeArray(a.children("li")).sort(b.proxy(d,f)));a.find("> li > ul").each(function(){f.sort(b(this))});this.clean_node(a)}}})})(jQuery);(function(b){var a=false,d=false,f=false,e=false,c=false,h=false,g=false,j=false,l=false;b.vakata.dnd={is_down:false,is_drag:false,helper:false,scroll_spd:10,init_x:0,
init_y:0,threshold:5,helper_left:5,helper_top:10,user_data:{},drag_start:function(i,k,m){b.vakata.dnd.is_drag&&b.vakata.drag_stop({});try{i.currentTarget.unselectable="on";i.currentTarget.onselectstart=function(){return false};if(i.currentTarget.style)i.currentTarget.style.MozUserSelect="none"}catch(o){}b.vakata.dnd.init_x=i.pageX;b.vakata.dnd.init_y=i.pageY;b.vakata.dnd.user_data=k;b.vakata.dnd.is_down=true;b.vakata.dnd.helper=b("<div id='vakata-dragged' />").html(m);b(document).bind("mousemove",
b.vakata.dnd.drag);b(document).bind("mouseup",b.vakata.dnd.drag_stop);return false},drag:function(i){if(b.vakata.dnd.is_down){if(!b.vakata.dnd.is_drag)if(Math.abs(i.pageX-b.vakata.dnd.init_x)>5||Math.abs(i.pageY-b.vakata.dnd.init_y)>5){b.vakata.dnd.helper.appendTo("body");b.vakata.dnd.is_drag=true;b(document).triggerHandler("drag_start.vakata",{event:i,data:b.vakata.dnd.user_data})}else return;if(i.type==="mousemove"){var k=b(document),m=k.scrollTop();k=k.scrollLeft();if(i.pageY-m<20){if(h&&g==="down"){clearInterval(h);
h=false}if(!h){g="up";h=setInterval(function(){b(document).scrollTop(b(document).scrollTop()-b.vakata.dnd.scroll_spd)},150)}}else if(h&&g==="up"){clearInterval(h);h=false}if(b(window).height()-(i.pageY-m)<20){if(h&&g==="up"){clearInterval(h);h=false}if(!h){g="down";h=setInterval(function(){b(document).scrollTop(b(document).scrollTop()+b.vakata.dnd.scroll_spd)},150)}}else if(h&&g==="down"){clearInterval(h);h=false}if(i.pageX-k<20){if(c&&j==="right"){clearInterval(c);c=false}if(!c){j="left";c=setInterval(function(){b(document).scrollLeft(b(document).scrollLeft()-
b.vakata.dnd.scroll_spd)},150)}}else if(c&&j==="left"){clearInterval(c);c=false}if(b(window).width()-(i.pageX-k)<20){if(c&&j==="left"){clearInterval(c);c=false}if(!c){j="right";c=setInterval(function(){b(document).scrollLeft(b(document).scrollLeft()+b.vakata.dnd.scroll_spd)},150)}}else if(c&&j==="right"){clearInterval(c);c=false}}b.vakata.dnd.helper.css({left:i.pageX+b.vakata.dnd.helper_left+"px",top:i.pageY+b.vakata.dnd.helper_top+"px"});b(document).triggerHandler("drag.vakata",{event:i,data:b.vakata.dnd.user_data})}},
drag_stop:function(i){c&&clearInterval(c);h&&clearInterval(h);b(document).unbind("mousemove",b.vakata.dnd.drag);b(document).unbind("mouseup",b.vakata.dnd.drag_stop);b(document).triggerHandler("drag_stop.vakata",{event:i,data:b.vakata.dnd.user_data});b.vakata.dnd.helper.remove();b.vakata.dnd.init_x=0;b.vakata.dnd.init_y=0;b.vakata.dnd.user_data={};b.vakata.dnd.is_down=false;b.vakata.dnd.is_drag=false}};b(function(){b.vakata.css.add_sheet({str:"#vakata-dragged { display:block; margin:0 0 0 0; padding:4px 4px 4px 24px; position:absolute; top:-2000px; line-height:16px; z-index:10000; } ",
title:"vakata"})});b.jstree.plugin("dnd",{__init:function(){this.data.dnd={active:false,after:false,inside:false,before:false,off:false,prepared:false,w:0,to1:false,to2:false,cof:false,cw:false,ch:false,i1:false,i2:false,mto:false};this.get_container().bind("mouseenter.jstree",b.proxy(function(k){if(b.vakata.dnd.is_drag&&b.vakata.dnd.user_data.jstree){if(this.data.themes){f.attr("class","jstree-"+this.data.themes.theme);e&&e.attr("class","jstree-"+this.data.themes.theme);b.vakata.dnd.helper.attr("class",
"jstree-dnd-helper jstree-"+this.data.themes.theme)}if(k.currentTarget===k.target&&b.vakata.dnd.user_data.obj&&b(b.vakata.dnd.user_data.obj).length&&b(b.vakata.dnd.user_data.obj).parents(".jstree:eq(0)")[0]!==k.target){k=b.jstree._reference(k.target);if(k.data.dnd.foreign){k=k._get_settings().dnd.drag_check.call(this,{o:a,r:k.get_container(),is_root:true});if(k===true||k.inside===true||k.before===true||k.after===true)b.vakata.dnd.helper.children("ins").attr("class","jstree-ok")}else{k.prepare_move(a,
k.get_container(),"last");k.check_move()&&b.vakata.dnd.helper.children("ins").attr("class","jstree-ok")}}}},this)).bind("mouseup.jstree",b.proxy(function(k){if(b.vakata.dnd.is_drag&&b.vakata.dnd.user_data.jstree&&k.currentTarget===k.target&&b.vakata.dnd.user_data.obj&&b(b.vakata.dnd.user_data.obj).length&&b(b.vakata.dnd.user_data.obj).parents(".jstree:eq(0)")[0]!==k.target){var m=b.jstree._reference(k.currentTarget);if(m.data.dnd.foreign){k=m._get_settings().dnd.drag_check.call(this,{o:a,r:m.get_container(),
