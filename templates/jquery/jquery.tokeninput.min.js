...
b.isFunction(c)&&c.call(i,d)}function u(f,d){var c=b.map(f,function(b){return b[a.tokenValue]});d.val(c.join(a.tokenDelimiter))}function q(){t.hide().empty();o=null}function z(){t.css({position:"absolute",top:b(s).offset().top+b(s).outerHeight(),left:b(s).offset().left,"z-index":999}).show()}function D(f,d){if(d&&d.length){t.empty();var c=b("<ul>").appendTo(t).mouseover(function(a){E(b(a.target).closest("li"))}).mousedown(function(a){C(b(a.target).closest("li").data("tokeninput"));i.change();return!1}).hide();
