var a;
(function(c){c.widget("ui.weekCalendar",{options:{date:new Date,timeFormat:"h:i a",dateFormat:"M d, Y",alwaysDisplayTimeMinutes:true,use24Hour:false,daysToShow:7,firstDayOfWeek:0,useShortDayNames:false,timeSeparator:" to ",startParam:"start",endParam:"end",businessHours:{start:8,end:18,limitDisplay:false},newEventText:"New Event",timeslotHeight:20,defaultEventLength:2,timeslotsPerHour:4,buttons:true,buttonText:{today:"today",lastWeek:"&nbsp;&lt;&nbsp;",nextWeek:"&nbsp;&gt;&nbsp;"},scrollToHourMillis:500,allowCalEventOverlap:false,
overlapEventsSeparate:false,readonly:false,draggable:function(){return true},resizable:function(){return true},eventClick:function(){},eventRender:function(b,i){return i},eventAfterRender:function(b,i){return i},eventDrag:function(){},eventDrop:function(){},eventResize:function(){},eventNew:function(){},eventMouseover:function(){},eventMouseout:function(){},calendarBeforeLoad:function(){},calendarAfterLoad:function(){},noEvents:function(){},shortMonths:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug",
"Sep","Oct","Nov","Dec"],longMonths:["January","February","March","April","May","June","July","August","September","October","November","December"],shortDays:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],longDays:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"]},_create:function(){var b=this;b._computeOptions();b._setupEventDelegation();b._renderCalendar();b._loadCalEvents();b._resizeCalendar();b._scrollToHour(b.options.date.getHours());c(window).unbind("resize.weekcalendar");
c(window).bind("resize.weekcalendar",function(){b._resizeCalendar()})},refresh:function(){this._loadCalEvents(this.element.data("startDate"))},clear:function(){this._clearCalendar()},today:function(){this._clearCalendar();this._loadCalEvents(new Date)},prevWeek:function(){var b=new Date(this.element.data("startDate").getTime()-k/6);this._clearCalendar();this._loadCalEvents(b)},nextWeek:function(){var b=new Date(this.element.data("startDate").getTime()+k+k/7);this._clearCalendar();this._loadCalEvents(b)},
gotoWeek:function(b){this._clearCalendar();this._loadCalEvents(b)},removeEvent:function(b){var i=this;i.element.find(".wc-cal-event").each(function(){if(c(this).data("calEvent").id===b){c(this).remove();return false}});i.element.find(".wc-day-column-inner").each(function(){i._adjustOverlappingEvents(c(this))})},removeUnsavedEvents:function(){var b=this;b.element.find(".wc-new-cal-event").each(function(){c(this).remove()});b.element.find(".wc-day-column-inner").each(function(){b._adjustOverlappingEvents(c(this))})},
...
i):this._formatDate(b,this.options.dateFormat)},formatTime:function(b,i){return i?this._formatDate(b,i):this._formatDate(b,this.options.timeFormat)},_setOption:function(b,i){var e=this;if(e.options[b]!=i){var d=c.map(e.element.find(".wc-cal-event"),function(){return c(this).data("calEvent")}),j={};j[b]=i;e._renderEvents({events:d,options:j},e.element.find(".wc-day-column-inner"))}},_computeOptions:function(){var b=this.options;if(b.businessHours.limitDisplay){b.timeslotsPerDay=b.timeslotsPerHour*
(b.businessHours.end-b.businessHours.start);b.millisToDisplay=(b.businessHours.end-b.businessHours.start)*60*60*1E3;b.millisPerTimeslot=b.millisToDisplay/b.timeslotsPerDay}else{b.timeslotsPerDay=b.timeslotsPerHour*24;b.millisToDisplay=f;b.millisPerTimeslot=f/b.timeslotsPerDay}},_resizeCalendar:function(){var b=this.options;if(b&&c.isFunction(b.height)){b=b.height(this.element);var i=this.element.find(".wc-header").outerHeight(),e=this.element.find(".wc-nav").outerHeight();this.element.find(".wc-scrollable-grid").height(b-
e-i)}},_setupEventDelegation:function(){var b=this,i=this.options;this.element.click(function(e){var d=c(e.target);if(!d.data("preventClick"))if(d.hasClass("wc-cal-event"))i.eventClick(d.data("calEvent"),d,e);else d.parent().hasClass("wc-cal-event")&&i.eventClick(d.parent().data("calEvent"),d.parent(),e)}).mouseover(function(e){var d=c(e.target);b._isDraggingOrResizing(d)||d.hasClass("wc-cal-event")&&i.eventMouseover(d.data("calEvent"),d,e)}).mouseout(function(e){var d=c(e.target);if(!b._isDraggingOrResizing(d))if(d.hasClass("wc-cal-event"))d.data("sizing")||
i.eventMouseout(d.data("calEvent"),d,e)})},_isDraggingOrResizing:function(b){return b.hasClass("ui-draggable-dragging")||b.hasClass("ui-resizable-resizing")},_renderCalendar:function(){var b,i,e,d=this,j=this.options;b=c('<div class="wc-container">').appendTo(d.element);if(j.buttons){i='<div class="wc-nav">                    <button class="wc-today">'+j.buttonText.today+'</button>                    <button class="wc-prev">'+j.buttonText.lastWeek+'</button>                    <button class="wc-next">'+
j.buttonText.nextWeek+"</button>                    </div>";c(i).appendTo(b);b.find(".wc-nav .wc-today").click(function(){d.element.weekCalendar("today");return false});b.find(".wc-nav .wc-prev").click(function(){d.element.weekCalendar("prevWeek");return false});b.find(".wc-nav .wc-next").click(function(){d.element.weekCalendar("nextWeek");return false})}i='<table class="wc-header"><tbody><tr><td class="wc-time-column-header"></td>';for(var g=1;g<=j.daysToShow;g++)i+='<td class="wc-day-column-header wc-day-'+
g+'"></td>';i+='<td class="wc-scrollbar-shim"></td></tr></tbody></table>';e='<div class="wc-scrollable-grid">                <table class="wc-time-slots">                <tbody>                <tr>                <td class="wc-grid-timeslot-header"></td>                <td colspan="'+j.daysToShow+'">                <div class="wc-time-slot-wrapper">                <div class="wc-time-slots">';var l=j.businessHours.limitDisplay?j.businessHours.start:0,m=j.businessHours.limitDisplay?j.businessHours.end:
24;for(g=l;g<m;g++){for(var q=0;q<j.timeslotsPerHour-1;q++)e+='<div class="wc-time-slot"></div>';e+='<div class="wc-time-slot wc-hour-end"></div>'}e+='</div></div></td></tr><tr><td class="wc-grid-timeslot-header">';for(g=l;g<m;g++){e+='<div class="wc-hour-header '+(j.businessHours.start<=g&&j.businessHours.end>g?"wc-business-hours":"")+'">';e+=j.use24Hour?'<div class="wc-time-header-cell">'+d._24HourForIndex(g)+"</div>":'<div class="wc-time-header-cell">'+d._hourForIndex(g)+'<span class="wc-am-pm">'+
d._amOrPm(g)+"</span></div>";e+="</div>"}e+="</td>";for(g=1;g<=j.daysToShow;g++)e+='<td class="wc-day-column day-'+g+'"><div class="wc-day-column-inner"></div></td>';e+="</tr></tbody></table></div>";c(i+e).appendTo(b);b.find(".wc-day-column-inner").each(function(){c(this).height(j.timeslotHeight*j.timeslotsPerDay);if(!j.readonly){d._addDroppableToWeekDay(c(this));d._setupEventCreationForWeekDay(c(this))}});b.find(".wc-time-slot").height(j.timeslotHeight-1);b.find(".wc-time-header-cell").css({height:j.timeslotHeight*
j.timeslotsPerHour-11,padding:5})},_setupEventCreationForWeekDay:function(b){var i=this,e=this.options;b.mousedown(function(d){var j=c(d.target);if(j.hasClass("wc-day-column-inner")){var g=c('<div class="wc-cal-event wc-new-cal-event wc-new-cal-event-creating"></div>');g.css({lineHeight:e.timeslotHeight-2+"px",fontSize:e.timeslotHeight/2+"px"});j.append(g);var l=j.offset().top;d=d.pageY-l;var m=(d-d%e.timeslotHeight)/e.timeslotHeight*e.timeslotHeight;g.css({top:m});j.bind("mousemove.newevent",function(q){g.show();
g.addClass("ui-resizable-resizing");q=Math.round(q.pageY-l-m);var x=q%e.timeslotHeight;if(x<q/2){q=q-x;g.css("height",q<e.timeslotHeight?e.timeslotHeight:q)}else g.css("height",q+(e.timeslotHeight-x))}).mouseup(function(){j.unbind("mousemove.newevent");g.addClass("ui-corner-all")})}}).mouseup(function(d){d=c(d.target).closest(".wc-day-column-inner");var j=d.find(".wc-new-cal-event-creating");if(j.length){j.hasClass("ui-resizable-resizing")||j.css({height:e.timeslotHeight*e.defaultEventLength}).show();
var g=parseInt(j.css("top"));g=i._getEventDurationFromPositionedEventElement(d,j,g);j.remove();j={start:g.start,end:g.end,title:e.newEventText};var l=i._renderEvent(j,d);if(e.allowCalEventOverlap)i._adjustOverlappingEvents(d);else{i._adjustForEventCollisions(d,l,j,j);i._positionEvent(d,l)}e.eventNew(g,l)}})},_loadCalEvents:function(b){var i,e,d=this,j=this.options;i=b||j.date;b=d._dateFirstDayOfWeek(i);weekEndDate=d._dateLastMilliOfWeek(i);j.calendarBeforeLoad(d.element);d.element.data("startDate",
b);d.element.data("endDate",weekEndDate);e=d.element.find(".wc-day-column-inner");d._updateDayColumnHeader(e);if(typeof j.data=="string"){j.loading&&j.loading(true);i={};i[j.startParam||"start"]=Math.round(b.getTime()/1E3);i[j.endParam||"end"]=Math.round(weekEndDate.getTime()/1E3);c.getJSON(j.data,i,function(g){d._renderEvents(g,e);j.loading&&j.loading(false)})}else if(c.isFunction(j.data))j.data(b,weekEndDate,function(g){d._renderEvents(g,e)});else j.data&&d._renderEvents(j.data,e);d._disableTextSelect(e)},
_updateDayColumnHeader:function(b){var i=this,e=this.options,d=i._cloneDate(i.element.data("startDate"));i.element.find(".wc-header td.wc-day-column-header").each(function(){var j=e.useShortDayNames?e.shortDays[d.getDay()]:e.longDays[d.getDay()];c(this).html(j+"<br/>"+i._formatDate(d,e.dateFormat));i._isToday(d)?c(this).addClass("wc-today"):c(this).removeClass("wc-today");d=i._addDays(d,1)});d=i._dateFirstDayOfWeek(i._cloneDate(i.element.data("startDate")));b.each(function(){c(this).data("startDate",
i._cloneDate(d));c(this).data("endDate",new Date(d.getTime()+f));i._isToday(d)?c(this).parent().addClass("wc-today"):c(this).parent().removeClass("wc-today");d=i._addDays(d,1)})},_renderEvents:function(b,i){this._clearCalendar();var e=this,d=this.options,j;if(c.isArray(b))j=e._cleanEvents(b);else if(b.events)j=e._cleanEvents(b.events);if(b.options){var g=false;c.each(b.options,function(l,m){if(m!==d[l]){d[l]=m;g=true}});e._computeOptions();if(g){e.element.empty();e._renderCalendar();i=e.element.find(".wc-time-slots .wc-day-column-inner");
e._updateDayColumnHeader(i);e._resizeCalendar()}}c.each(j,function(l,m){(l=e._findWeekDayForEvent(m,i))&&e._renderEvent(m,l)});i.each(function(){e._adjustOverlappingEvents(c(this))});d.calendarAfterLoad(e.element);j.length||d.noEvents()},_renderEvent:function(b,i){var e=this,d=this.options;if(!(b.start.getTime()>b.end.getTime())){var j,g;j=c('<div class="'+(b.id?"wc-cal-event":"wc-cal-event wc-new-cal-event")+' ui-corner-all">                <div class="wc-time ui-corner-all"></div>                <div class="wc-title"></div></div>');
j=(g=d.eventRender(b,j))?g.appendTo(i):j.appendTo(i);j.css({lineHeight:d.timeslotHeight-2+"px",fontSize:d.timeslotHeight/2+"px"});e._refreshEventDetails(b,j);e._positionEvent(i,j);j.show();!d.readonly&&d.resizable(b,j)&&e._addResizableToCalEvent(b,j,i);!d.readonly&&d.draggable(b,j)&&e._addDraggableToCalEvent(b,j);d.eventAfterRender(b,j);return j}},_adjustOverlappingEvents:function(b){var i=this;if(i.options.allowCalEventOverlap){b=i._groupOverlappingEventElements(b);c.each(b,function(){var e=this;
c.each(e,function(d){var j=this;if(i.options.overlapEventsSeparate)var g=100/e.length,l=d*g;else{g=100-(e.length-1)*10;l=d*10}c.each(j,function(){i.options.overlapEventsSeparate||c(this).bind("mouseover.z-index",function(){var m=c(this);c.each(j,function(){c(this).css({"z-index":"1"})});m.css({"z-index":"3"})});c(this).css({width:g+"%",left:l+"%",right:0})})})})}},_groupOverlappingEventElements:function(b){b=b.find(".wc-cal-event:visible").sort(function(g,l){return c(g).data("calEvent").start.getTime()-
c(l).data("calEvent").start.getTime()});var i=new Date(0,0,0),e=[],d=[],j;c.each(b,function(){j=c(this);if(d.length>0)if(i.getTime()<=j.data("calEvent").start.getTime()){e.push(d);d=[]}for(var g=0;g<d.length;g++)if(d[g].length>0)if(d[g][d[g].length-1].data("calEvent").end.getTime()<=j.data("calEvent").start.getTime()){d[g].push(j);if(i.getTime()<j.data("calEvent").end.getTime())i=j.data("calEvent").end;return}d.push([j]);if(i.getTime()<j.data("calEvent").end.getTime())i=j.data("calEvent").end});d.length>
0&&e.push(d);return e},_findWeekDayForEvent:function(b,i){var e;i.each(function(){if(c(this).data("startDate").getTime()<=b.start.getTime()&&c(this).data("endDate").getTime()>=b.end.getTime()){e=c(this);return false}});return e},_updateEventInCalendar:function(b){var i=this;i._cleanEvent(b);b.id&&i.element.find(".wc-cal-event").each(function(){if(c(this).data("calEvent").id===b.id||c(this).hasClass("wc-new-cal-event")){c(this).remove();return false}});var e=i._findWeekDayForEvent(b,i.element.find(".wc-time-slots .wc-day-column-inner"));
if(e){var d=i._renderEvent(b,e);i._adjustForEventCollisions(e,d,b,b);i._refreshEventDetails(b,d);i._positionEvent(e,d);i._adjustOverlappingEvents(e)}},_positionEvent:function(b,i){var e=this.options,d=i.data("calEvent");b=b.height()/e.millisToDisplay;e=e.businessHours.limitDisplay?e.businessHours.start:0;e=d.start.getTime()-(new Date(d.start.getFullYear(),d.start.getMonth(),d.start.getDate(),e)).getTime();d=d.end.getTime()-d.start.getTime();i.css({top:b*e,height:b*d})},_getEventDurationFromPositionedEventElement:function(b,
i,e){var d=this.options,j=d.businessHours.limitDisplay?d.businessHours.start*60*60*1E3:0;b=new Date(b.data("startDate").getTime()+j+Math.round(e/d.timeslotHeight)*d.millisPerTimeslot);i=new Date(b.getTime()+i.height()/d.timeslotHeight*d.millisPerTimeslot);return{start:b,end:i}},_adjustForEventCollisions:function(b,i,e,d,j){if(!this.options.allowCalEventOverlap){var g,l,m=this;b.find(".wc-cal-event").not(i).each(function(){var q=c(this).data("calEvent");if(e.start.getTime()<q.end.getTime()&&e.end.getTime()>=
q.end.getTime())g=q.end;if(e.end.getTime()>q.start.getTime()&&e.start.getTime()<=q.start.getTime())l=q.start;if(d.resizable==false||e.end.getTime()<=q.end.getTime()&&e.start.getTime()>=q.start.getTime()){g=d.start;l=d.end;return false}});e.start=g||e.start;if(g&&j){e.end=new Date(g.getTime()+(d.end.getTime()-d.start.getTime()));m._adjustForEventCollisions(b,i,e,d)}else e.end=l||e.end;if(e.start.getTime()>=e.end.getTime()){e.start=d.start;e.end=d.end}i.data("calEvent",e)}},_addDraggableToCalEvent:function(b,
i){var e=this,d=this.options;e._findWeekDayForEvent(b,e.element.find(".wc-time-slots .wc-day-column-inner"));i.draggable({handle:".wc-time",containment:".wc-scrollable-grid",revert:"valid",opacity:0.5,grid:[i.outerWidth()+1,d.timeslotHeight],start:function(j,g){d.eventDrag(b,g.draggable)}})},_addDroppableToWeekDay:function(b){var i=this,e=this.options;b.droppable({accept:".wc-cal-event",drop:function(d,j){var g=j.draggable;d=Math.round(parseInt(j.position.top));j=i._getEventDurationFromPositionedEventElement(b,
g,d);d=g.data("calEvent");j=c.extend(true,{},d,{start:j.start,end:j.end});i._adjustForEventCollisions(b,g,j,d,true);var l=i.element.find(".wc-day-column-inner");e.eventDrop(j,d,m);var m=i._renderEvent(j,i._findWeekDayForEvent(j,l));g.hide();g.data("preventClick",true);m=i._findWeekDayForEvent(g.data("calEvent"),i.element.find(".wc-time-slots .wc-day-column-inner"));m.data("startDate")!=b.data("startDate")&&i._adjustOverlappingEvents(m);i._adjustOverlappingEvents(b);setTimeout(function(){g.remove()},
1E3)}})},_addResizableToCalEvent:function(b,i,e){var d=this,j=this.options;i.resizable({grid:j.timeslotHeight,containment:e,handles:"s",minHeight:j.timeslotHeight,stop:function(g,l){var m=l.element;g=new Date(m.data("calEvent").start.getTime()+m.height()/j.timeslotHeight*j.millisPerTimeslot);g=c.extend(true,{},b,{start:b.start,end:g});d._adjustForEventCollisions(e,m,g,b);d._refreshEventDetails(g,m);d._positionEvent(e,m);d._adjustOverlappingEvents(e);j.eventResize(g,b,m);m.data("preventClick",true);
setTimeout(function(){m.removeData("preventClick")},500)}})},_refreshEventDetails:function(b,i){var e=this,d=this.options;b.end.getTime()-b.start.getTime()<=36E5/d.timeslotsPerHour?i.find(".wc-time").html(e._formatDate(b.start,d.timeFormat)+": "+b.title):i.find(".wc-time").html(e._formatDate(b.start,d.timeFormat)+d.timeSeparator+e._formatDate(b.end,d.timeFormat));i.find(".wc-title").html(b.title);i.data("calEvent",b)},_clearCalendar:function(){this.element.find(".wc-day-column-inner div").remove()},
_scrollToHour:function(b){var i=this,e=this.options,d=this.element.find(".wc-scrollable-grid"),j=b;if(i.options.businessHours.limitDisplay)j=b<=i.options.businessHours.start?0:b>i.options.businessHours.end?i.options.businessHours.end-i.options.businessHours.start-1:b-i.options.businessHours.start;var g=this.element.find(".wc-grid-timeslot-header .wc-hour-header:eq("+j+")");d.animate({scrollTop:0},0,function(){var l=g.offset().top-d.offset().top-g.outerHeight();d.animate({scrollTop:l},e.scrollToHourMillis)})},
_hourForIndex:function(b){return b===0?12:b<13?b:b-12},_24HourForIndex:function(b){return b===0?"00:00":b<10?"0"+b+":00":b+":00"},_amOrPm:function(b){return b<12?"AM":"PM"},_isToday:function(b){b=this._cloneDate(b);this._clearTime(b);var i=new Date;this._clearTime(i);return i.getTime()===b.getTime()},_cleanEvents:function(b){var i=this;c.each(b,function(e,d){i._cleanEvent(d)});return b},_cleanEvent:function(b){if(b.date)b.start=b.date;b.start=this._cleanDate(b.start);b.end=this._cleanDate(b.end);
if(!b.end)b.end=this._addDays(this._cloneDate(b.start),1)},_disableTextSelect:function(b){b.each(function(){if(c.browser.mozilla)c(this).css("MozUserSelect","none");else c.browser.msie?c(this).bind("selectstart",function(){return false}):c(this).mousedown(function(){return false})})},_dateFirstDayOfWeek:function(b){var i=this;b=new Date(b.getFullYear(),b.getMonth(),b.getDate());var e=new Date(b);e.setDate(e.getDate()-i._getAdjustedDayIndex(b));return e},_dateLastDayOfWeek:function(b){b=new Date(b.getFullYear(),
b.getMonth(),b.getDate());var i=new Date(b);i.setDate(i.getDate()+(6-this._getAdjustedDayIndex(b)));return i},_getAdjustedDayIndex:function(b){b=(new Date(b.getFullYear(),b.getMonth(),b.getDate())).getDay();var i=[0,1,2,3,4,5,6];this._rotate(i,this.options.firstDayOfWeek);return i[b]},_dateLastMilliOfWeek:function(b){b=this._dateLastDayOfWeek(b);return new Date(b.getTime()+f)},_clearTime:function(b){b.setHours(0);b.setMinutes(0);b.setSeconds(0);b.setMilliseconds(0);return b},_addDays:function(b,i,
e){b.setDate(b.getDate()+i);if(e)return b;return this._clearTime(b)},_rotate:function(b,i){var e=b.length;i=(Math.abs(i)>=e&&(i%=e),i<0&&(i+=e),i);for(var d,j;i;i=(Math.ceil(e/i)-1)*i-e+(e=i))for(d=e;d>i;j=b[--d],b[d]=b[d-i],b[d-i]=j);return b},_cloneDate:function(b){return new Date(b.getTime())},_cleanDate:function(b){if(typeof b=="string")return c.weekCalendar.parseISO8601(b,true)||Date.parse(b)||new Date(parseInt(b));if(typeof b=="number")return new Date(b);return b},_formatDate:function(b,i){for(var e=
this.options,d="",j=0;j<i.length;j++){var g=i.charAt(j);if(c.isFunction(this._replaceChars[g])){g=this._replaceChars[g](b,e);if(g==="00"&&e.alwaysDisplayTimeMinutes===false)d=d.slice(0,-1);else d+=g}else d+=g}return d},_replaceChars:{d:function(b){return(b.getDate()<10?"0":"")+b.getDate()},D:function(b,i){return i.shortDays[b.getDay()]},j:function(b){return b.getDate()},l:function(b,i){return i.longDays[b.getDay()]},N:function(b){return b.getDay()+1},S:function(b){return b.getDate()%10==1&&b.getDate()!=
11?"st":b.getDate()%10==2&&b.getDate()!=12?"nd":b.getDate()%10==3&&b.getDate()!=13?"rd":"th"},w:function(b){return b.getDay()},z:function(){return"Not Yet Supported"},W:function(){return"Not Yet Supported"},F:function(b,i){return i.longMonths[b.getMonth()]},m:function(b){return(b.getMonth()<9?"0":"")+(b.getMonth()+1)},M:function(b,i){return i.shortMonths[b.getMonth()]},n:function(b){return b.getMonth()+1},t:function(){return"Not Yet Supported"},L:function(){return"Not Yet Supported"},o:function(){return"Not Supported"},
Y:function(b){return b.getFullYear()},y:function(b){return(""+b.getFullYear()).substr(2)},a:function(b){return b.getHours()<12?"am":"pm"},A:function(b){return b.getHours()<12?"AM":"PM"},B:function(){return"Not Yet Supported"},g:function(b){return b.getHours()%12||12},G:function(b){return b.getHours()},h:function(b){return((b.getHours()%12||12)<10?"0":"")+(b.getHours()%12||12)},H:function(b){return(b.getHours()<10?"0":"")+b.getHours()},i:function(b){return(b.getMinutes()<10?"0":"")+b.getMinutes()},
s:function(b){return(b.getSeconds()<10?"0":"")+b.getSeconds()},e:function(){return"Not Yet Supported"},I:function(){return"Not Supported"},O:function(b){return(b.getTimezoneOffset()<0?"-":"+")+(b.getTimezoneOffset()/60<10?"0":"")+b.getTimezoneOffset()/60+"00"},T:function(){return"Not Yet Supported"},Z:function(b){return b.getTimezoneOffset()*60},c:function(){return"Not Yet Supported"},r:function(b){return b.toString()},U:function(b){return b.getTime()/1E3}}});c.extend(c.ui.weekCalendar,{version:"1.2.2-pre"});
var f=864E5,k=f*7;c.weekCalendar=function(){return{parseISO8601:function(b,i){b=b.match(new RegExp("([0-9]{4})(-([0-9]{2})(-([0-9]{2})(T([0-9]{2}):([0-9]{2})(:([0-9]{2})(.([0-9]+))?)?(Z|(([-+])([0-9]{2}):([0-9]{2})))?)?)?)?"));if(!b)return null;var e=0,d=new Date(b[1],0,1);b[3]&&d.setMonth(b[3]-1);b[5]&&d.setDate(b[5]);b[7]&&d.setHours(b[7]);b[8]&&d.setMinutes(b[8]);b[10]&&d.setSeconds(b[10]);b[12]&&d.setMilliseconds(Number("0."+b[12])*1E3);if(!i){if(b[14]){e=Number(b[16])*60+Number(b[17]);e*=b[15]==
"-"?1:-1}e-=d.getTimezoneOffset()}return new Date(Number(d)+e*60*1E3)}}}()})(jQuery);
...
this.getLabelHeight=function(){if(this.label)return this.label.height()};this.getDate=function(){return this.date}}var k=[],b={},i=null,e=0,d=null,j=null,g=0,l=0,m=0,q=0,x={startDate:null,endDate:null},u=[],s={containerId:"#jMonthCalendar",headerHeight:50,firstDayOfWeek:0,calendarStartDate:new Date,dragableEvents:true,dragHoverClass:"DateBoxOver",navLinks:{enableToday:true,enableNextYear:true,enablePrevYear:true,p:"&lsaquo; Prev",n:"Next &rsaquo;",t:"Today",showMore:"Show More"},onMonthChanging:function(){},
...
o[2]-1,o[3],o[4],o[5],o[6]);else if(o=n.match(p))return new Date(o[1],o[2]-1,o[3],o[4],o[5],o[6]);else if(o=n.match(z))return new Date(o[1],o[2]-1,o[3]);if(o=n.match(/^\\\/Date\(([0-9]{13})-([0-9]{4})\)\\\/$/))return new Date(o[1]);if(n.match(/^new/))return eval("("+n+")")}},K=function(){if(u&&u.length>0){var n=[],o=[];c.each(u,function(){var p=this;if(p.StartDateTime){if(typeof p.StartDateTime=="object"&&p.StartDateTime.getDate)this.StartDateTime=p.StartDateTime;if(typeof p.StartDateTime=="string"&&
...
0)o.push(this);else this.StartDateTime.clone().clearTime().compareTo(this.EndDateTime.clone().clearTime())==-1&&n.push(this)});n.sort(H);o.sort(H);u=[];c.merge(u,n);c.merge(u,o)}},H=function(n,o){return n.StartDateTime.compareTo(o.StartDateTime)},M=function(){I();k=[]},I=function(){for(var n=0;n<k.length;n++)k[n].clear();b={}},N=function(n){var o=s.calendarStartDate;if(n==undefined)i=new Date(o.getFullYear(),o.getMonth(),1);else{i=n;i.setDate(1)}e=i.getDaysInMonth();d=i.clone().moveToFirstDayOfMonth();
j=i.clone().moveToLastDayOfMonth();g=d.getDay()-s.firstDayOfWeek;l=g+e;m=Math.ceil(l/7);q=m*7;x.startDate=d.clone().addDays(-1*g);x.endDate=j.clone().addDays(q-(e+g))},O=function(){var n=i.clone().addMonths(-1),o=c('<div class="MonthNavPrev"><a class="link-prev">'+s.navLinks.p+"</a></div>").click(function(){c.J.ChangeMonth(n);return false}),p=i.clone().addMonths(1),z=c('<div class="MonthNavNext"><a class="link-next">'+s.navLinks.n+"</a></div>").click(function(){c.J.ChangeMonth(p);return false}),B=
i.clone().addYears(-1),v;if(s.navLinks.enablePrevYear)v=c('<div class="YearNavPrev"><a>'+B.getFullYear()+"</a></div>").click(function(){c.J.ChangeMonth(B);return false});var A=i.clone().addYears(1),t;if(s.navLinks.enableNextYear)t=c('<div class="YearNavNext"><a>'+A.getFullYear()+"</a></div>").click(function(){c.J.ChangeMonth(A);return false});var w;if(s.navLinks.enableToday)w=c('<div class="TodayLink"><a class="link-today">'+s.navLinks.t+"</a></div>").click(function(){c.J.ChangeMonth(new Date);return false});
var C=c('<tr><td colspan="7"><div class="FormHeader MonthNavigation"></div></td></tr>'),y=c(".MonthNavigation",C);y.append(o,z);s.navLinks.enableToday&&y.append(w);y.append(c('<div class="MonthName"></div>').append(Date.CultureInfo.monthNames[i.getMonth()]+" "+i.getFullYear()));s.navLinks.enablePrevYear&&y.append(v);s.navLinks.enableNextYear&&y.append(t);o=c("<tr></tr>");for(z=s.firstDayOfWeek;z<s.firstDayOfWeek+7;z++){v=z%7;t=Date.CultureInfo.dayNames[v];o.append('<th title="'+t+'" class="DateHeader'+
(v==0||v==6?" Weekend":"")+'"><span>'+t+"</span></th>")}o=c('<thead id="CalendarHead"></thead>').css({height:s.headerHeight+"px"}).append(o);return o=o.prepend(C)};c.J.DrawCalendar=function(n){var o=new Date;o.clearTime();var p=s.calendarStartDate;M();N(n);n=O();i.getMonth()==p.getMonth()&&(i.getFullYear(),p.getFullYear());var z=(c(s.containerId).outerHeight()-s.headerHeight)/m,B=null;p=c('<tbody id="CalendarBody"></tbody>');for(var v=0;v<q;v++){var A=x.startDate.clone().addDays(v);if(v%7==0||v==
0){B=c("<tr></tr>");B.css({height:z+"px"});p.append(B)}var t=(s.firstDayOfWeek+v)%7,w={"class":"DateBox"+(t==0||t==6?" Weekend ":""),date:A.toString("M/d/yyyy")};if(A.compareTo(d)==-1||A.compareTo(j)==1)w["class"]+=" Inactive";if(A.compareTo(o)==0)w["class"]+=" Today";t=c('<div class="DateLabel"><a>'+A.getDate()+"</a></div>");t.bind("click",{Date:A.clone()},s.onDayLinkClick);w=c("<td></td>").attr(w).append(t);w.bind("dblclick",{Date:A.clone()},s.onDayCellDblClick);w.bind("click",{Date:A.clone()},
s.onDayCellClick);s.dragableEvents&&w.droppable({hoverClass:s.dragHoverClass,tolerance:"pointer",drop:function(C,y){var J=y.draggable.attr("eventid"),D=(new Date(c(this).attr("date"))).clearTime(),F;c.each(u,function(){if(this.EventID==J){var L=(new TimeSpan(D-this.StartDateTime)).days;this.StartDateTime.addDays(L);this.EndDateTime.addDays(L);F=this}});c.J.ClearEventsOnCalendar();G();s.onEventDropped.call(this,F,D)}});k.push(new f(v,A,w,t));B.append(w)}p.append(B);o=c(s.containerId);n=c('<table class="MonthlyCalendar" cellpadding="0" tablespacing="0"></table>').append(n,
p);o.hide();o.html(n);o.fadeIn("normal");G()};var G=function(){K();I();if(u&&u.length>0){var n=c(s.containerId);c.each(u,function(){var p=this,z=p.StartDateTime.clone().clearTime(),B=p.EndDateTime.clone().clearTime(),v=(new TimeSpan(z-x.startDate)).days,A=(new TimeSpan(B-x.startDate)).days;A=A>k.length-1?k.length-1:A;for(v=v<0?0:v;v<=A;v++){var t=k[v],w=z.compareTo(t.date),C=B.compareTo(t.date),y=v!=0&&w==-1&&C>=0&&t.weekNumber!=k[v-1].weekNumber||v==0&&w==-1;if((w==0||v==0&&w==-1||y||w==-1&&C>=0)&&
t.vOffset>=t.getCellBox().height()-t.getLabelHeight()-32){if(!t.isTooManySet){y=c('<div class="MoreEvents" id="ME_'+v+'">'+s.navLinks.showMore+"</div>");w=t.getCellPosition();var J=v;y.css({top:w.top+(t.getCellBox().height()-t.getLabelHeight()),left:w.left,width:t.getLabelWidth()-7,position:"absolute"});y.click(function(F){P(F,J)});b[y.attr("id")]=y;t.isTooManySet=true}t.events.push(p)}else if(w==0||v==0&&w==-1||y){var D=Q(p,t.weekNumber);w=t.getCellPosition();D.css({top:w.top+t.getLabelHeight()+
t.vOffset,left:w.left,width:t.getLabelWidth()-7,position:"absolute"});t.vOffset+=19;if(y){D.prepend(c("<span />").addClass("ui-icon").addClass("ui-icon-triangle-1-w"));(y=b["Event_"+p.EventID+"_"+(t.weekNumber-1)])&&y.prepend(c("<span />").addClass("ui-icon").addClass("ui-icon-triangle-1-e"))}b[D.attr("id")]=D;t.events.push(p)}else if(w==-1&&C>=0)if(y=b["Event_"+p.EventID+"_"+t.weekNumber]){w=y.css("width");y.css({width:parseInt(w)+t.getLabelWidth()+1});t.vOffset+=19;t.events.push(p)}if(v==A&&C>0)(y=
...
c(s.containerId)).remove()};c.J.AddEvents=function(n){if(n){n.length>0?c.merge(u,n):u.push(n);c.J.ClearEventsOnCalendar();G()}};c.J.ReplaceEventCollection=function(n){if(n){u=[];u=n}c.J.ClearEventsOnCalendar();G()};c.J.ChangeMonth=function(n){if(!s.onMonthChanging.call(this,n)){c.J.DrawCalendar(n);s.onMonthChanged.call(this,n)}};c.J.Initialize=function(n,o){c.extend(s,n);if(o){c.J.ClearEventsOnCalendar();u=o}c.J.DrawCalendar()}})(jQuery);
...
j=j||null;return function(g){var l=null;for(var m=l=null,q=null,x=[[],g],u=false,s=0;s<e.length;s++){l=m=null;u=e.length==1;try{l=e[s].call(this,g)}catch(E){continue}q=[[l[0]],l[1]];if(l[1].length>0&&!u)try{m=d.call(this,l[1])}catch(K){u=true}else u=true;if(!u&&m[1].length===0)u=true;if(!u){l=[];for(u=0;u<e.length;u++)s!=u&&l.push(e[u]);l=f.set(l,d).call(this,m[1]);if(l[0].length>0){q[0]=q[0].concat(l[0]);q[1]=l[1]}}if(q[1].length<x[1].length)x=q;if(x[1].length===0)break}if(x[0].length===0)return x;
if(j){try{m=j.call(this,x[1])}catch(H){throw new c.Exception(x[1]);}x[1]=m[1]}return x}},forward:function(e,d){return function(j){return e[d].call(this,j)}},replace:function(e,d){return function(j){j=e.call(this,j);return[d,j[1]]}},process:function(e,d){return function(j){j=e.call(this,j);return[d.call(this,j[0]),j[1]]}},min:function(e,d){return function(j){var g=d.call(this,j);if(g[0].length<e)throw new c.Exception(j);return g}}},k=function(e){return function(){var d=null,j=[];if(arguments.length>
