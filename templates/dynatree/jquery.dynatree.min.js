...
b.title!==undefined&&_log("warn","Option 'title' is no longer supported.");if(b.minExpandLevel<1){_log("warn","Option 'minExpandLevel' must be >= 1.");b.minExpandLevel=1}if(b.classNames!==f.ui.dynatree.prototype.options.classNames)b.classNames=f.extend({},f.ui.dynatree.prototype.options.classNames,b.classNames);if(b.ajaxDefaults!==f.ui.dynatree.prototype.options.ajaxDefaults)b.ajaxDefaults=f.extend({},f.ui.dynatree.prototype.options.ajaxDefaults,b.ajaxDefaults);if(b.dnd!==f.ui.dynatree.prototype.options.dnd)b.dnd=
f.extend({},f.ui.dynatree.prototype.options.dnd,b.dnd);b.imagePath||f("script").each(function(){if(this.src.search(/.*dynatree[^\/]*\.js$/i)>=0){b.imagePath=this.src.indexOf("/")>=0?this.src.slice(0,this.src.lastIndexOf("/"))+"/skin/":"skin/";logMsg("Guessing imagePath from '%s': '%s'",this.src,b.imagePath);return false}});this.persistence=new n(b.cookieId,b.cookie);if(b.persist){f.cookie||_log("warn","Please include jquery.cookie.js to use persistence.");this.persistence.read()}this.logDebug("DynaTree.persistence: %o",
this.persistence.toDict());this.cache={tagEmpty:"<span class='"+b.classNames.empty+"'></span>",tagVline:"<span class='"+b.classNames.vline+"'></span>",tagExpander:"<span class='"+b.classNames.expander+"'></span>",tagConnector:"<span class='"+b.classNames.connector+"'></span>",tagNodeIcon:"<span class='"+b.classNames.nodeIcon+"'></span>",tagCheckbox:"<span class='"+b.classNames.checkbox+"'></span>",lastentry:undefined};if(b.children||b.initAjax&&b.initAjax.url||b.initId)f(this.divTree).empty();else this.divRoot&&
f(this.divRoot).remove();this.tnRoot=new p(null,this,{});this.tnRoot.bExpanded=true;this.tnRoot.render();this.divTree.appendChild(this.tnRoot.ul);var c=this.tnRoot,d=b.persist&&this.persistence.isReloading(),e=false,g=this.enableUpdate(false);this.logDebug("Dynatree._load(): read tree structure...");if(b.children)c.addChild(b.children);else if(b.initAjax&&b.initAjax.url){e=true;c.data.isLazy=true;this._reloadAjax(a)}else if(b.initId)this._createFromTag(c,f("#"+b.initId));else{a=this.$tree.find(">ul:first").hide();
this._createFromTag(c,a);a.remove()}this._checkConsistency();this.logDebug("Dynatree._load(): render nodes...");this.enableUpdate(g);this.logDebug("Dynatree._load(): bind events...");this.$widget.bind();this.logDebug("Dynatree._load(): postInit...");this.phase="postInit";b.persist&&this.persistence.write();if(this.focusNode&&this.focusNode.isVisible()){this.logDebug("Focus on init: %o",this.focusNode);this.focusNode.focus()}!e&&b.onPostInit&&b.onPostInit.call(this,d,false);this.phase="idle"},_reloadAjax:function(a){var b=
