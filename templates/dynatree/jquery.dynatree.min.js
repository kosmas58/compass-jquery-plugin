...
null&&e.push(c.lazy);this.isLastSib&&e.push(c.lastsib);this.bSelected&&e.push(c.selected);this.hasSubSel&&e.push(c.partsel);this.tree.activeNode===this&&e.push(c.active);this.data.addClass&&e.push(this.data.addClass);e.push(c.combinedExpanderPrefix+(this.bExpanded?"e":"c")+(this.data.isLazy&&this.childList==null?"d":"")+(d?"l":""));e.push(c.combinedIconPrefix+(this.bExpanded?"e":"c")+(this.data.isFolder?"f":""));this.span.className=e.join(" ");if(d)this.li.className="dynatree-lastsib"}if(this.bExpanded&&
this.childList){for(c=0;c<this.childList.length;c++)this.childList[c].render();this._fixOrder()}if(this.ul){c=this.ul.style.display=="none";if(a&&b.fx&&!!c==!!this.bExpanded){a=b.fx.duration||200;g(this.ul).animate(b.fx,a)}else this.ul.style.display=this.bExpanded||!this.parent?"":"none"}},getParent:function(){return this.parent},getChildren:function(){return this.childList},hasChildren:function(){return this.childList!=null},isLastSibling:function(){var a=this.parent;if(!a)return true;return a.childList[a.childList.length-
...
if(this.childList.length==1)this.childList=null;else this.childList.shift()}},setLazyNodeStatus:function(a,b){var c=b&&b.tooltip?b.tooltip:null;b=b&&b.info?" ("+b.info+")":"";switch(a){case DTNodeStatus_Ok:this._setStatusNode(null);this.isLoading=false;this.render();if(this.tree.options.autoFocus)this===this.tree.tnRoot&&this.childList?this.childList[0].focus():this.focus();break;case DTNodeStatus_Loading:this.isLoading=true;this._setStatusNode({title:this.tree.options.strings.loading+b,tooltip:c,
...
this.ul.removeChild(a.li);for(var c=0;c<b.length;c++)if(b[c]===a){this.childList.splice(c,1);delete a;break}},removeChildren:function(a,b){var c=this.tree,d=this.childList;if(d){for(var e=0;e<d.length;e++){var f=d[e];f===c.activeNode&&!b&&f.deactivate();if(this.tree.options.persist&&!b){f.bSelected&&this.tree.persistence.clearSelect(f.data.key);f.bExpanded&&this.tree.persistence.clearExpand(f.data.key)}f.removeChildren(true,b);this.ul&&this.ul.removeChild(f.li);delete f}this.childList=null}if(!a){this.isLoading=
false;this.render()}},reload:function(a){if(this.parent==null)return this.tree.reload();if(!this.data.isLazy)throw"node.reload() requires lazy nodes.";if(this.bExpanded){this.expand(false);this.removeChildren();this.expand(true)}else{this.removeChildren();a&&this._loadContent()}},_addChildNode:function(a,b){var c=this.tree,d=c.options,e=c.persistence;a.parent=this;if(this.childList==null)this.childList=[];else b||g(this.childList[this.childList.length-1].span).removeClass(d.classNames.lastsib);if(b){b=
g.inArray(b,this.childList);if(b<0)throw"<beforeNode> must be a child of <this>";this.childList.splice(b,0,a)}else this.childList.push(a);b=c.isInitializing();if(d.persist&&e.cookiesFound&&b){if(e.activeKey==a.data.key)c.activeNode=a;if(e.focusedKey==a.data.key)c.focusNode=a;a.bExpanded=g.inArray(a.data.key,e.expandedKeyList)>=0;a.bSelected=g.inArray(a.data.key,e.selectedKeyList)>=0}else{if(a.data.activate){c.activeNode=a;if(d.persist)e.activeKey=a.data.key}if(a.data.focus){c.focusNode=a;if(d.persist)e.focusedKey=
a.data.key}a.bExpanded=a.data.expand==true;a.bExpanded&&d.persist&&e.addExpand(a.data.key);a.bSelected=a.data.select==true;a.bSelected&&d.persist&&e.addSelect(a.data.key)}if(d.minExpandLevel>=a.getLevel())this.bExpanded=true;if(a.bSelected&&d.selectMode==3)for(d=this;d;){d.hasSubSel||d._setSubSel(true);d=d.parent}c.bEnableUpdate&&this.render();return a},addChild:function(a,b){if(!(!a||a.length==0)){if(a instanceof o)return this._addChildNode(a,b);a.length||(a=[a]);for(var c=this.tree.enableUpdate(false),
d=null,e=0;e<a.length;e++){var f=a[e],h=this._addChildNode(new o(this,this.tree,f),b);d||(d=h);f.children&&h.addChild(f.children,null)}this.tree.enableUpdate(c);return d}},append:function(a){this.tree.logWarning("node.append() is deprecated (use node.addChild() instead).");return this.addChild(a,null)},appendAjax:function(a){this.removeChildren(false,true);this.setLazyNodeStatus(DTNodeStatus_Loading);var b=this,c=a.success,d=a.error,e=g.extend({},this.tree.options.ajaxDefaults,a,{success:function(f){var h=
b.tree.phase;b.tree.phase="init";b.addChild(f,null);b.tree.phase="postInit";b.setLazyNodeStatus(DTNodeStatus_Ok);c&&c.call(e,b);b.tree.phase=h},error:function(f,h,i){b.tree.logWarning("appendAjax failed:",h,":\n",f,"\n",i);b.setLazyNodeStatus(DTNodeStatus_Error,{info:h,tooltip:""+i});d&&d.call(e,b,f,h,i)}});g.ajax(e)},lastentry:undefined};var m=n.create();m._getTreePersistData=function(a,b){a=new m(a,b);a.read();return a.toDict()};getDynaTreePersistData=m._getTreePersistData;m.prototype={initialize:function(a,
b){this._log("DynaTreeStatus: initialize");if(a===undefined)a=g.ui.dynatree.prototype.options.cookieId;b=g.extend({},g.ui.dynatree.prototype.options.cookie,b);this.cookieId=a;this.cookieOpts=b;this.cookiesFound=undefined;this.selectedKeyList=this.expandedKeyList=this.focusedKey=this.activeKey=null},_log:function(){Array.prototype.unshift.apply(arguments,["debug"]);_log.apply(this,arguments)},read:function(){this._log("DynaTreeStatus: read");this.cookiesFound=false;var a=g.cookie(this.cookieId+"-active");
this.activeKey=a==null?"":a;if(a!=null)this.cookiesFound=true;a=g.cookie(this.cookieId+"-focus");this.focusedKey=a==null?"":a;if(a!=null)this.cookiesFound=true;a=g.cookie(this.cookieId+"-expand");this.expandedKeyList=a==null?[]:a.split(",");if(a!=null)this.cookiesFound=true;a=g.cookie(this.cookieId+"-select");this.selectedKeyList=a==null?[]:a.split(",");if(a!=null)this.cookiesFound=true},write:function(){this._log("DynaTreeStatus: write");g.cookie(this.cookieId+"-active",this.activeKey==null?"":this.activeKey,
this.cookieOpts);g.cookie(this.cookieId+"-focus",this.focusedKey==null?"":this.focusedKey,this.cookieOpts);g.cookie(this.cookieId+"-expand",this.expandedKeyList==null?"":this.expandedKeyList.join(","),this.cookieOpts);g.cookie(this.cookieId+"-select",this.selectedKeyList==null?"":this.selectedKeyList.join(","),this.cookieOpts)},addExpand:function(a){this._log("addExpand(%o)",a);if(g.inArray(a,this.expandedKeyList)<0){this.expandedKeyList.push(a);g.cookie(this.cookieId+"-expand",this.expandedKeyList.join(","),
this.cookieOpts)}},clearExpand:function(a){this._log("clearExpand(%o)",a);a=g.inArray(a,this.expandedKeyList);if(a>=0){this.expandedKeyList.splice(a,1);g.cookie(this.cookieId+"-expand",this.expandedKeyList.join(","),this.cookieOpts)}},addSelect:function(a){this._log("addSelect(%o)",a);if(g.inArray(a,this.selectedKeyList)<0){this.selectedKeyList.push(a);g.cookie(this.cookieId+"-select",this.selectedKeyList.join(","),this.cookieOpts)}},clearSelect:function(a){this._log("clearSelect(%o)",a);a=g.inArray(a,
this.selectedKeyList);if(a>=0){this.selectedKeyList.splice(a,1);g.cookie(this.cookieId+"-select",this.selectedKeyList.join(","),this.cookieOpts)}},isReloading:function(){return this.cookiesFound==true},toDict:function(){return{cookiesFound:this.cookiesFound,activeKey:this.activeKey,focusedKey:this.activeKey,expandedKeyList:this.expandedKeyList,selectedKeyList:this.selectedKeyList}},lastentry:undefined};var p=n.create();p.version="$Version:$";p.prototype={initialize:function(a){this.phase="init";this.$widget=
a;this.options=a.options;this.$tree=a.element;this.timer=null;this.divTree=this.$tree.get(0);l(this)},_load:function(){var a=this.options;this.bEnableUpdate=true;this._nodeCount=1;this.focusNode=this.activeNode=null;a.rootVisible!==undefined&&_log("warn","Option 'rootVisible' is no longer supported.");a.title!==undefined&&_log("warn","Option 'title' is no longer supported.");if(a.minExpandLevel<1){_log("warn","Option 'minExpandLevel' must be >= 1.");a.minExpandLevel=1}if(a.classNames!==g.ui.dynatree.prototype.options.classNames)a.classNames=
g.extend({},g.ui.dynatree.prototype.options.classNames,a.classNames);a.imagePath||g("script").each(function(){if(this.src.search(_rexDtLibName)>=0){a.imagePath=this.src.indexOf("/")>=0?this.src.slice(0,this.src.lastIndexOf("/"))+"/skin/":"skin/";return false}});this.persistence=new m(a.cookieId,a.cookie);if(a.persist){g.cookie||_log("warn","Please include jquery.cookie.js to use persistence.");this.persistence.read()}this.logDebug("DynaTree.persistence: %o",this.persistence.toDict());this.cache={tagEmpty:"<span class='"+
a.classNames.empty+"'></span>",tagVline:"<span class='"+a.classNames.vline+"'></span>",tagExpander:"<span class='"+a.classNames.expander+"'></span>",tagConnector:"<span class='"+a.classNames.connector+"'></span>",tagNodeIcon:"<span class='"+a.classNames.nodeIcon+"'></span>",tagCheckbox:"<span class='"+a.classNames.checkbox+"'></span>",lastentry:undefined};if(a.children||a.initAjax&&a.initAjax.url||a.initId)g(this.divTree).empty();else this.divRoot&&g(this.divRoot).remove();this.tnRoot=new o(null,
...
if(b){this.activeNode=null;b.activate();a&&b.focus()}},getSelectedNodes:function(a){var b=[];this.tnRoot.visit(function(c){if(c.bSelected){b.push(c);if(a==true)return false}});return b},activateKey:function(a){a=a===null?null:this.getNodeByKey(a);if(!a){this.activeNode&&this.activeNode.deactivate();return this.activeNode=null}a.focus();a.activate();return a},selectKey:function(a,b){a=this.getNodeByKey(a);if(!a)return null;a.select(b);return a},enableUpdate:function(a){if(this.bEnableUpdate==a)return a;
(this.bEnableUpdate=a)&&this.redraw();return!a},visit:function(a,b,c){return this.tnRoot.visit(a,b,c)},_createFromTag:function(a,b){var c=this;b.find(">li").each(function(){var d=g(this),e=d.find(">span:first");if(e.length)e=e.html();else{e=d.html();var f=e.search(/<ul/i);e=f>=0?g.trim(e.substring(0,f)):g.trim(e)}e={title:e,isFolder:d.hasClass("folder"),isLazy:d.hasClass("lazy"),expand:d.hasClass("expanded"),select:d.hasClass("selected"),activate:d.hasClass("active"),focus:d.hasClass("focused")};
...
a,b,c,this);var h=this.options.dnd,i=null,j=g(b.span);switch(a){case "helper":c=g("<div class='dynatree-drag-helper'><span class='dynatree-drag-helper-img' /></div>").append(g(d.target).closest("a").clone());c.data("dtSourceNode",b);logMsg("helper.sourceNode=%o",c.data("dtSourceNode"));i=c;break;case "start":if(h.onDragStart)i=h.onDragStart(b);if(i===false){this.logDebug("tree.onDragStart() cancelled");f._clear()}else j.addClass("dynatree-drag-source");break;case "enter":i=h.onDragEnter?h.onDragEnter(b,
c):null;e.helper.data("enterResponse",i);this.logDebug("helper.enterResponse: %o",i);this._setDndStatus(c,b,e.helper,"over",i!==false);break;case "over":h.autoExpandMS&&b.hasChildren()&&!b.bExpanded&&b.scheduleAction("expand",h.autoExpandMS);a=e.helper.data("enterResponse");f=null;if(a===false)break;else if(typeof a==="string")f=a;else{a=j.position();d={x:d.clientX-a.left,y:d.clientY-a.top};j={x:d.x/j.width(),y:d.y/j.height()};f=j.y>0.25&&j.y<0.75||j.x>0.8?"over":j.y<=0.25?"before":"after"}if(h.onDragOver)i=
h.onDragOver(b,c,f);this._setDndStatus(c,b,e.helper,f,i!==false);break;case "drop":h.onDrop&&h.onDrop(b,c);break;case "leave":b.scheduleAction("cancel");e.helper.data("enterResponse",null);this._setDndStatus(c,b,e.helper,"out",undefined);h.onDragLeave&&h.onDragLeave(b,c);break;case "stop":j.removeClass("dynatree-drag-source");h.onDragStop&&h.onDragStop(b);break;default:throw"Unsupported drag event: "+a;}return i},lastentry:undefined};g.widget("ui.dynatree",{_init:function(){if(parseFloat(g.ui.version)<
1.8){_log("warn","ui.dynatree._init() was called; you should upgrade to jquery.ui.core.js v1.8 or higher.");return this._create()}_log("debug","ui.dynatree._init() was called; no current default functionality.")},_create:function(){logMsg("Dynatree._create(): version='%s', debugLevel=%o.",p.version,this.options.debugLevel);this.options.event+=".dynatree";this.element.get(0);this.tree=new p(this);this.tree._load();this.tree.logDebug("Dynatree._init(): done.")},bind:function(){function a(e){e=e=g.event.fix(e||
window.event);var f=k(e.target);return f?f.onFocus(e):false}var b=this.element,c=this.options;this.unbind();var d="click.dynatree dblclick.dynatree";if(c.keyboard)d+=" keypress.dynatree keydown.dynatree";b.bind(d,function(e){var f=k(e.target);if(!f)return true;var h=f.tree.phase;f.tree.phase="userEvent";try{f.tree.logDebug("bind(%o): dtnode: %o",e,f);switch(e.type){case "click":return c.onClick&&c.onClick(f,e)===false?false:f.onClick(e);case "dblclick":return c.onDblClick&&c.onDblClick(f,e)===false?
false:f.onDblClick(e);case "keydown":return c.onKeydown&&c.onKeydown(f,e)===false?false:f.onKeydown(e);case "keypress":return c.onKeypress&&c.onKeypress(f,e)===false?false:f.onKeypress(e)}}catch(i){f.tree.logWarning("bind(%o): dtnode: %o, error: %o",e,f,i)}finally{f.tree.phase=h}});b=this.tree.divTree;if(b.addEventListener){b.addEventListener("focus",a,true);b.addEventListener("blur",a,true)}else b.onfocusin=b.onfocusout=a},unbind:function(){this.element.unbind(".dynatree")},enable:function(){this.bind();
g.Widget.prototype.enable.apply(this,arguments)},disable:function(){this.unbind();g.Widget.prototype.disable.apply(this,arguments)},getTree:function(){return this.tree},getRoot:function(){return this.tree.getRoot()},getActiveNode:function(){return this.tree.getActiveNode()},getSelectedNodes:function(){return this.tree.getSelectedNodes()},lastentry:undefined});g.ui.dynatree.prototype.options={title:"Dynatree",minExpandLevel:1,imagePath:null,children:null,initId:null,initAjax:null,autoFocus:true,keyboard:true,
persist:false,autoCollapse:false,clickFolderMode:3,activeVisible:true,checkbox:false,selectMode:2,fx:null,enableDrag:false,enableDrop:false,onClick:null,onDblClick:null,onKeydown:null,onKeypress:null,onFocus:null,onBlur:null,onQueryActivate:null,onQuerySelect:null,onQueryExpand:null,onPostInit:null,onActivate:null,onDeactivate:null,onSelect:null,onExpand:null,onLazyRead:null,dnd:{onDragStart:null,onDragStop:null,helper:null,autoExpandMS:1E3,onDragEnter:null,onDragOver:null,onDrop:null,onDragLeave:null},
...
nodeWait:"dynatree-statusnode-wait",hidden:"dynatree-hidden",combinedExpanderPrefix:"dynatree-exp-",combinedIconPrefix:"dynatree-ico-",hasChildren:"dynatree-has-children",active:"dynatree-active",selected:"dynatree-selected",expanded:"dynatree-expanded",lazy:"dynatree-lazy",focused:"dynatree-focused",partsel:"dynatree-partsel",lastsib:"dynatree-lastsib"},debugLevel:2,lastentry:undefined};g.ui.dynatree.nodedatadefaults={title:null,key:null,isFolder:false,isLazy:false,tooltip:null,icon:null,addClass:null,
activate:false,focus:false,expand:false,select:false,hideCheckbox:false,unselectable:false,children:null,lastentry:undefined};var q=false,r=function(){if(!q){g.ui.plugin.add("draggable","connectToDynatree",{start:function(a,b){var c=g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null;logMsg("draggable-connectToDynatree.start, %o",d);logMsg("    this: %o",this);logMsg("    event: %o",a);logMsg("    draggable: %o",c);logMsg("    ui: %o",b);if(d){c.offset.click.top=-2;c.offset.click.left=
+16;logMsg("    draggable.offset.click FIXED: %s/%s",c.offset.click.left,c.offset.click.top);return d.tree._onDragEvent("start",d,null,a,b,c)}},drag:function(a,b){var c=g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null,e=b.helper.data("dtTargetNode")||null,f=k(a.target);b.helper.data("dtTargetNode",f);e&&e!==f&&e.tree._onDragEvent("leave",e,d,a,b,c);if(f)if(f.tree.options.dnd.onDrop)f===e?f.tree._onDragEvent("over",f,d,a,b,c):f.tree._onDragEvent("enter",f,d,a,b,c)},stop:function(a,b){var c=
g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null,e=k(a.target);logMsg("draggable-connectToDynatree.stop, %o",d);var f=a.type;logMsg("    type: %o, downEvent: %o, upEvent: %o",f,c._mouseDownEvent,a);(f=f=="mouseup"&&a.which==1)||logMsg("Drag was cancelled");if(e){f&&e.tree._onDragEvent("drop",e,d,a,b,c);e.tree._onDragEvent("leave",e,d,a,b,c)}d&&d.tree._onDragEvent("stop",d,null,a,b,c)}});q=true}}})(jQuery);var _rexDtLibName=/.*dynatree[^/]*\.js$/i;
