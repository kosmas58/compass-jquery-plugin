...
(function(g){function l(a){for(var b=5;a&&b--;){if(a.dtnode)return a.dtnode;a=a.parentNode}return null}function m(a){var b=a.options.dnd||null;if(b&&(b.onDragStart||b.onDrop))s();b&&b.onDragStart&&a.$tree.draggable({addClasses:false,appendTo:"body",containment:false,delay:0,distance:4,revert:false,connectToDynatree:true,helper:function(c){var d=l(c.target);return d.tree._onDragEvent("helper",d,null,c,null,null)},_last:null});b&&b.onDrop&&a.$tree.droppable({addClasses:false,tolerance:"intersect",greedy:false,
...
if(c.className.indexOf(b.node)<0)return null;var d=a.pageX-c.offsetLeft;a=a.pageY-c.offsetTop;for(var e=0;e<c.childNodes.length;e++){var f=c.childNodes[e],h=f.offsetLeft-c.offsetLeft,i=f.offsetTop-c.offsetTop,j=f.clientWidth,k=f.clientHeight;if(d>=h&&d<=h+j&&a>=i&&a<=i+k)if(f.className==b.title)return"title";else if(f.className==b.expander)return"expander";else if(f.className==b.checkbox)return"checkbox";else if(f.className==b.nodeIcon)return"icon"}return"prefix"},getEventTargetType:function(a){var b=
...
h,"ok");else if(h.data.isLazy&&(h.childList===null||h.childList===undefined)){c.logDebug("%s._loadKeyPath(%s) -> reloading %s...",this,a,h);var i=this;h.reloadChildren(function(j,k){if(k){c.logDebug("%s._loadKeyPath(%s) -> reloaded %s.",j,a,j);b.call(c,h,"loaded");j._loadKeyPath(d.join(c.options.keyPathSeparator),b)}else{c.logWarning("%s._loadKeyPath(%s) -> reloadChildren() failed.",i,a);b.call(c,h,"error")}})}else{b.call(c,h,"loaded");h._loadKeyPath(d.join(c.options.keyPathSeparator),b)}return}}c.logWarning("Node not found: "+
...
a.options;this.$tree=a.element;this.timer=null;this.divTree=this.$tree.get(0);a=g(this.divTree).parent().offset();this.parentTop=a.top;this.parentLeft=a.left;m(this)},_load:function(a){var b=this.options;this.bEnableUpdate=true;this._nodeCount=1;this.focusNode=this.activeNode=null;b.rootVisible!==undefined&&_log("warn","Option 'rootVisible' is no longer supported.");b.title!==undefined&&_log("warn","Option 'title' is no longer supported.");if(b.minExpandLevel<1){_log("warn","Option 'minExpandLevel' must be >= 1.");
b.minExpandLevel=1}if(b.classNames!==g.ui.dynatree.prototype.options.classNames)b.classNames=g.extend({},g.ui.dynatree.prototype.options.classNames,b.classNames);if(b.ajaxDefaults!==g.ui.dynatree.prototype.options.ajaxDefaults)b.ajaxDefaults=g.extend({},g.ui.dynatree.prototype.options.ajaxDefaults,b.ajaxDefaults);if(b.dnd!==g.ui.dynatree.prototype.options.dnd)b.dnd=g.extend({},g.ui.dynatree.prototype.options.dnd,b.dnd);b.imagePath||g("script").each(function(){if(this.src.search(/.*dynatree[^\/]*\.js$/i)>=
0){b.imagePath=this.src.indexOf("/")>=0?this.src.slice(0,this.src.lastIndexOf("/"))+"/skin/":"skin/";logMsg("Guessing imagePath from '%s': '%s'",this.src,b.imagePath);return false}});this.persistence=new n(b.cookieId,b.cookie);if(b.persist){g.cookie||_log("warn","Please include jquery.cookie.js to use persistence.");this.persistence.read()}this.logDebug("DynaTree.persistence: %o",this.persistence.toDict());this.cache={tagEmpty:"<span class='"+b.classNames.empty+"'></span>",tagVline:"<span class='"+
b.classNames.vline+"'></span>",tagExpander:"<span class='"+b.classNames.expander+"'></span>",tagConnector:"<span class='"+b.classNames.connector+"'></span>",tagNodeIcon:"<span class='"+b.classNames.nodeIcon+"'></span>",tagCheckbox:"<span class='"+b.classNames.checkbox+"'></span>",lastentry:undefined};if(b.children||b.initAjax&&b.initAjax.url||b.initId)g(this.divTree).empty();else this.divRoot&&g(this.divRoot).remove();this.tnRoot=new p(null,this,{});this.tnRoot.bExpanded=true;this.tnRoot.render();
...
document.getElementById(this.options.idPrefix+a);if(b)return b.dtnode?b.dtnode:null;var c=null;this.visit(function(d){if(d.data.key==a){c=d;return false}},true);return c},getActiveNode:function(){return this.activeNode},reactivate:function(a){var b=this.activeNode;if(b){this.activeNode=null;b.activate();a&&b.focus()}},getSelectedNodes:function(a){var b=[];this.tnRoot.visit(function(c){if(c.bSelected){b.push(c);if(a===true)return"skip"}});return b},activateKey:function(a){a=a===null?null:this.getNodeByKey(a);
if(!a){this.activeNode&&this.activeNode.deactivate();return this.activeNode=null}a.focus();a.activate();return a},loadKeyPath:function(a,b){var c=a.split(this.options.keyPathSeparator);c[0]===""&&c.shift();if(c[0]==this.tnRoot.data.key){this.logDebug("Removed leading root key.");c.shift()}a=c.join(this.options.keyPathSeparator);return this.tnRoot._loadKeyPath(a,b)},selectKey:function(a,b){var c=this.getNodeByKey(a);if(!c)return null;c.select(b);return c},enableUpdate:function(a){if(this.bEnableUpdate==
a)return a;(this.bEnableUpdate=a)&&this.redraw();return!a},visit:function(a,b){return this.tnRoot.visit(a,b)},_createFromTag:function(a,b){var c=this;b.find(">li").each(function(){var d=g(this),e=d.find(">span:first");if(e.length)e=e.html();else{e=d.html();var f=e.search(/<ul/i);e=f>=0?g.trim(e.substring(0,f)):g.trim(e)}e={title:e,isFolder:d.hasClass("folder"),isLazy:d.hasClass("lazy"),expand:d.hasClass("expanded"),select:d.hasClass("selected"),activate:d.hasClass("active"),focus:d.hasClass("focused"),
noLink:d.hasClass("noLink")};if(d.attr("title"))e.tooltip=d.attr("title");if(d.attr("id"))e.key=d.attr("id");if(d.attr("data"))if(f=g.trim(d.attr("data"))){if(f.charAt(0)!="{")f="{"+f+"}";try{g.extend(e,eval("("+f+")"))}catch(h){throw"Error parsing node data: "+h+"\ndata:\n'"+f+"'";}}e=a.addChild(e);d=d.find(">ul:first");d.length&&c._createFromTag(e,d)})},_checkConsistency:function(){},_setDndStatus:function(a,b,c,d,e){a=a?g(a.span):null;b=g(b.span);if(!this.$dndMarker){this.$dndMarker=g("<div id='dynatree-drop-marker'></div>").hide().prependTo(g(this.divTree).parent());
logMsg("Creating marker: %o",this.$dndMarker)}if(d==="after"||d==="before"||d==="over"){var f=b.position();switch(d){case "before":this.$dndMarker.removeClass("dynatree-drop-after dynatree-drop-over");this.$dndMarker.addClass("dynatree-drop-before");f.top-=8;break;case "after":this.$dndMarker.removeClass("dynatree-drop-before dynatree-drop-over");this.$dndMarker.addClass("dynatree-drop-after");f.top+=8;break;default:this.$dndMarker.removeClass("dynatree-drop-after dynatree-drop-before");this.$dndMarker.addClass("dynatree-drop-over");
b.addClass("dynatree-drop-target");f.left+=8}this.$dndMarker.css({left:f.left+"px",top:f.top+"px"}).show()}else{b.removeClass("dynatree-drop-target");this.$dndMarker.hide()}d==="after"?b.addClass("dynatree-drop-after"):b.removeClass("dynatree-drop-after");d==="before"?b.addClass("dynatree-drop-before"):b.removeClass("dynatree-drop-before");if(e===true){a&&a.addClass("dynatree-drop-accept");b.addClass("dynatree-drop-accept");c.addClass("dynatree-drop-accept")}else{a&&a.removeClass("dynatree-drop-accept");
b.removeClass("dynatree-drop-accept");c.removeClass("dynatree-drop-accept")}if(e===false){a&&a.addClass("dynatree-drop-reject");b.addClass("dynatree-drop-reject");c.addClass("dynatree-drop-reject")}else{a&&a.removeClass("dynatree-drop-reject");b.removeClass("dynatree-drop-reject");c.removeClass("dynatree-drop-reject")}},_onDragEvent:function(a,b,c,d,e,f){a!=="over"&&this.logDebug("tree._onDragEvent(%s, %o, %o) - %o",a,b,c,this);var h=this.options.dnd,i=null,j=g(b.span);switch(a){case "helper":c=g("<div class='dynatree-drag-helper'><span class='dynatree-drag-helper-img' /></div>").append(g(d.target).closest("a").clone());
c.data("dtSourceNode",b);logMsg("helper.sourceNode=%o",c.data("dtSourceNode"));i=c;break;case "start":if(b.isStatusNode())i=false;else if(h.onDragStart)i=h.onDragStart(b);if(i===false){this.logDebug("tree.onDragStart() cancelled");e.helper.trigger("mouseup");e.helper.hide()}else j.addClass("dynatree-drag-source");break;case "enter":i=h.onDragEnter?h.onDragEnter(b,c):null;i={over:i!==false&&(i===true||i==="over"||g.inArray("over",i)>=0),before:i!==false&&(i===true||i==="before"||g.inArray("before",
i)>=0),after:i!==false&&(i===true||i==="after"||g.inArray("after",i)>=0)};e.helper.data("enterResponse",i);this.logDebug("helper.enterResponse: %o",i);break;case "over":f=e.helper.data("enterResponse");a=null;if(f===false)break;else if(typeof f==="string")a=f;else{var k=j.position();k.top+=this.parentTop;k.left+=this.parentLeft;d={x:d.pageX-k.left,y:d.pageY-k.top};j={x:d.x/j.width(),y:d.y/j.height()};if(f.after&&j.y>0.75)a="after";else if(!f.over&&f.after&&j.y>0.5)a="after";else if(f.before&&j.y<=
0.25)a="before";else if(!f.over&&f.before&&j.y<=0.5)a="before";else if(f.over)a="over";if(h.preventVoidMoves)if(b===c){logMsg("    drop over source node prevented");a=null}else if(a==="before"&&c&&b===c.getNextSibling()){logMsg("    drop after source node prevented");a=null}else if(a==="after"&&c&&b===c.getPrevSibling()){logMsg("    drop before source node prevented");a=null}else if(a==="over"&&c&&c.parent===b&&c.isLastSibling()){logMsg("    drop last child over own parent prevented");a=null}logMsg("hitMode: %s - %s - %s",
a,b.parent===c,b.isLastSibling());e.helper.data("hitMode",a)}a==="over"&&h.autoExpandMS&&b.hasChildren()!==false&&!b.bExpanded&&b.scheduleAction("expand",h.autoExpandMS);if(a&&h.onDragOver)i=h.onDragOver(b,c,a);this._setDndStatus(c,b,e.helper,a,i!==false);break;case "drop":(a=e.helper.data("hitMode"))&&h.onDrop&&h.onDrop(b,c,a,e,f);break;case "leave":b.scheduleAction("cancel");e.helper.data("enterResponse",null);e.helper.data("hitMode",null);this._setDndStatus(c,b,e.helper,"out",undefined);h.onDragLeave&&
h.onDragLeave(b,c);break;case "stop":j.removeClass("dynatree-drag-source");h.onDragStop&&h.onDragStop(b);break;default:throw"Unsupported drag event: "+a;}return i},cancelDrag:function(){var a=g.ui.ddmanager.current;a&&a.cancel()},lastentry:undefined};g.widget("ui.dynatree",{_init:function(){if(parseFloat(g.ui.version)<1.8){_log("warn","ui.dynatree._init() was called; you should upgrade to jquery.ui.core.js v1.8 or higher.");return this._create()}_log("debug","ui.dynatree._init() was called; no current default functionality.")},
_create:function(){logMsg("Dynatree._create(): version='%s', debugLevel=%o.",q.version,this.options.debugLevel);this.options.event+=".dynatree";this.element.get(0);this.tree=new q(this);this.tree._load();this.tree.logDebug("Dynatree._init(): done.")},bind:function(){function a(e){e=e=g.event.fix(e||window.event);var f=l(e.target);return f?f._onFocus(e):false}var b=this.element,c=this.options;this.unbind();var d="click.dynatree dblclick.dynatree";if(c.keyboard)d+=" keypress.dynatree keydown.dynatree";
b.bind(d,function(e){var f=l(e.target);if(!f)return true;var h=f.tree.phase;f.tree.phase="userEvent";try{f.tree.logDebug("bind(%o): dtnode: %o",e,f);switch(e.type){case "click":return c.onClick&&c.onClick(f,e)===false?false:f._onClick(e);case "dblclick":return c.onDblClick&&c.onDblClick(f,e)===false?false:f._onDblClick(e);case "keydown":return c.onKeydown&&c.onKeydown(f,e)===false?false:f._onKeydown(e);case "keypress":return c.onKeypress&&c.onKeypress(f,e)===false?false:f._onKeypress(e)}}catch(i){f.tree.logWarning("bind(%o): dtnode: %o, error: %o",
...
getSelectedNodes:function(){return this.tree.getSelectedNodes()},lastentry:undefined});if(parseFloat(g.ui.version)<1.8)g.ui.dynatree.getter="getTree getRoot getActiveNode getSelectedNodes";g.ui.dynatree.prototype.options={title:"Dynatree",minExpandLevel:1,imagePath:null,children:null,initId:null,initAjax:null,autoFocus:true,keyboard:true,persist:false,autoCollapse:false,clickFolderMode:3,activeVisible:true,checkbox:false,selectMode:2,fx:null,noLink:false,onClick:null,onDblClick:null,onKeydown:null,
...
nodeLoading:"dynatree-loading",hasChildren:"dynatree-has-children",active:"dynatree-active",selected:"dynatree-selected",expanded:"dynatree-expanded",lazy:"dynatree-lazy",focused:"dynatree-focused",partsel:"dynatree-partsel",lastsib:"dynatree-lastsib"},debugLevel:2,lastentry:undefined};if(parseFloat(g.ui.version)<1.8)g.ui.dynatree.defaults=g.ui.dynatree.prototype.options;g.ui.dynatree.nodedatadefaults={title:null,key:null,isFolder:false,isLazy:false,tooltip:null,icon:null,addClass:null,noLink:false,
activate:false,focus:false,expand:false,select:false,hideCheckbox:false,unselectable:false,children:null,lastentry:undefined};var r=false,s=function(){if(!r){g.ui.plugin.add("draggable","connectToDynatree",{start:function(a,b){var c=g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null;logMsg("draggable-connectToDynatree.start, %s",d);logMsg("    this: %o",this);logMsg("    event: %o",a);logMsg("    draggable: %o",c);logMsg("    ui: %o",b);if(d){c.offset.click.top=-2;c.offset.click.left=
+16;logMsg("    draggable.offset.click FIXED: %s/%s",c.offset.click.left,c.offset.click.top);return d.tree._onDragEvent("start",d,null,a,b,c)}},drag:function(a,b){var c=g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null,e=b.helper.data("dtTargetNode")||null,f=l(a.target);logMsg("getDtNodeFromElement(%o): %s",a.target,f);if(a.target&&!f)if(g(a.target).closest("div.dynatree-drag-helper,#dynatree-drop-marker").length>0){logMsg("Drag event over helper: ignored.");return}b.helper.data("dtTargetNode",
f);e&&e!==f&&e.tree._onDragEvent("leave",e,d,a,b,c);if(f)if(f.tree.options.dnd.onDrop)f===e?f.tree._onDragEvent("over",f,d,a,b,c):f.tree._onDragEvent("enter",f,d,a,b,c)},stop:function(a,b){var c=g(this).data("draggable"),d=b.helper.data("dtSourceNode")||null,e=b.helper.data("dtTargetNode")||null;logMsg("draggable-connectToDynatree.stop: targetNode(from event): %s, dtTargetNode: %s",e,b.helper.data("dtTargetNode"));logMsg("draggable-connectToDynatree.stop, %s",d);var f=a.type;logMsg("    type: %o, downEvent: %o, upEvent: %o",
f,c._mouseDownEvent,a);logMsg("    targetNode: %o",e);(f=f=="mouseup"&&a.which==1)||logMsg("Drag was cancelled");if(e){f&&e.tree._onDragEvent("drop",e,d,a,b,c);e.tree._onDragEvent("leave",e,d,a,b,c)}d&&d.tree._onDragEvent("stop",d,null,a,b,c)}});r=true}}})(jQuery);
