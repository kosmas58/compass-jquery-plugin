...
if(this.elementIsWrapper){e(this.element);var h=this.element;h.after(this.originalElement.css({position:h.css("position"),width:h.outerWidth(),height:h.outerHeight(),top:h.css("top"),left:h.css("left")})).remove()}this.originalElement.css("resize",this.originalResizeStyle);e(this.originalElement);return this},_mouseCapture:function(e){var h=false;for(var b in this.handles)if(a(this.handles[b])[0]==e.target)h=true;return!this.options.disabled&&h},_mouseStart:function(e){var h=this.options,b=this.element.position(),
...
e);break;case h.PAGE_DOWN:d._move("nextPage",e);break;case h.UP:d._move("previous",e);e.preventDefault();break;case h.DOWN:d._move("next",e);e.preventDefault();break;case h.ENTER:case h.NUMPAD_ENTER:d.menu.element.is(":visible")&&e.preventDefault();case h.TAB:if(!d.menu.active)return;d.menu.select(e);break;case h.ESCAPE:d.element.val(d.term);d.close(e);break;default:clearTimeout(d.searching);d.searching=setTimeout(function(){if(d.term!=d.element.val()){d.selectedItem=null;d.search(null,e)}},d.options.delay);
break}}}).bind("focus.autocomplete",function(){if(!d.options.disabled){d.selectedItem=null;d.previous=d.element.val()}}).bind("blur.autocomplete",function(e){if(!d.options.disabled){clearTimeout(d.searching);d.closing=setTimeout(function(){d.close(e);d._change(e)},150)}});this._initSource();this.response=function(){return d._response.apply(d,arguments)};this.menu=a("<ul></ul>").addClass("ui-autocomplete").appendTo(a(this.options.appendTo||"body",f)[0]).mousedown(function(e){var h=d.menu.element[0];
e.target===h&&setTimeout(function(){a(document).one("mousedown",function(b){b.target!==d.element[0]&&b.target!==h&&!a.ui.contains(h,b.target)&&d.close()})},1);setTimeout(function(){clearTimeout(d.closing)},13)}).menu({focus:function(e,h){var b=h.item.data("item.autocomplete");false!==d._trigger("focus",null,{item:b})&&/^key/.test(e.originalEvent.type)&&d.element.val(b.value)},selected:function(e,h){var b=h.item.data("item.autocomplete"),g=d.previous;if(d.element[0]!==f.activeElement){d.element.focus();
d.previous=g}if(false!==d._trigger("select",e,{item:b})){d.term=b.value;d.element.val(b.value)}d.close(e);d.selectedItem=b},blur:function(){d.menu.element.is(":visible")&&d.element.val()!==d.term&&d.element.val(d.term)}}).zIndex(this.element.zIndex()+1).css({top:0,left:0}).hide().data("menu");a.fn.bgiframe&&this.menu.element.bgiframe()},destroy:function(){this.element.removeClass("ui-autocomplete-input").removeAttr("autocomplete").removeAttr("role").removeAttr("aria-autocomplete").removeAttr("aria-haspopup");
this.menu.element.remove();a.Widget.prototype.destroy.call(this)},_setOption:function(d,f){a.Widget.prototype._setOption.apply(this,arguments);d==="source"&&this._initSource();if(d==="appendTo")this.menu.element.appendTo(a(f||"body",this.element[0].ownerDocument)[0])},_initSource:function(){var d=this,f,e;if(a.isArray(this.options.source)){f=this.options.source;this.source=function(h,b){b(a.ui.autocomplete.filter(f,h.term))}}else if(typeof this.options.source==="string"){e=this.options.source;this.source=
function(h,b){d.xhr&&d.xhr.abort();d.xhr=a.getJSON(e,h,function(g,i,k){k===d.xhr&&b(g);d.xhr=null})}}else this.source=this.options.source},search:function(d,f){d=d!=null?d:this.element.val();this.term=this.element.val();if(d.length<this.options.minLength)return this.close(f);clearTimeout(this.closing);if(this._trigger("search")!==false)return this._search(d)},_search:function(d){this.element.addClass("ui-autocomplete-loading");this.source({term:d},this.response)},_response:function(d){if(d.length){d=
...
"string")return{label:f,value:f};return a.extend({label:f.label||f.value,value:f.value||f.label},f)})},_suggest:function(d){var f=this.menu.element.empty().zIndex(this.element.zIndex()+1),e;this._renderMenu(f,d);this.menu.deactivate();this.menu.refresh();this.menu.element.show().position(a.extend({of:this.element},this.options.position));d=f.width("").outerWidth();e=this.element.outerWidth();f.outerWidth(Math.max(d,e))},_renderMenu:function(d,f){var e=this;a.each(f,function(h,b){e._renderItem(d,b)})},
_renderItem:function(d,f){return a("<li></li>").data("item.autocomplete",f).append(a("<a></a>").text(f.label)).appendTo(d)},_move:function(d,f){if(this.menu.element.is(":visible"))if(this.menu.first()&&/^previous/.test(d)||this.menu.last()&&/^next/.test(d)){this.element.val(this.term);this.menu.deactivate()}else this.menu[d](f);else this.search(null,f)},widget:function(){return this.menu.element}});a.extend(a.ui.autocomplete,{escapeRegex:function(d){return d.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&")},
filter:function(d,f){var e=RegExp(a.ui.autocomplete.escapeRegex(f),"i");return a.grep(d,function(h){return e.test(h.label||h.value||h)})}})})(jQuery);
...
-1).mouseenter(function(f){d.activate(f,a(this).parent())}).mouseleave(function(){d.deactivate()})},activate:function(d,f){this.deactivate();if(this.hasScroll()){var e=f.offset().top-this.element.offset().top,h=this.element.attr("scrollTop"),b=this.element.height();if(e<0)this.element.attr("scrollTop",h+e);else e>=b&&this.element.attr("scrollTop",h+e-b+f.height())}this.active=f.eq(0).children("a").addClass("ui-state-hover").attr("id","ui-active-menuitem").end();this._trigger("focus",d,{item:f})},
deactivate:function(){if(this.active){this.active.children("a").removeClass("ui-state-hover").removeAttr("id");this._trigger("blur");this.active=null}},next:function(d){this.move("next",".ui-menu-item:first",d)},previous:function(d){this.move("prev",".ui-menu-item:last",d)},first:function(){return this.active&&!this.active.prevAll(".ui-menu-item").length},last:function(){return this.active&&!this.active.nextAll(".ui-menu-item").length},move:function(d,f,e){if(this.active){d=this.active[d+"All"](".ui-menu-item").eq(0);
d.length?this.activate(e,d):this.activate(e,this.element.children(f))}else this.activate(e,this.element.children(f))},nextPage:function(d){if(this.hasScroll())if(!this.active||this.last())this.activate(d,this.element.children(":first"));else{var f=this.active.offset().top,e=this.element.height(),h=this.element.children("li").filter(function(){var b=a(this).offset().top-f-e+a(this).height();return b<10&&b>-10});h.length||(h=this.element.children(":last"));this.activate(d,h)}else this.activate(d,this.element.children(!this.active||
this.last()?":first":":last"))},previousPage:function(d){if(this.hasScroll())if(!this.active||this.first())this.activate(d,this.element.children(":last"));else{var f=this.active.offset().top,e=this.element.height();result=this.element.children("li").filter(function(){var h=a(this).offset().top-f+e-a(this).height();return h<10&&h>-10});result.length||(result=this.element.children(":first"));this.activate(d,result)}else this.activate(d,this.element.children(!this.active||this.first()?":last":":first"))},
hasScroll:function(){return this.element.height()<this.element.attr("scrollHeight")},select:function(d){this._trigger("selected",d,{item:this.active})}})})(jQuery);
...
"",I=0;I<j[1];I++){var M=this._daylightSavingAdjust(new Date(s,n,b.selectedDay)),A=" ui-corner-all",E="";if(p){E+='<div class="ui-datepicker-group';if(j[1]>1)switch(I){case 0:E+=" ui-datepicker-group-first";A=" ui-corner-"+(i?"right":"left");break;case j[1]-1:E+=" ui-datepicker-group-last";A=" ui-corner-"+(i?"left":"right");break;default:E+=" ui-datepicker-group-middle";A="";break}E+='">'}E+='<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix'+A+'">'+(/all|left/.test(A)&&H==0?i?
m:v:"")+(/all|right/.test(A)&&H==0?i?v:m:"")+this._generateMonthYearHeader(b,n,s,r,u,H>0||I>0,C,B)+'</div><table class="ui-datepicker-calendar"><thead><tr>';var F=o?'<th class="ui-datepicker-week-col">'+this._get(b,"weekHeader")+"</th>":"";for(A=0;A<7;A++){var z=(A+l)%7;F+="<th"+((A+l+6)%7>=5?' class="ui-datepicker-week-end"':"")+'><span title="'+w[z]+'">'+y[z]+"</span></th>"}E+=F+"</tr></thead><tbody>";F=this._getDaysInMonth(s,n);if(s==b.selectedYear&&n==b.selectedMonth)b.selectedDay=Math.min(b.selectedDay,
...
break;case "resizable":b&&!e&&h.resizable("destroy");b&&typeof e==="string"&&h.resizable("option","handles",e);!b&&e!==false&&this._makeResizable(e);break;case "title":a(".ui-dialog-title",this.uiDialogTitlebar).html(""+(e||"&#160;"));break;case "width":g=true;break}a.Widget.prototype._setOption.apply(this,arguments);g&&this._size()},_size:function(){var f=this.options,e;this.element.css({width:"auto",minHeight:0,height:0});if(f.minWidth>f.width)f.width=f.minWidth;e=this.uiDialog.css({height:"auto",
width:f.width}).height();this.element.css(f.height==="auto"?{minHeight:Math.max(f.minHeight-e,0),height:a.support.minHeight?"auto":Math.max(f.minHeight-e,0)}:{minHeight:0,height:Math.max(f.height-e,0)}).show();this.uiDialog.is(":data(resizable)")&&this.uiDialog.resizable("option","minHeight",this._minHeight())}});a.extend(a.ui.dialog,{version:"1.8.5",uuid:0,maxZ:0,getTitleId:function(f){f=f.attr("id");if(!f){this.uuid+=1;f=this.uuid}return"ui-dialog-title-"+f},overlay:function(f){this.$el=a.ui.dialog.overlay.create(f)}});
...
false;if(!d._keySliding){d._keySliding=true;a(this).addClass("ui-state-active");g=d._start(e,b);if(g===false)return}break}k=d.options.step;g=d.options.values&&d.options.values.length?i=d.values(b):i=d.value();switch(e.keyCode){case a.ui.keyCode.HOME:i=d._valueMin();break;case a.ui.keyCode.END:i=d._valueMax();break;case a.ui.keyCode.PAGE_UP:i=d._trimAlignValue(g+(d._valueMax()-d._valueMin())/5);break;case a.ui.keyCode.PAGE_DOWN:i=d._trimAlignValue(g-(d._valueMax()-d._valueMin())/5);break;case a.ui.keyCode.UP:case a.ui.keyCode.RIGHT:if(g===
d._valueMax())return;i=d._trimAlignValue(g+k);break;case a.ui.keyCode.DOWN:case a.ui.keyCode.LEFT:if(g===d._valueMin())return;i=d._trimAlignValue(g-k);break}d._slide(e,b,i);return h}}).keyup(function(e){var h=a(this).data("index.ui-slider-handle");if(d._keySliding){d._keySliding=false;d._stop(e,h);d._change(e,h);a(this).removeClass("ui-state-active")}});this._refreshValue();this._animateOff=false},destroy:function(){this.handles.remove();this.range.remove();this.element.removeClass("ui-slider ui-slider-horizontal ui-slider-vertical ui-slider-disabled ui-widget ui-widget-content ui-corner-all").removeData("slider").unbind(".slider");
...
!m.startselected){m.$element.removeClass("ui-selected");m.selected=false;m.$element.addClass("ui-unselecting");m.unselecting=true;f._trigger("unselecting",d,{unselecting:m.element})}}}});return false}},_mouseStop:function(d){var f=this;this.dragged=false;a(".ui-unselecting",this.element[0]).each(function(){var e=a.data(this,"selectable-item");e.$element.removeClass("ui-unselecting");e.unselecting=false;e.startselected=false;f._trigger("unselected",d,{unselected:e.element})});a(".ui-selecting",this.element[0]).each(function(){var e=
a.data(this,"selectable-item");e.$element.removeClass("ui-selecting").addClass("ui-selected");e.selecting=false;e.selected=true;e.startselected=true;f._trigger("selected",d,{selected:e.element})});this._trigger("stop",d);this.helper.remove();return false}});a.extend(a.ui.selectable,{version:"1.8.5"})})(jQuery);
(function(a){a.widget("ui.selectmenu",{_init:function(){var d=this,f=this.options;this.ids=[this.element.attr("id")+"-button",this.element.attr("id")+"-menu"];this._safemouseup=true;this.newelement=a('<a class="'+this.widgetBaseClass+' ui-widget ui-state-default ui-corner-all" id="'+this.ids[0]+'" role="button" href="#" aria-haspopup="true" aria-owns="'+this.ids[1]+'"></a>').insertAfter(this.element);var e=this.element.attr("tabindex");e&&this.newelement.attr("tabindex",e);this.newelement.data("selectelement",
this.element);this.selectmenuIcon=a('<span class="'+this.widgetBaseClass+'-icon ui-icon"></span>').prependTo(this.newelement).addClass(f.style=="popup"?"ui-icon-triangle-2-n-s":"ui-icon-triangle-1-s");a("label[for="+this.element.attr("id")+"]").attr("for",this.ids[0]).bind("click",function(){d.newelement[0].focus();return false});this.newelement.bind("mousedown",function(m){d._toggle(m);if(f.style=="popup"){d._safemouseup=false;setTimeout(function(){d._safemouseup=true},300)}return false}).bind("click",
function(){return false}).keydown(function(m){var l=true;switch(m.keyCode){case a.ui.keyCode.ENTER:l=true;break;case a.ui.keyCode.SPACE:l=false;d._toggle(m);break;case a.ui.keyCode.UP:case a.ui.keyCode.LEFT:l=false;d._moveSelection(-1);break;case a.ui.keyCode.DOWN:case a.ui.keyCode.RIGHT:l=false;d._moveSelection(1);break;case a.ui.keyCode.TAB:l=true;break;default:l=false;d._typeAhead(m.keyCode,"mouseup");break}return l}).bind("mouseover focus",function(){a(this).addClass(d.widgetBaseClass+"-focus ui-state-hover")}).bind("mouseout blur",
function(){a(this).removeClass(d.widgetBaseClass+"-focus ui-state-hover")});a(document).mousedown(function(m){d.close(m)});this.element.click(function(){this._refreshValue()}).focus(function(){this.newelement[0].focus()});this.list=a('<ul class="'+d.widgetBaseClass+"-menu ui-widget ui-widget-content"+(f.style=="dropdown"?" ui-corner-bottom":" ui-corner-all")+'" aria-hidden="true" role="listbox" aria-labelledby="'+this.ids[0]+'" id="'+this.ids[1]+'"></ul>').appendTo("body");var h=[];this.element.find("option").each(function(){h.push({value:a(this).attr("value"),
text:d._formatText(jQuery(this).text()),selected:a(this).attr("selected"),classes:a(this).attr("class"),parentOptGroup:a(this).parent("optgroup").attr("label")})});var b=d.options.style=="popup"?" ui-state-active":"";for(var g in h){e=a('<li role="presentation"><a href="#" tabindex="-1" role="option" aria-selected="false">'+h[g].text+"</a></li>").data("index",g).addClass(h[g].classes).data("optionClasses",h[g].classes||"").mouseup(function(m){if(d._safemouseup){var l=a(this).data("index")!=d._selectedIndex();
d.value(a(this).data("index"));d.select(m);l&&d.change(m);d.close(m,true)}return false}).click(function(){return false}).bind("mouseover focus",function(){d._selectedOptionLi().addClass(b);d._focusedOptionLi().removeClass(d.widgetBaseClass+"-item-focus ui-state-hover");a(this).removeClass("ui-state-active").addClass(d.widgetBaseClass+"-item-focus ui-state-hover")}).bind("mouseout blur",function(){a(this).is(d._selectedOptionLi())&&a(this).addClass(b);a(this).removeClass(d.widgetBaseClass+"-item-focus ui-state-hover")});
if(h[g].parentOptGroup){var i=d.widgetBaseClass+"-group-"+h[g].parentOptGroup;this.list.find("li."+i).size()?this.list.find("li."+i+":last ul").append(e):a('<li role="presentation" class="'+d.widgetBaseClass+"-group "+i+'"><span class="'+d.widgetBaseClass+'-group-label">'+h[g].parentOptGroup+"</span><ul></ul></li>").appendTo(this.list).find("ul").append(e)}else e.appendTo(this.list);this.list.bind("mousedown mouseup",function(){return false});if(f.icons)for(var k in f.icons)if(e.is(f.icons[k].find)){e.data("optionClasses",
h[g].classes+" "+d.widgetBaseClass+"-hasIcon").addClass(d.widgetBaseClass+"-hasIcon");i=f.icons[k].icon||"";e.find("a:eq(0)").prepend('<span class="'+d.widgetBaseClass+"-item-icon ui-icon "+i+'"></span>')}}this.list.find("li:last").addClass("ui-corner-bottom");f.style=="popup"&&this.list.find("li:first").addClass("ui-corner-top");if(f.transferClasses){g=this.element.attr("class")||"";this.newelement.add(this.list).addClass(g)}g=this.element.width();this.newelement.width(f.width?f.width:g);f.style==
"dropdown"?this.list.width(f.menuWidth?f.menuWidth:f.width?f.width:g):this.list.width(f.menuWidth?f.menuWidth:f.width?f.width-f.handleWidth:g-f.handleWidth);f.maxHeight&&f.maxHeight<this.list.height()&&this.list.height(f.maxHeight);this._optionLis=this.list.find("li:not(."+d.widgetBaseClass+"-group)");this.list.keydown(function(m){var l=true;switch(m.keyCode){case a.ui.keyCode.UP:case a.ui.keyCode.LEFT:l=false;d._moveFocus(-1);break;case a.ui.keyCode.DOWN:case a.ui.keyCode.RIGHT:l=false;d._moveFocus(1);
break;case a.ui.keyCode.HOME:l=false;d._moveFocus(":first");break;case a.ui.keyCode.PAGE_UP:l=false;d._scrollPage("up");break;case a.ui.keyCode.PAGE_DOWN:l=false;d._scrollPage("down");break;case a.ui.keyCode.END:l=false;d._moveFocus(":last");break;case a.ui.keyCode.ENTER:case a.ui.keyCode.SPACE:l=false;d.close(m,true);a(m.target).parents("li:eq(0)").trigger("mouseup");break;case a.ui.keyCode.TAB:l=true;d.close(m,true);break;case a.ui.keyCode.ESCAPE:l=false;d.close(m,true);break;default:l=false;d._typeAhead(m.keyCode,
"focus");break}return l});if(f.style=="dropdown"){this.newelement.addClass(d.widgetBaseClass+"-dropdown");this.list.addClass(d.widgetBaseClass+"-menu-dropdown")}else{this.newelement.addClass(d.widgetBaseClass+"-popup");this.list.addClass(d.widgetBaseClass+"-menu-popup")}this.newelement.prepend('<span class="'+d.widgetBaseClass+'-status">'+h[this._selectedIndex()].text+"</span>");this.element.hide();this.element.attr("disabled")==true&&this.disable();this.value(this._selectedIndex())},destroy:function(){this.element.removeData(this.widgetName).removeClass(this.widgetBaseClass+
"-disabled "+this.namespace+"-state-disabled").removeAttr("aria-disabled");a("label[for="+this.newelement.attr("id")+"]").attr("for",this.element.attr("id")).unbind("click");this.newelement.remove();this.list.remove();this.element.show()},_typeAhead:function(d,f){var e=this;if(!e._prevChar)e._prevChar=["",0];var h=String.fromCharCode(d);c=h.toLowerCase();var b=false;this.list.find("li a").each(function(g){if(!b){var i=a(this).text();if(i.indexOf(h)==0||i.indexOf(c)==0)if(e._prevChar[0]==h){if(e._prevChar[1]<
g){b=true;a(this).trigger(f);e._prevChar[1]=g}}else{b=true;a(this).trigger(f);e._prevChar[1]=g}}});this._prevChar[0]=h},_uiHash:function(){return{value:this.value()}},open:function(d){if(this.newelement.attr("aria-disabled")!="true"){this._refreshPosition();this._closeOthers(d);this.newelement.addClass("ui-state-active");this.list.appendTo("body").addClass(this.widgetBaseClass+"-open").attr("aria-hidden",false).find("li:not(."+this.widgetBaseClass+"-group):eq("+this._selectedIndex()+") a")[0].focus();
this.options.style=="dropdown"&&this.newelement.removeClass("ui-corner-all").addClass("ui-corner-top");this._refreshPosition();this._trigger("open",d,this._uiHash())}},close:function(d,f){if(this.newelement.is(".ui-state-active")){this.newelement.removeClass("ui-state-active");this.list.attr("aria-hidden",true).removeClass(this.widgetBaseClass+"-open");this.options.style=="dropdown"&&this.newelement.removeClass("ui-corner-top").addClass("ui-corner-all");f&&this.newelement[0].focus();this._trigger("close",
d,this._uiHash())}},change:function(d){this.element.trigger("change");this._trigger("change",d,this._uiHash())},select:function(d){this._trigger("select",d,this._uiHash())},_closeOthers:function(d){a("."+this.widgetBaseClass+".ui-state-active").not(this.newelement).each(function(){a(this).data("selectelement").selectmenu("close",d)});a("."+this.widgetBaseClass+".ui-state-hover").trigger("mouseout")},_toggle:function(d,f){this.list.is("."+this.widgetBaseClass+"-open")?this.close(d,f):this.open(d)},
_formatText:function(d){return this.options.format?this.options.format(d):d},_selectedIndex:function(){return this.element[0].selectedIndex},_selectedOptionLi:function(){return this._optionLis.eq(this._selectedIndex())},_focusedOptionLi:function(){return this.list.find("."+this.widgetBaseClass+"-item-focus")},_moveSelection:function(d){return this._optionLis.eq(parseInt(this._selectedOptionLi().data("index"),10)+d).trigger("mouseup")},_moveFocus:function(d){d=isNaN(d)?parseInt(this._optionLis.filter(d).data("index"),
10):parseInt(this._focusedOptionLi().data("index"),10)+d;if(d<0)d=0;if(d>this._optionLis.size()-1)d=this._optionLis.size()-1;var f=this.widgetBaseClass+"-item-"+Math.round(Math.random()*1E3);this._focusedOptionLi().find("a:eq(0)").attr("id","");this._optionLis.eq(d).find("a:eq(0)").attr("id",f)[0].focus();this.list.attr("aria-activedescendant",f)},_scrollPage:function(d){var f=Math.floor(this.list.outerHeight()/this.list.find("li:first").outerHeight());f=d=="up"?-f:f;this._moveFocus(f)},_setData:function(d,
f){this.options[d]=f;if(d=="disabled"){this.close();this.element.add(this.newelement).add(this.list)[f?"addClass":"removeClass"](this.widgetBaseClass+"-disabled "+this.namespace+"-state-disabled").attr("aria-disabled",f)}},value:function(d){if(arguments.length){this.element[0].selectedIndex=d;this._refreshValue();this._refreshPosition()}return this.element[0].selectedIndex},_refreshValue:function(){var d=this.options.style=="popup"?" ui-state-active":"",f=this.widgetBaseClass+"-item-"+Math.round(Math.random()*
1E3);this.list.find("."+this.widgetBaseClass+"-item-selected").removeClass(this.widgetBaseClass+"-item-selected"+d).find("a").attr("aria-selected","false").attr("id","");this._selectedOptionLi().addClass(this.widgetBaseClass+"-item-selected"+d).find("a").attr("aria-selected","true").attr("id",f);d=this.newelement.data("optionClasses")?this.newelement.data("optionClasses"):"";var e=this._selectedOptionLi().data("optionClasses")?this._selectedOptionLi().data("optionClasses"):"";this.newelement.removeClass(d).data("optionClasses",
e).addClass(e).find("."+this.widgetBaseClass+"-status").html(this._selectedOptionLi().find("a:eq(0)").html());this.list.attr("aria-activedescendant",f)},_refreshPosition:function(){this.list.css("left",this.newelement.offset().left);var d=this.newelement.offset().top,f=this.list[0].scrollTop;this.list.find("li:lt("+this._selectedIndex()+")").each(function(){f-=a(this).outerHeight()});d+=this.newelement.is("."+this.widgetBaseClass+"-popup")?f:this.newelement.height();this.list.css("top",d)}});a.extend(a.ui.selectmenu,
