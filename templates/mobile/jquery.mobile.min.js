 ...
 * http://jquery.org/license
 */
(function(c,b,d){c.extend(c.mobile,{ns:"",subPageUrlKey:"ui-page",nonHistorySelectors:"dialog",activePageClass:"ui-page-active",activeBtnClass:"ui-btn-active",ajaxEnabled:true,hashListeningEnabled:true,defaultPageTransition:"slide",defaultDialogTransition:"pop",loadingMessage:"loading",pageLoadErrorMessage:"Error Loading Page",gradeA:function(){return c.support.mediaquery||c.mobile.browser.ie&&c.mobile.browser.ie>=7},keyCode:{ALT:18,BACKSPACE:8,CAPS_LOCK:20,COMMA:188,COMMAND:91,COMMAND_LEFT:91,COMMAND_RIGHT:93,CONTROL:17,DELETE:46,DOWN:40,END:35,ENTER:13,ESCAPE:27,HOME:36,INSERT:45,LEFT:37,MENU:93,NUMPAD_ADD:107,NUMPAD_DECIMAL:110,NUMPAD_DIVIDE:111,NUMPAD_ENTER:108,NUMPAD_MULTIPLY:106,NUMPAD_SUBTRACT:109,PAGE_DOWN:34,PAGE_UP:33,PERIOD:190,RIGHT:39,SHIFT:16,SPACE:32,TAB:9,UP:38,WINDOWS:91},silentScroll:function(e){e=e||0;c.event.special.scrollstart.enabled=false;setTimeout(function(){b.scrollTo(0,e);c(document).trigger("silentscroll",{x:0,y:e})},20);setTimeout(function(){c.event.special.scrollstart.enabled=true},150)},normalizeRegex:/-([a-z])/g,nsNormalize:function(e){if(!e){return}return c.camelCase(c.mobile.ns+e)}});c.fn.jqmData=function(f,e){return this.data(f?c.mobile.nsNormalize(f):f,e)};c.jqmData=function(e,g,f){return c.data(e,c.mobile.nsNormalize(g),f)};c.fn.jqmRemoveData=function(e){return this.removeData(c.mobile.nsNormalize(e))};c.jqmRemoveData=function(e,f){return c.removeData(e,c.mobile.nsNormalize(f))};c.jqmHasData=function(e,f){return c.hasData(e,c.mobile.nsNormalize(f))};var a=c.find;c.find=function(f,h,g,e){f=f.replace(/:jqmData\(([^)]*)\)/g,"[data-"+(c.mobile.ns||"")+"$1]");return a.call(this,f,h,g,e)};c.extend(c.find,a);c.find.matches=function(e,f){return c.find(e,null,null,f)};c.find.matchesSelector=function(e,f){return c.find(f,null,null,[e]).length>0}})(jQuery,this);(function(b,e){var k=b(window),o=b("html"),i=b("head"),m={get:function(x){if(x===e){x=location.hash}return m.stripHash(x).replace(/[^\/]*\.[^\/*]+$/,"")},getFilePath:function(y){var x="&"+b.mobile.subPageUrlKey;return y&&y.split(x)[0].split(v)[0]},set:function(x){location.hash=x},origin:"",setOrigin:function(){m.origin=m.get(location.protocol+"//"+location.host+location.pathname)},makeAbsolute:function(y){var x=m.isPath(location.hash);if(m.isQuery(y)){return(x?m.cleanHash(location.hash):location.pathname)+y}return(x?m.get():m.get(location.pathname))+y},isPath:function(x){return/\//.test(x)},isQuery:function(x){return/^\?/.test(x)},clean:function(x){var y=new RegExp("^"+location.protocol+"//"+location.host);return x.replace(y,"")},stripHash:function(x){return x.replace(/^#/,"")},cleanHash:function(x){return m.stripHash(x.replace(/\?.*$/,"").replace(v,""))},isExternal:function(x){return m.hasProtocol(m.clean(x))},hasProtocol:function(x){return(/^(:?\w+:)/).test(x)},isRelative:function(x){return(/^[^\/|#]/).test(x)&&!m.hasProtocol(x)},isEmbeddedPage:function(x){return(/^#/).test(x)}},w=null,a={stack:[],activeIndex:0,getActive:function(){return a.stack[a.activeIndex]},getPrev:function(){return a.stack[a.activeIndex-1]},getNext:function(){return a.stack[a.activeIndex+1]},addNew:function(y,A,z,x){if(a.getNext()){a.clearForward()}a.stack.push({url:y,transition:A,title:z,page:x});a.activeIndex=a.stack.length-1},clearForward:function(){a.stack=a.stack.slice(0,a.activeIndex+1)},directHashChange:function(z){var x,y,A;b.each(a.stack,function(B,C){if(z.currentUrl===C.url){x=B<a.activeIndex;y=!x;A=B}});this.activeIndex=A!==e?A:this.activeIndex;if(x){z.isBack()}else{if(y){z.isForward()}}},ignoreNextHashChange:false},q="[tabindex],a,button:visible,select:visible,input",j=[],r=false,v="&ui-state=dialog",h=i.children("base"),d=location.protocol+"//"+location.host,n=d+location.pathname;if(h.length){var s=h.attr("href");if(s){if(s.search(/^[^:\/]+:\/\/[^\/]+\/?/)===-1){d=d+s}else{d=s}}d=d+(d.charAt(d.length-1)==="/"?" ":"/")}var c=b.support.dynamicBaseTag?{element:(h.length?h:b("<base>",{href:n}).prependTo(i)),set:function(x){c.element.attr("href",d+m.get(x))},reset:function(){c.element.attr("href",n)}}:e;m.setOrigin();function g(y){var z=y.jqmData("lastClicked");if(z&&z.length){z.focus()}else{var x=y.find(".ui-title:eq(0)");if(x.length){x.focus()}else{y.find(q).eq(0).focus()}}}function u(x){if(!!w&&(!w.closest(".ui-page-active").length||x)){w.removeClass(b.mobile.activeBtnClass)}w=null}function l(){r=false;if(j.length>0){b.mobile.changePage.apply(null,j.pop())}}function t(A,B,D,x){b.mobile.silentScroll();var y=k.scrollTop();if(B){B.jqmData("lastScroll",y).jqmData("lastClicked",w);B.data("page")._trigger("beforehide",null,{nextPage:A})}A.data("page")._trigger("beforeshow",null,{prevPage:B||b("")});b.mobile.hidePageLoadingMsg();var z=b.mobile.transitionHandlers[D||"none"]||b.mobile.defaultTransitionHandler,C=z(D,x,A,B);C.done(function(){b.mobile.silentScroll(A.jqmData("lastScroll")||0);b(document).one("silentscroll",function(){g(A)});if(B){B.data("page")._trigger("hide",null,{nextPage:A})}A.data("page")._trigger("show",null,{prevPage:B||b("")})});return C}function p(x,y){if(y){x.attr("data-"+b.mobile.ns+"role",y)}x.page()}b.fn.animationComplete=function(x){if(b.support.cssTransitions){return b(this).one("webkitAnimationEnd",x)}else{setTimeout(x,0);return b(this)}};b.mobile.updateHash=m.set;b.mobile.path=m;b.mobile.base=c;b.mobile.urlstack=a.stack;b.mobile.urlHistory=a;b.mobile.noneTransitionHandler=function(y,x,A,z){if(z){z.removeClass(b.mobile.activePageClass)}A.addClass(b.mobile.activePageClass);return b.Deferred().resolve(y,x,A,z).promise()};b.mobile.defaultTransitionHandler=b.mobile.noneTransitionHandler;b.mobile.transitionHandlers={none:b.mobile.defaultTransitionHandler};b.mobile.allowCrossDomainPages=false;b.mobile.loadPage=function(B,A){var z=b.Deferred(),D=b.extend({},b.mobile.loadPage.defaults,A),C=m.stripHash(B),y=m.getFilePath(C),E=null,x=null;D.pageContainer=D.pageContainer||b.mobile.pageContainer;if(D.data&&D.type==="get"){if(b.type(D.data)==="object"){D.data=b.param(D.data)}C+="?"+D.data;D.data=e;y=m.getFilePath(C)}E=D.pageContainer.children(":jqmData(url='"+C+"')");if(c){c.reset()}if(E.length){if(!D.reloadPage){p(E,D.role);z.resolve(C,A,E);return z.promise()}x=E}if(D.showLoadMsg){b.mobile.showPageLoadingMsg()}b.ajax({url:y,type:D.type,data:D.data,dataType:"html",success:function(H){var I=b("<div></div>"),F=H.match(/<title[^>]*>([^<]*)/)&&RegExp.$1,K=new RegExp(".*(<[^>]+\\bdata-"+b.mobile.ns+"role=[\"']?page[\"']?[^>]*>).*"),G=new RegExp("\\bdata-"+b.mobile.ns+"url=[\"']?([^\"'>]*)[\"']?");if(K.test(H)&&RegExp.$1&&G.test(RegExp.$1)&&RegExp.$1){B=y=m.getFilePath(RegExp.$1)}if(c){c.set(y)}I.get(0).innerHTML=H;E=I.find(":jqmData(role='page'), :jqmData(role='dialog')").first();if(F&&!E.jqmData("title")){E.jqmData("title",F)}if(!b.support.dynamicBaseTag){var J=m.get(y);E.find("[src], link[href], a[rel='external'], :jqmData(ajax='false'), a[target]").each(function(){var M=b(this).is("[href]")?"href":b(this).is("[src]")?"src":"action",L=b(this).attr(M);L=L.replace(location.protocol+"//"+location.host+location.pathname,"");if(!/^(\w+:|#|\/)/.test(L)){b(this).attr(M,J+L)}})}E.attr("data-"+b.mobile.ns+"url",y).appendTo(D.pageContainer);p(E,D.role);if(C.indexOf("&"+b.mobile.subPageUrlKey)>-1){E=D.pageContainer.children(":jqmData(url='"+C+"')")}if(D.showLoadMsg){b.mobile.hidePageLoadingMsg()}z.resolve(C,A,E,x)},error:function(){if(c){c.set(m.get())}if(D.showLoadMsg){b.mobile.hidePageLoadingMsg();b("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>"+b.mobile.pageLoadErrorMessage+"</h1></div>").css({display:"block",opacity:0.96,top:k.scrollTop()+100}).appendTo(D.pageContainer).delay(800).fadeOut(400,function(){b(this).remove()})}z.reject(C,A)}});return z.promise()};b.mobile.loadPage.defaults={type:"get",data:e,reloadPage:false,role:"page",showLoadMsg:true,pageContainer:e};b.mobile.changePage=function(y,x){if(typeof x!=="object"){var G=null;if(typeof y==="object"&&y.url&&y.type){G={type:y.type,data:y.data,forcePageLoad:true};y=y.url}var J=arguments.length;if(J>1){var O=["transition","reverse","changeHash","fromHashChange"],I;for(I=1;I<J;I++){var M=arguments[I];if(typeof M!=="undefined"){G=G||{};G[O[I-1]]=M}}}if(G){return b.mobile.changePage(y,G)}}if(r){j.unshift(arguments);return}r=true;var K=b.extend({},b.mobile.changePage.defaults,x);K.pageContainer=K.pageContainer||b.mobile.pageContainer;if(typeof y=="string"){var H=b.mobile.loadPage(y,K);if(H){H.done(function(R,Q,S,P){r=false;Q.duplicateCachedPage=P;b.mobile.changePage(S,Q)}).fail(function(Q,P){r=false;u(true);l()});return}}var D=K.pageContainer,N=b.mobile.activePage,z=y.jqmData("url"),F=m.getFilePath(z),C=a.getActive(),E=0,A=document.title,B=K.role==="dialog"||y.jqmData("role")==="dialog";D.trigger("beforechangepage");if(C&&C.page[0]===y[0]){r=false;return}p(y,K.role);if(K.fromHashChange){a.directHashChange({currentUrl:z,isBack:function(){E=-1},isForward:function(){E=1}})}b(document.activeElement||"").add("input:focus, textarea:focus, select:focus").blur();if(B&&C){z=C.url+v}if(K.changeHash!==false&&z){a.ignoreNextHashChange=true;m.set(z)}var L=y.jqmData("title")||y.children(":jqmData(role='header')").find(".ui-title").text();if(!!L&&A==document.title){A=L}if(!E){a.addNew(z,K.transition,A,y)}document.title=a.getActive().title;b.mobile.activePage=y;K.transition=K.transition||(E?C.transition:e)||(K.role==="dialog"?b.mobile.defaultDialogTransition:b.mobile.defaultPageTransition);K.reverse=K.reverse||E<0;var H=t(y,N,K.transition,K.reverse,K.changeHash);H.done(function(){u();if(K.duplicateCachedPage){K.duplicateCachedPage.remove()}o.removeClass("ui-mobile-rendering");l();D.trigger("changepage")})};b.mobile.changePage.defaults={transition:e,reverse:false,changeHash:true,fromHashChange:false,role:"page",duplicateCachedPage:e,pageContainer:e};b("form").live("submit",function(z){var B=b(this);if(!b.mobile.ajaxEnabled||B.is(":jqmData(ajax='false')")){return}var y=B.attr("method"),x=m.clean(B.attr("action")),A=B.attr("target");if(m.isExternal(x)||A){return}if(m.isRelative(x)){x=m.makeAbsolute(x)}b.mobile.changePage(x.length&&x||m.get(),{type:y.length&&y.toLowerCase()||"get",data:B.serialize(),transition:B.jqmData("transition"),direction:B.jqmData("direction"),reloadPage:true});z.preventDefault()});function f(x){while(x){if(x.nodeName.toLowerCase()=="a"){break}x=x.parentNode}return x}b(document).bind("vclick",function(z){var y=f(z.target);if(y){var x=m.clean(y.getAttribute("href")||"#");if(x!=="#"&&x.replace(m.get(),"")!=="#"){b(y).closest(".ui-btn").not(".ui-disabled").addClass(b.mobile.activeBtnClass)}}});b(document).bind("click",function(y){var L=f(y.target);if(!L){return}var G=b(L),A=G.attr("href")||"#",F=m.hasProtocol(A),z=m.clean(A),B=G.is("[rel='external']"),K=m.isEmbeddedPage(z),J=(b.mobile.allowCrossDomainPages&&location.protocol==="file:"&&z.search(/^https?:/)!=-1),x=(m.isExternal(z)&&!J)||(B&&!K),I=G.is("[target]"),D=G.is(":jqmData(ajax='false')");if(G.is(":jqmData(rel='back')")){window.history.back();return false}if(z.replace(m.get(),"")=="#"){y.preventDefault();return}w=G.closest(".ui-btn");if(x||D||I||!b.mobile.ajaxEnabled){window.setTimeout(function(){u(true)},200);return}var H=G.jqmData("transition"),M=G.jqmData("direction"),E=(M&&M==="reverse")||G.jqmData("back"),C=G.attr("data-"+b.mobile.ns+"rel")||"page";if(m.isRelative(z)&&!F){z=m.makeAbsolute(z)}z=m.stripHash(z);b.mobile.changePage(z,{transition:H,reverse:E,role:C});y.preventDefault()});k.bind("hashchange",function(z,x){var B=m.stripHash(location.hash),A=b.mobile.urlHistory.stack.length===0?"none":e;if(!b.mobile.hashListeningEnabled||a.ignoreNextHashChange){a.ignoreNextHashChange=false;return}if(a.stack.length>1&&B.indexOf(v)>-1){if(!b.mobile.activePage.is(".ui-dialog")){a.directHashChange({currentUrl:B,isBack:function(){window.history.back()},isForward:function(){window.history.forward()}});return}else{var y=function(){B=b.mobile.urlHistory.getActive().page};a.directHashChange({currentUrl:B,isBack:y,isForward:y})}}if(B){b.mobile.changePage(B,{transition:A,changeHash:false,fromHashChange:true})}else{b.mobile.changePage(b.mobile.firstPage,{transition:A,changeHash:false,fromHashChange:true})}})})(jQuery);
/*!
 * jQuery Mobile v1.0a4.1
 * http://jquerymobile.com/
 *
