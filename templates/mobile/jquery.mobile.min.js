 ...
 * http://jquery.org/license
 */
(function(e,k,f){jQuery.mobile={};jQuery.extend(jQuery.mobile,{subPageUrlKey:"ui-page",nonHistorySelectors:"[data-rel=dialog]",activePageClass:"ui-page-active",activeBtnClass:"ui-btn-active",transitions:["slide","slideup","slidedown","pop","flip","fade"],defaultTransition:"slide",gradeA:function(){return jQuery.support.mediaquery}},jQuery.mobileDefaults);if(!jQuery.mobile.gradeA()){return}var s=jQuery(k),v=jQuery("html"),p=jQuery("head"),u,t=jQuery('<div class="ui-loader ui-body-a ui-corner-all"><span class="ui-icon ui-icon-loading spin"></span><h1>loading</h1></div>'),g,C,j=null,q=350,l=[{url:location.hash.replace(/^#/,"")}],x="[tabindex],a,button:visible,select:visible,input",c=null,n=true,b=h(location.protocol+"//"+location.host+location.pathname),z=[320,480,768,1024];e.mobile.urlStack=l;function a(G){return G.replace(/[^a-zA-Z0-9]/g,"-")}e.mobile.idStringEscape=a;function B(){jQuery.event.special.scrollstart.enabled=false;setTimeout(function(){k.scrollTo(0,0)},0);setTimeout(function(){jQuery.event.special.scrollstart.enabled=true},150)}function h(H){var G=H.replace(/#/,"").split("/");G.pop();return G.join("/")+(G.length?"/":"")}function r(G){return h(G||location.hash)}var E=!e.support.dynamicBaseTag?e.noop:function(G){e("#ui-base").attr("href",b+r(G))};var F=!e.support.dynamicBaseTag?e.noop:function(){e("#ui-base").attr("href",b)};e("form").live("submit",function(){var H=e(this).attr("method"),G=e(this).attr("action").replace(location.protocol+"//"+location.host,"");if(/^(:?\w+:)/.test(G)){return}if(G.indexOf("/")&&G.indexOf("#")!==0){G=r()+G}e.changePage({url:G,type:H,data:e(this).serialize()},f,f,true);return false});jQuery("a").live("click",function(I){var K=e(this),H=K.attr("href").replace(location.protocol+"//"+location.host,""),M=/^(:?\w+:)/.test(H)||K.is("[target],[rel=external]");if(H==="#"){return false}j=K.closest(".ui-btn").addClass(e.mobile.activeBtnClass);if(M){location.href=H}else{var L=K.data("transition"),G=K.data("back"),J=!K.is(e.mobile.nonHistorySelectors);c=K.attr("data-rel");if(H.indexOf("/")&&H.indexOf("#")!==0){H=r()+H}H.replace(/^#/,"");y(H,L,G,J)}I.preventDefault()});function D(G){if(G){v.removeClass("ui-loading")}else{t.appendTo(C).css({top:e(k).scrollTop()+75});v.addClass("ui-loading")}}function i(H){var G=H.find(".ui-title:eq(0)");if(G.length){G.focus()}else{H.find(x).eq(0).focus()}}function o(G){if(c){G.attr("data-role",c);c=f}}function w(G){var H=["data-role","data-theme","data-fullscreen"],I=G.wrap("<div>").parent();e.each(H,function(J){if(G.attr(H[J])){I.attr(H[J],G.attr(H[J]));G.removeAttr(H[J])}});return I}function A(G){if(!!j&&(!j.closest(".ui-page-active").length||G)){j.removeClass(e.mobile.activeBtnClass)}j=null}function y(U,R,M,S){var Q=e.type(U)==="array",V=Q?U[0]:e.activePage,U=Q?U[1]:U,H=fileUrl=e.type(U)==="string"?U.replace(/^#/,""):null,L=f,T="get",I=false,G=null,M=(M!==f)?M:(l.length>1&&l[l.length-2].url===H),R=R||e.mobile.defaultTransition;if(e.type(U)==="object"&&U.url){H=U.url,L=U.data,T=U.type,I=true;if(L&&T=="get"){H+="?"+L;L=f}}F();if(M){R=l.pop().transition}else{l.push({url:H,transition:R})}function K(){jQuery(document.activeElement).blur();V.data("page")._trigger("beforehide",{nextPage:U});U.data("page")._trigger("beforeshow",{prevPage:V});function W(){D(true);if(V.data("page")._trigger("hide",null,{nextPage:U})!==false&&U.data("page")._trigger("show",null,{prevPage:V})!==false){e.activePage=U}i(U);if(S&&H){n=false;location.hash=H}A();if(G!=null){G.remove()}}if(R){C.addClass("ui-mobile-viewport-transitioning");V.addClass(R+" out "+(M?"reverse":""));U.addClass(e.mobile.activePageClass+" "+R+" in "+(M?"reverse":""));U.animationComplete(function(){V.add(U).removeClass(" out in reverse "+e.mobile.transitions.join(" "));V.removeClass(e.mobile.activePageClass);W();C.removeClass("ui-mobile-viewport-transitioning")})}else{V.removeClass(e.mobile.activePageClass);U.addClass(e.mobile.activePageClass);W()}}function P(){o(U);U.page()}function J(W){return W.match("&"+jQuery.mobile.subPageUrlKey)?W.split("&"+jQuery.mobile.subPageUrlKey)[0]:W}if(H){U=jQuery("[id='"+H+"']"),fileUrl=J(H)}else{var N=U.attr("id"),O=J(N);if(N!=O){fileUrl=O}}if(U.length&&!I){if(fileUrl){E(fileUrl)}P();K()}else{if(U.length){G=U}D();e.ajax({url:fileUrl,type:T,data:L,success:function(W){E(fileUrl);var X=jQuery("<div></div>");X.get(0).innerHTML=W;U=X.find('[data-role="page"]');if(!e.support.dynamicBaseTag){var Y=r(fileUrl);U.find("[src],link[href]").each(function(){var aa=e(this).is("[href]")?"href":"src",Z=e(this).attr(aa);Z.replace(location.protocol+"//"+location.host+location.pathname,"");if(!/^(\w+:|#|\/)/.test(Z)){e(this).attr(aa,Y+Z)}})}if(U.attr("id")){U=w(U)}U.attr("id",fileUrl).appendTo(C);P();K()},error:function(){D(true);A(true);jQuery("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:e(k).scrollTop()+100}).appendTo(C).delay(800).fadeOut(400,function(){e(this).remove()})}})}}jQuery(function(){u=jQuery("body");D();s.bind("hashchange",function(H,G){if(!n){n=true;return}var J=location.hash,I=G?false:f;if(J){y(J,I)}else{if(e.activePage.length&&!g.is(e.activePage)&&!G){y(g,I,true)}else{g.trigger("pagebeforeshow",{prevPage:e("")});g.addClass(e.mobile.activePageClass);D(true);if(g.trigger("pageshow",{prevPage:e("")})!==false){i(g)}}}})});v.addClass("ui-mobile");s.bind("orientationchange.htmlclass",function(G){v.removeClass("portrait landscape").addClass(G.orientation)});function m(){var G=s.width(),K="min-width-",I="max-width-",M=[],J=[],L="px",H;v.removeClass(K+z.join(L+" "+K)+L+" "+I+z.join(L+" "+I)+L);e.each(z,function(N){if(G>=z[N]){M.push(K+z[N]+L)}if(G<=z[N]){J.push(I+z[N]+L)}});if(M.length){H=M.join(" ")}if(J.length){H+=" "+J.join(" ")}v.addClass(H)}s.bind("orientationchange resize",m);m();e.mobile.addResolutionBreakpoints=function(G){if(e.type(G)==="array"){z=z.concat(G)}else{z.push(G)}m()};var d=p.prepend('<meta name="viewport" content="width=device-width, minimum-scale=1, maximum-scale=1" />'+(e.support.dynamicBaseTag?'<base  href="" id="ui-base" />':""));F();jQuery.fn.animationComplete=function(G){if(jQuery.support.WebKitAnimationEvent){return jQuery(this).one("webkitAnimationEnd",G)}else{setTimeout(G,q)}};jQuery.extend({pageLoading:D,changePage:y,hideBrowserChrome:B});jQuery(function(){var G=jQuery("[data-role='page']");g=e.activePage=G.first();C=g.parent().addClass("ui-mobile-viewport");jQuery.extend({pageContainer:C});G.page();s.trigger("hashchange",[true]);s.trigger("orientationchange.htmlclass")});s.load(B).unload(A)})(jQuery,this);(function(a){a.themeswitcher=function(){var d="/stylesheets/compiled/jquery/mobile/",b=["default","valencia"],e=a.activePage,f=a("<div data-role='dialog' data-theme='a'><div data-role='header' data-theme='b'><div class='ui-title'>Switch Theme:</div></div><div data-role='content' data-theme='c'><ul data-role='listview' data-inset='true'></ul></div></div>").appendTo(a.pageContainer),h=f.find("ul");a.each(b,function(j){a("<li><a href='#'>"+b[j].charAt(0).toUpperCase()+b[j].substr(1)+"</a></li>").click(function(){g(b[j]);c();return false}).appendTo(h)});function g(i){a("head").append("<link rel='stylesheet' href='"+d+i+".css' />")}function c(){a.changePage([f,e],"pop",true);f.bind("pagehide",function(){f.remove()});return false}f.find(".ui-btn-left").click(c);f.page();a.changePage([e,f],"pop",false)}})(jQuery);