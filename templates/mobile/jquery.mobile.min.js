 ...
 */
(function(b,d){if(b.cleanData){var c=b.cleanData;b.cleanData=function(e){for(var f=0,g;(g=e[f])!=null;f++){b(g).triggerHandler("remove")}c(e)}}else{var a=b.fn.remove;b.fn.remove=function(e,f){return this.each(function(){if(!f){if(!e||b.filter(e,[this]).length){b("*",this).add([this]).each(function(){b(this).triggerHandler("remove")})}}return a.call(b(this),e,f)})}}b.widget=function(f,h,e){var g=f.split(".")[0],j;f=f.split(".")[1];j=g+"-"+f;if(!e){e=h;h=b.Widget}b.expr[":"][j]=function(k){return !!b.data(k,f)};b[g]=b[g]||{};b[g][f]=function(k,l){if(arguments.length){this._createWidget(k,l)}};var i=new h();i.options=b.extend(true,{},i.options);b[g][f].prototype=b.extend(true,i,{namespace:g,widgetName:f,widgetEventPrefix:b[g][f].prototype.widgetEventPrefix||f,widgetBaseClass:j},e);b.widget.bridge(f,b[g][f])};b.widget.bridge=function(f,e){b.fn[f]=function(i){var g=typeof i==="string",h=Array.prototype.slice.call(arguments,1),j=this;i=!g&&h.length?b.extend.apply(null,[true,i].concat(h)):i;if(g&&i.charAt(0)==="_"){return j}if(g){this.each(function(){var k=b.data(this,f);if(!k){throw"cannot call methods on "+f+" prior to initialization; attempted to call method '"+i+"'"}if(!b.isFunction(k[i])){throw"no such method '"+i+"' for "+f+" widget instance"}var l=k[i].apply(k,h);if(l!==k&&l!==d){j=l;return false}})}else{this.each(function(){var k=b.data(this,f);if(k){k.option(i||{})._init()}else{b.data(this,f,new e(i,this))}})}return j}};b.Widget=function(e,f){if(arguments.length){this._createWidget(e,f)}};b.Widget.prototype={widgetName:"widget",widgetEventPrefix:"",options:{disabled:false},_createWidget:function(f,g){b.data(g,this.widgetName,this);this.element=b(g);this.options=b.extend(true,{},this.options,this._getCreateOptions(),f);var e=this;this.element.bind("remove."+this.widgetName,function(){e.destroy()});this._create();this._trigger("create");this._init()},_getCreateOptions:function(){var e={};if(b.metadata){e=b.metadata.get(element)[this.widgetName]}return e},_create:function(){},_init:function(){},destroy:function(){this.element.unbind("."+this.widgetName).removeData(this.widgetName);this.widget().unbind("."+this.widgetName).removeAttr("aria-disabled").removeClass(this.widgetBaseClass+"-disabled ui-state-disabled")},widget:function(){return this.element},option:function(f,g){var e=f;if(arguments.length===0){return b.extend({},this.options)}if(typeof f==="string"){if(g===d){return this.options[f]}e={};e[f]=g}this._setOptions(e);return this},_setOptions:function(f){var e=this;b.each(f,function(g,h){e._setOption(g,h)});return this},_setOption:function(e,f){this.options[e]=f;if(e==="disabled"){this.widget()[f?"addClass":"removeClass"](this.widgetBaseClass+"-disabled ui-state-disabled").attr("aria-disabled",f)}return this},enable:function(){return this._setOption("disabled",false)},disable:function(){return this._setOption("disabled",true)},_trigger:function(f,g,h){var k=this.options[f];g=b.Event(g);g.type=(f===this.widgetEventPrefix?f:this.widgetEventPrefix+f).toLowerCase();h=h||{};if(g.originalEvent){for(var e=b.event.props.length,j;e;){j=b.event.props[--e];g[j]=g.originalEvent[j]}}this.element.trigger(g,h);return !(b.isFunction(k)&&k.call(this.element[0],g,h)===false||g.isDefaultPrevented())}}})(jQuery);(function(a){a.widget("mobile.widget",{_getCreateOptions:function(){var c=this.element,b={};a.each(this.options,function(d){var e=c.data(d.replace(/[A-Z]/g,function(f){return"-"+f.toLowerCase()}));if(e!==undefined){b[d]=e}});return b}})})(jQuery);$.media=(function(){var b={},a=$("html"),c=$("<div id='jquery-mediatest'>"),d=$("<body>").append(c);return function(f){if(!(f in b)){var e=$("<style type='text/css'>@media "+f+"{#jquery-mediatest{position:absolute;}}</style>");a.prepend(d).prepend(e);b[f]=c.css("position")==="absolute";d.add(e).remove()}return b[f]}})();var fakeBody=$("<body>").prependTo("html"),fbCSS=fakeBody[0].style,vendors=["webkit","moz","o"],webos=window.palmGetResource||window.PalmServiceBridge,bb=window.blackberry;function propExists(d){var c=d.charAt(0).toUpperCase()+d.substr(1),b=(d+" "+vendors.join(c+" ")+c).split(" ");for(var a in b){if(fbCSS[a]!==undefined){return true}}}function baseTagTest(){var d=location.protocol+"//"+location.host+location.pathname+"ui-dir/",c=$("<base>",{href:d}).appendTo("head"),b=$("<a href='testurl'></a>").prependTo(fakeBody),a=b[0].href;c.remove();return a.indexOf(d)===0}$.extend($.support,{orientation:"orientation" in window,touch:"ontouchend" in document,WebKitAnimationEvent:typeof WebKitTransitionEvent==="object",pushState:!!history.pushState,mediaquery:$.media("only all"),cssPseudoElement:!!propExists("content"),boxShadow:!!propExists("boxShadow")&&!bb,scrollTop:("pageXOffset" in window||"scrollTop" in document.documentElement||"scrollTop" in fakeBody[0])&&!webos,dynamicBaseTag:baseTagTest()});fakeBody.remove();if(!$.support.boxShadow){$("html").addClass("ui-mobile-nosupport-boxshadow")}$.each("touchstart touchmove touchend orientationchange tap taphold swipe swipeleft swiperight scrollstart scrollstop".split(" "),function(b,a){$.fn[a]=function(c){return c?this.bind(a,c):this.trigger(a)};$.attrFn[a]=true});var supportTouch=$.support.touch,scrollEvent="touchmove scroll",touchStartEvent=supportTouch?"touchstart":"mousedown",touchStopEvent=supportTouch?"touchend":"mouseup",touchMoveEvent=supportTouch?"touchmove":"mousemove";$.event.special.scrollstart={enabled:true,setup:function(){var a=this,d=$(a),c,e;function b(g,h){c=h;var f=g.type;g.type=c?"scrollstart":"scrollstop";$.event.handle.call(a,g);g.type=f}d.bind(scrollEvent,function(f){if(!$.event.special.scrollstart.enabled){return}if(!c){b(f,true)}clearTimeout(e);e=setTimeout(function(){b(f,false)},50)})}};$.event.special.tap={setup:function(){var a=this,b=$(a);b.bind(touchStartEvent,function(f){if(f.which&&f.which!==1){return}var e=false,c=true,d,h;function g(){e=true}h=setTimeout(function(){if(c&&!e){d=f.type;f.type="taphold";$.event.handle.call(a,f);f.type=d}},750);b.one(touchMoveEvent,g).one(touchStopEvent,function(i){b.unbind(touchMoveEvent,g);clearTimeout(h);c=false;if(!e){d=i.type;i.type="tap";$.event.handle.call(a,i);i.type=d}})})}};$.event.special.swipe={setup:function(){var a=this,b=$(a);b.bind(touchStartEvent,function(d){var f=d.originalEvent.touches?d.originalEvent.touches[0]:d,g={time:(new Date).getTime(),coords:[f.pageX,f.pageY],origin:$(d.target)},c;function e(h){if(!g){return}var i=h.originalEvent.touches?h.originalEvent.touches[0]:h;c={time:(new Date).getTime(),coords:[i.pageX,i.pageY]};if(Math.abs(g.coords[0]-c.coords[0])>10){h.preventDefault()}}b.bind(touchMoveEvent,e).one(touchStopEvent,function(h){b.unbind(touchMoveEvent,e);if(g&&c){if(c.time-g.time<1000&&Math.abs(g.coords[0]-c.coords[0])>30&&Math.abs(g.coords[1]-c.coords[1])<20){g.origin.trigger("swipe").trigger(g.coords[0]>c.coords[0]?"swipeleft":"swiperight")}}g=c=undefined})})}};$.event.special.orientationchange={orientation:function(a){return document.body&&a.width()/a.height()<1.1?"portrait":"landscape"},setup:function(){var b=this,d=$(b),a=$.event.special.orientationchange.orientation(d);function c(){var e=$.event.special.orientationchange.orientation(d);if(a!==e){$.event.handle.call(b,"orientationchange",{orientation:e});a=e}}if($.support.orientation){b.addEventListener("orientationchange",c,false)}else{d.bind("resize",c)}}};$.each({scrollstop:"scrollstart",taphold:"tap",swipeleft:"swipe",swiperight:"swipe"},function(b,a){$.event.special[b]={setup:function(){$(this).bind(a,$.noop)}}});
/*
 ...
 * http://jquery.org/license
 */
(function(q,D,l){if(!q.support.mediaquery){return}q.mobile={};q.extend(q.mobile,{subPageUrlKey:"ui-page",degradeInputs:{color:true,date:true,datetime:true,"datetime-local":true,email:true,month:true,number:true,range:true,search:true,tel:true,time:true,url:true,week:true},addBackBtn:true});var J=q(D),B=q("html"),t=q("head"),z,w=q('<div class="ui-loader ui-body-a ui-corner-all"><span class="ui-icon ui-icon-loading spin"></span><h1>loading</h1></div>'),G,b,o="ui-page-start",a="ui-page-active",k="ui-btn-active",v=null,I,m,e="slide slideup slidedown pop flip fade",K=350,H="Back",s=[{url:location.hash.replace(/^#/,""),transition:"slide"}],u="[tabindex],a,button:visible,select:visible,input",i=null,L=true,f="[data-rel=dialog]",c=location.protocol+"//"+location.host+location.pathname,C=[320,480,768,1024];$.mobile.urlStack=s;function j(N){return N.replace(/[^a-zA-Z0-9]/g,"-")}$.mobile.idStringEscape=j;function h(){q.event.special.scrollstart.enabled=false;setTimeout(function(){D.scrollTo(0,0)},0);setTimeout(function(){q.event.special.scrollstart.enabled=true},150)}function n(N){var P=N||location.hash,O=P.replace(/#/,"").split("/");if(O.length&&/[.|&]/.test(O[O.length-1])){O.pop()}O=O.join("/");if(O!==""&&O.charAt(O.length-1)!=="/"){O+="/"}return O}var F=!$.support.dynamicBaseTag?$.noop:function(N){$("#ui-base").attr("href",c+n(N))};var p=!$.support.dynamicBaseTag?$.noop:function(){$("#ui-base").attr("href",c)};q("a").live("click",function(Q){var S=$(this),O=S.attr("href").replace(location.protocol+"//"+location.host,""),T=/^(:?\w+:)/.test(O)||S.is("[target],[rel=external]");if(O==="#"){return false}v=S.closest(".ui-btn").addClass(k);if(T){location.href=O}else{var N=S.data("transition")||"slide",P=S.data("back")||l,R=!S.is(f);i=S.attr("data-rel");if(O.indexOf("/")&&O.indexOf("#")!==0){O=n()+O}O.replace(/^#/,"");g(O,N,P,R)}Q.preventDefault()});function A(N){if(N){B.removeClass("ui-loading")}else{w.appendTo(b).css({top:$(D).scrollTop()+75});B.addClass("ui-loading")}}function M(O){var N=O.find(".ui-title:eq(0)");if(N.length){N.focus()}else{O.find(u).eq(0).focus()}}function d(N){if(i){N.attr("data-role",i);i=l}}function r(N){var O=["data-role","data-theme","data-fullscreen"],P=N.wrap("<div>").parent();$.each(O,function(Q){if(N.attr(O[Q])){P.attr(O[Q],N.attr(O[Q]));N.removeAttr(O[Q])}});return P}function y(){if(v&&!v.closest(".ui-page-active").length){v.removeClass(k)}v=null}function g(X,V,Q,W){var U=$.type(X)==="array",Y=U?X[0]:$.activePage,X=U?X[1]:X,N=fileUrl=$.type(X)==="string"?X.replace(/^#/,""):null,Q=(Q!==l)?Q:(s.length>1&&s[s.length-2].url===N),V=(V!==l)?V:(I||"slide");I=l;m=l;p();if(Q){V=s.pop().transition}else{s.push({url:N,transition:V})}function P(){q(document.activeElement).blur();Y.data("page")._trigger("beforehide",{nextPage:X});X.data("page")._trigger("beforeshow",{prevPage:Y});function Z(){A(true);if(Y.data("page")._trigger("hide",null,{nextPage:X})!==false&&X.data("page")._trigger("show",null,{prevPage:Y})!==false){$.activePage=X}M(X);if(W&&N){L=false;location.hash=N;setTimeout(function(){L=true},500)}y()}if(V){Y.addClass(V+" out "+(Q?"reverse":""));X.addClass(a+" "+V+" in "+(Q?"reverse":""));X.animationComplete(function(){Y.add(X).removeClass(" out in reverse "+e);Y.removeClass(a);Z()})}else{Y.removeClass(a);X.addClass(a);Z()}}function T(){d(X);X.page()}function O(Z){return Z.match("&"+q.mobile.subPageUrlKey)?Z.split("&"+q.mobile.subPageUrlKey)[0]:Z}if(N){X=q("[id='"+N+"']"),fileUrl=O(N)}else{var R=X.attr("id"),S=O(R);if(R!=S){fileUrl=S}}if(X.length){if(fileUrl){F(fileUrl)}T();P()}else{A();$.ajax({url:fileUrl,success:function(Z){F(fileUrl);var aa=q("<div></div>");aa.get(0).innerHTML=Z;X=aa.find('[data-role="page"]');if(!$.support.dynamicBaseTag){var ab=n(fileUrl);X.find("[src],[href]").each(function(){var ad=$(this).is("[href]")?"href":"src",ac=$(this).attr(ad);ac.replace(location.protocol+"//"+location.host+location.pathname,"");if(!/^(\w+:|#|\/)/.test(ac)){$(this).attr(ad,ab+ac)}})}if(X.attr("id")){X=r(X)}X.attr("id",fileUrl).appendTo(b);T();P()},error:function(){A(true);y();q("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:$(D).scrollTop()+100}).appendTo(b).delay(800).fadeOut(400,function(){$(this).remove()})}})}}q(function(){z=q("body");A();J.bind("hashchange",function(O,N){if(!L){return}var Q=location.hash,P=(N&&N.manuallyTriggered)?false:l;if(Q){g(Q,P)}else{if($.activePage.length&&!G.is($.activePage)&&!(N&&N.manuallyTriggered)){g(G,P,true)}else{G.trigger("pagebeforeshow",{prevPage:$("")});G.addClass(a);A(true);if(G.trigger("pageshow",{prevPage:$("")})!==false){M(G)}}}})});B.addClass("ui-mobile");J.bind("orientationchange",function(N,O){B.removeClass("portrait landscape").addClass(O.orientation)});function x(){var N=J.width(),R="min-width-",P="max-width-",T=[],Q=[],S="px",O;B.removeClass(R+C.join(S+" "+R)+S+" "+P+C.join(S+" "+P)+S);$.each(C,function(U){if(N>=C[U]){T.push(R+C[U]+S)}if(N<=C[U]){Q.push(P+C[U]+S)}});if(T.length){O=T.join(" ")}if(Q.length){O+=" "+Q.join(" ")}B.addClass(O)}J.bind("orientationchange resize",x);x();$.mobile.addResolutionBreakpoints=function(N){if($.type(N)==="array"){C=C.concat(N)}else{C.push(N)}x()};var E=t.prepend('<meta name="viewport" content="width=device-width, minimum-scale=1, maximum-scale=1" />'+($.support.dynamicBaseTag?'<base  href="" id="ui-base" />':""));p();q.fn.animationComplete=function(N){if(q.support.WebKitAnimationEvent){return q(this).one("webkitAnimationEnd",N)}else{setTimeout(N,K)}};q.extend({pageLoading:A,changePage:g,hideBrowserChrome:h});q(function(){var N=q("[data-role='page']");G=$.activePage=N.first();b=G.parent();q.extend({pageContainer:b});if(!G.attr("id")){G.attr("id",o)}N.page();J.trigger("hashchange",{manuallyTriggered:true});B.addClass(q.event.special.orientationchange.orientation(J))});J.load(h)})(jQuery,this);(function(a){a.themeswitcher=function(){var d="http://jquerymobile.com/test/themes/",b=["default","valencia"],e=a(".ui-page-active"),f=a("<div data-role='dialog' data-theme='a'><div data-role='header' data-theme='b'><a href='#' class='ui-btn-left' data-icon='delete' data-iconpos='notext'>Cancel</a><div class='ui-title'>Switch Theme:</div></div><div data-role='content' data-theme='c'><ul data-role='listview' data-inset='true'></ul></div></div>").appendTo(a.pageContainer),h=f.find("ul");a.each(b,function(j){a("<li><a href='#'>"+b[j].charAt(0).toUpperCase()+b[j].substr(1)+"</a></li>").click(function(){g(b[j]);c();return false}).appendTo(h)});function g(i){a("head").append("<link rel='stylesheet' href='"+d+i+"/' />")}function c(){a.changePage(f,e,"pop",true);f.bind("pagehide",function(){f.remove()});return false}f.find(".ui-btn-left").click(c);f.page();a.changePage(e,f,"pop",false)}})(jQuery);