 ...
 * http://jquery.org/license
 */
(function(d,i,e){jQuery.mobile={subPageUrlKey:"ui-page",nonHistorySelectors:"dialog",activePageClass:"ui-page-active",activeBtnClass:"ui-btn-active",ajaxLinksEnabled:true,ajaxFormsEnabled:true,transitions:["slide","slideup","slidedown","pop","flip","fade"],defaultTransition:"slide",loadingMessage:"loading",metaViewportContent:"width=device-width, minimum-scale=1, maximum-scale=1",headExtras:e,gradeA:function(){return jQuery.support.mediaquery}};d(i.document).trigger("mobileinit");if(!jQuery.mobile.gradeA()){return}var r=jQuery(i),u=jQuery("html"),o=jQuery("head"),t,s=d.mobile.loadingMessage?jQuery('<div class="ui-loader ui-body-a ui-corner-all"><span class="ui-icon ui-icon-loading spin"></span><h1>'+d.mobile.loadingMessage+"</h1></div>"):e,q=d.mobile.metaViewportContent?d("<meta>",{name:"viewport",content:d.mobile.metaViewportContent}).prependTo(o):e,b=g(location.protocol+"//"+location.host+location.pathname),n=d.support.dynamicBaseTag?d("<base>",{href:b}).prependTo(o):e,f,B,F=null,j=[{url:location.hash.replace(/^#/,""),transition:e}],w="[tabindex],a,button:visible,select:visible,input",c=null,l=true,y=[320,480,768,1024];u.addClass("ui-mobile ui-mobile-rendering");if(d.mobile.headExtras){o.prepend(d.mobile.headExtras)}d.mobile.urlStack=j;function a(G){return G.replace(/[^a-zA-Z0-9]/g,"-")}d.mobile.idStringEscape=a;function A(){jQuery.event.special.scrollstart.enabled=false;setTimeout(function(){i.scrollTo(0,0)},0);setTimeout(function(){jQuery.event.special.scrollstart.enabled=true},150)}function g(H){var G=H.replace(/#/,"").split("/");G.pop();return G.join("/")+(G.length?"/":"")}function p(G){return g(G||location.hash)}var D=!d.support.dynamicBaseTag?d.noop:function(G){n.attr("href",b+p(G))};var E=!d.support.dynamicBaseTag?d.noop:function(){n.attr("href",b)};E();d("form").live("submit",function(I){if(!d.mobile.ajaxFormsEnabled){return}var H=d(this).attr("method"),G=d(this).attr("action").replace(location.protocol+"//"+location.host,"");if(/^(:?\w+:)/.test(G)){return}if(G.indexOf("/")&&G.indexOf("#")!==0){G=p()+G}d.changePage({url:G,type:H,data:d(this).serialize()},e,e,true);I.preventDefault()});jQuery("a").live("click",function(I){var K=d(this),H=K.attr("href").replace(location.protocol+"//"+location.host,""),M=/^(:?\w+:)/.test(H)||K.is("[target],[rel=external]");if(H==="#"){return false}F=K.closest(".ui-btn").addClass(d.mobile.activeBtnClass);if(M||!d.mobile.ajaxLinksEnabled){location.href=H}else{var L=K.data("transition"),G=K.data("back"),J=!K.is("[data-rel="+d.mobile.nonHistorySelectors+"]");c=K.attr("data-rel");if(H.indexOf("/")&&H.indexOf("#")!==0){H=p()+H}H.replace(/^#/,"");x(H,L,G,J)}I.preventDefault()});function C(G){if(G){u.removeClass("ui-loading")}else{if(d.mobile.loadingMessage){s.appendTo(B).css({top:d(i).scrollTop()+75})}u.addClass("ui-loading")}}function h(H){var G=H.find(".ui-title:eq(0)");if(G.length){G.focus()}else{H.find(w).eq(0).focus()}}function m(G){if(c){G.attr("data-role",c);c=e}}d.mobile.updateHash=function(G,H){if(H){l=false}location.hash=G};function v(G){var H=["data-role","data-theme","data-fullscreen"],I=G.wrap("<div>").parent();d.each(H,function(J){if(G.attr(H[J])){I.attr(H[J],G.attr(H[J]));G.removeAttr(H[J])}});return I}function z(G){if(!!F&&(!F.closest(".ui-page-active").length||G)){F.removeClass(d.mobile.activeBtnClass)}F=null}function x(U,R,M,S){var Q=d.type(U)==="array",V=Q?U[0]:d.activePage,U=Q?U[1]:U,H=fileUrl=d.type(U)==="string"?U.replace(/^#/,""):null,L=e,T="get",I=false,G=null,M=(M!==e)?M:(j.length>1&&j[j.length-2].url===H),R=(R!==e)?R:d.mobile.defaultTransition;if(d.type(U)==="object"&&U.url){H=U.url,L=U.data,T=U.type,I=true;if(L&&T=="get"){H+="?"+L;L=e}}E();if(M){R=j.pop().transition}else{j.push({url:H,transition:R})}function K(){jQuery(i.document.activeElement).blur();V.data("page")._trigger("beforehide",{nextPage:U});U.data("page")._trigger("beforeshow",{prevPage:V});function W(){C(true);if(V.data("page")._trigger("hide",null,{nextPage:U})!==false&&U.data("page")._trigger("show",null,{prevPage:V})!==false){d.activePage=U}h(U);if(S&&H){d.mobile.updateHash(H,true)}z();if(G!=null){G.remove()}}if(R&&(R!=="none")){B.addClass("ui-mobile-viewport-transitioning");V.addClass(R+" out "+(M?"reverse":""));U.addClass(d.mobile.activePageClass+" "+R+" in "+(M?"reverse":""));U.animationComplete(function(){V.add(U).removeClass(" out in reverse "+d.mobile.transitions.join(" "));V.removeClass(d.mobile.activePageClass);W();B.removeClass("ui-mobile-viewport-transitioning")})}else{V.removeClass(d.mobile.activePageClass);U.addClass(d.mobile.activePageClass);W()}}function P(){m(U);U.page()}function J(W){return W.match("&"+jQuery.mobile.subPageUrlKey)?W.split("&"+jQuery.mobile.subPageUrlKey)[0]:W}if(H){U=jQuery("[id='"+H+"']"),fileUrl=J(H)}else{var N=U.attr("id"),O=J(N);if(N!=O){fileUrl=O}}if(U.length&&!I){if(fileUrl){D(fileUrl)}P();K()}else{if(U.length){G=U}C();d.ajax({url:fileUrl,type:T,data:L,success:function(W){D(fileUrl);var X=jQuery("<div></div>");X.get(0).innerHTML=W;U=X.find('[data-role="page"]');if(!d.support.dynamicBaseTag){var Y=p(fileUrl);U.find("[src],link[href]").each(function(){var aa=d(this).is("[href]")?"href":"src",Z=d(this).attr(aa);Z.replace(location.protocol+"//"+location.host+location.pathname,"");if(!/^(\w+:|#|\/)/.test(Z)){d(this).attr(aa,Y+Z)}})}if(U.attr("id")){U=v(U)}U.attr("id",fileUrl).appendTo(B);P();K()},error:function(){C(true);z(true);jQuery("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:d(i).scrollTop()+100}).appendTo(B).delay(800).fadeOut(400,function(){d(this).remove()})}})}}jQuery(function(){t=jQuery("body");C();r.bind("hashchange",function(H,G){if(!l){l=true;return}if(d(".ui-page-active").is("[data-role="+d.mobile.nonHistorySelectors+"]")){return}var J=location.hash,I=G?false:e;if(J){x(J,I)}else{if(d.activePage.length&&!f.is(d.activePage)&&!G){x(f,I,true)}else{f.trigger("pagebeforeshow",{prevPage:d("")});f.addClass(d.mobile.activePageClass);C(true);if(f.trigger("pageshow",{prevPage:d("")})!==false){h(f)}}}})});r.bind("orientationchange.htmlclass",function(G){u.removeClass("portrait landscape").addClass(G.orientation)});function k(){var G=r.width(),K="min-width-",I="max-width-",M=[],J=[],L="px",H;u.removeClass(K+y.join(L+" "+K)+L+" "+I+y.join(L+" "+I)+L);d.each(y,function(N){if(G>=y[N]){M.push(K+y[N]+L)}if(G<=y[N]){J.push(I+y[N]+L)}});if(M.length){H=M.join(" ")}if(J.length){H+=" "+J.join(" ")}u.addClass(H)}r.bind("orientationchange resize",k);k();d.mobile.addResolutionBreakpoints=function(G){if(d.type(G)==="array"){y=y.concat(G)}else{y.push(G)}k()};jQuery.fn.animationComplete=function(G){if(jQuery.support.cssTransitions){return jQuery(this).one("webkitAnimationEnd",G)}else{G()}};jQuery.extend({pageLoading:C,changePage:x,hideBrowserChrome:A});jQuery(function(){var G=jQuery("[data-role='page']");f=d.activePage=G.first();B=f.parent().addClass("ui-mobile-viewport");jQuery.extend({pageContainer:B});G.page();r.trigger("hashchange",[true]);r.trigger("orientationchange.htmlclass");u.removeClass("ui-mobile-rendering")});r.load(A).unload(z)})(jQuery,this);(function(a){a.themeswitcher=function(){var d="/stylesheets/compiled/jquery/mobile/",b=["default","valencia"],e=a.activePage,f=a("<div data-role='dialog' data-theme='a'><div data-role='header' data-theme='b'><div class='ui-title'>Switch Theme:</div></div><div data-role='content' data-theme='c'><ul data-role='listview' data-inset='true'></ul></div></div>").appendTo(a.pageContainer),h=f.find("ul");a.each(b,function(j){a("<li><a href='#'>"+b[j].charAt(0).toUpperCase()+b[j].substr(1)+"</a></li>").click(function(){g(b[j]);c();return false}).appendTo(h)});function g(i){a("head").append("<link rel='stylesheet' href='"+d+i+".css' />")}function c(){a.changePage([f,e],"pop",true);f.bind("pagehide",function(){f.remove()});return false}f.find(".ui-btn-left").click(c);f.page();a.changePage([e,f],"pop",false)}})(jQuery);